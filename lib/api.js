...
const DEFAULT_TX_CACHE_EXPIRATION = 5 * 60 * 5000 // 5 minutes until transactions must be refetched
...
class Api {
  ...
  async fetch (txid, refresh = false) {
    ...
    const cached = refresh ? null : this.txCache.get(txid)
    if (cached && Date.now() - cached.lastFetchedTime < this.txCacheExpiration) {
  ...
  _correctForServerUtxoIndexDelay (utxos, address) {
    ...
    this.broadcastCache.forEach(tx => {
      const inputSpendsOutput = (input, output) =>
        input.prevTxId.toString('hex') === output.txid &&
        input.outputIndex === output.vout

      utxos = utxos.filter(utxo => !tx.inputs.some(input => inputSpendsOutput(input, utxo)))
    })
  ...
  _copyCache (blockchain) {
    ...
    this.broadcastCache = [...blockchain.txCache]
  }
}
