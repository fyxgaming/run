...
class Xray {
  ...
  constructor (options = {}) {
    ...
    this.tokenSerializer = options.tokenSerializer || null
    ...
    this.scanners = [
      ...
      // new BasicArrayScanner(this._getAllowedIntrinsics('Array'), this.preferredIntrinsics.Array),
      // new Uint8ArrayScanner(this._getAllowedIntrinsics('Uint8Array'), this.preferredIntrinsics.Uint8Array),
      // new SetScanner(this._getAllowedIntrinsics('Set'), this.preferredIntrinsics.Set),
      // new MapScanner(this._getAllowedIntrinsics('Map'), this.preferredIntrinsics.Map)
    ]

    if (this.allowDeployables) this.scanners.unshift(new CodeScanner())
    if (this.allowTokens) this.scanners.unshift(new TokenScanner())
    if (this.allowDeployables) this.scanners.push(new ArbitraryObjectScanner(this.basicObjectScanner, this.tokenSerializer))
  }

  /**
   * Scans an object, which performs the operations
   * @param {object} x Object to scan
   */
  scan (x) {
   ...
   */
  cloneable (x) {
    if (this.caches.cloneable.has(x)) {
      return this.caches.cloneable.get(x)
    }
   ...
   */
  serializable (x) {
    if (this.caches.serializable.has(x)) {
      return this.caches.serializable.get(x)
    }
   ...
   */
  deserializable (x) {
    if (this.caches.deserializable.has(x)) {
      return this.caches.deserializable.get(x)
    }
...
class Builder {
  ...
  useTokenSerializer (tokenSerializer) { this.options.tokenSerializer = tokenSerializer; return this }
...
class Result {
  constructor (options) {
    ...
    if (options.allowTokens) this.tokens = new RunSet()
  }
}
...
class BasicObjectScanner {
  ...
  clone (x, xray) {
    const y = this.Object.create(this.Object.prototype)
...
class TokenSerializer {
  getTokenFromLocation (x) { throw new Error('Not implemented') }
  getLocationForToken (x) { throw new Error('Not implemented') }
  getSandboxForCode (x) { throw new Error('Not implemented') }
}
*/
...
class ArbitraryObjectScanner {
  constructor (basicObjectScanner, tokenSerializer) {
  ...
  deserializable (x, xray) {
    if (!this.tokenSerializer) throw new Error(`No token serializer provided to deserialize ${x}`)
    if (typeof x !== 'object' || !x) return false
    if (x.$class !== 'arbitraryObject') return false
    if (!this.tokenSerializer.getTokenFromLocation(x.type)) return false
    if (typeof x.value !== 'object') return false
    return true
  }

  clone (x, xray) {
    if (!this.tokenSerializer) throw new Error(`No token serializer provided to clone ${x}`)
    const clone = this.basicObjectScanner.clone(Object.assign({}, x), xray)
    const sandbox = this.tokenSerializer.getSandboxForCode(x.constructor)
    Object.setPrototypeOf(clone, sandbox.prototype)
    return clone
  }

  serialize (x, xray) {
    if (!this.tokenSerializer) throw new Error(`No token serializer provided to serialize ${x}`)
    const type = this.tokenSerializer.getLocationForToken(x.constructor)
  ...
  deserialize (x, xray) {
    if (!this.tokenSerializer) throw new Error(`No token serializer provided to deserialize ${x}`)
    const type = this.tokenSerializer.getTokenFromLocation(x.type)
    const obj = Object.assign(this.Object.create(this.Object.prototype), x.value)
