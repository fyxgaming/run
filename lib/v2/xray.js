...
class Xray {
  ...
  serialize (x) {
    ...
    const prevAllowDups = this.allowDups // Only external calls get deduped
    this.allowDups = false
    try {
      const scanner = this.scanners.find(scanner => scanner.serializable(x, this))
      if (scanner) {
        const y = scanner.serialize(x, this)
        const z = prevAllowDups ? dedup(y, this) : y
        this.caches.serialize.set(x, z)
        return z
      }
