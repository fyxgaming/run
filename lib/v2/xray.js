...
// More efficient storage { $class: 'dup', n: 0 } -> { $dup: 0 }
...
class UndefinedScanner {
  ...
  deserializable (x, xray) { if (typeof x === 'object' && x && x.$class === 'undefined') return true }
...
class Uint8ArrayScanner {
  ...
  deserializable (x, xray) {
    if (typeof x !== 'object' || !x || x.$class !== 'Uint8Array') return
...
class SetScanner {
  ...
  deserializable (x, xray) {
    if (typeof x !== 'object' || !x || x.$class !== 'Set') return
...
class MapScanner {
  ...
  deserializable (x, xray) {
    if (typeof x !== 'object' || !x || x.$class !== 'Map') return
  ...
  deserializable (x, xray) {
    ...
    try {
      if (topLevel) {
        ...
        if (this.checkingDeserializability) {
          if (typeof x !== 'object' || !x || typeof x.$dup === 'undefined') return
          if (typeof x.$dup !== 'number') return false
          if (!Number.isInteger(x.$dup) || x.$dup < 0 || x.$dup > this.dups.length) return false
          return true
        ...
        if (this.deserializingDups) {
          // TODO
          return true
        }

        if (this.deserializingMaster) {
          // TODO
          return true
        }
      }
  ...
  deserialize (x, xray) {
    ...
    try {
      if (topLevel) {
        ...
        // Deserialize each dup partially, where dup refs are still present
        try {
        ...
        // Replace inner dups with the real things now
        try {
          this.cloningToDedup = true
          this.dups = xray.clone(this.dups)
        } finally {
          this.cloningToDedup = false
        }
        ...
        // If we are deserializing the dups, leave any dup references intact
        if (this.deserializingDups) {
          return null
        }

        // If we are deserializing the master, then replace any dups
        if (this.deserializingMaster) {
          return null
        }
      }
...
// x.dups -> deserialize, but leave dups intact
// clone -> replace dups with dups
// x.value -> deserialize and replace dups

// TODO
// This needs to be applied AFTER dups are created
// And on a different object

function redup (x) {
  if (typeof x !== 'object' || !x) return x
  if (typeof x.$dedup !== 'object') return x
  if (!Array.isArray(x.dups)) return x

  function replaceDups (y) {
    if (typeof y !== 'object' || !y) return y
    if (typeof y.$dup !== 'undefined') return x.dups[y.$dup]
