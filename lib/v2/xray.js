...
class Xray {
  ...
  deserializable (x) {
    ...
    const prevAllowDups = this.allowDups // Only external calls get reduped
    this.allowDups = false
    try {
      const redupedX = prevAllowDups ? redup(x) : x
      const value = this.scanners.some(scanner => scanner.deserializable(redupedX, this))
      this.caches.deserializable.set(x, value)
      return value
    } finally { this.allowDups = prevAllowDups }
  }
  ...
  deserialize (x) {
    ...
    try {
      const redupedX = prevAllowDups ? redup(x) : x
      const scanner = this.scanners.find(scanner => scanner.deserializable(redupedX, this))
      if (scanner) {
        const y = scanner.deserialize(redupedX, this)
...
class DedupScanner {
  ...
  scan (x, xray) { return undefined }
  cloneable (x, xray) { return false }

  serializable (x, xray) {
    const deduping = this.deduping
    this.deduping = true
    try {
      return !deduping && xray.serializable(x)
    } finally { this.deduping = deduping }
  }

  deserializable (x, xray) {
  }

  clone (x, xray) { return undefined }
