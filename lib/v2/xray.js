...
class Xray {
  constructor (options = {}) {
    ...
    this.allowDeployablas = !!options.allowDeployables
    ...
    this.scanners = [
      ...
      new BasicObjectScanner(this.preferredIntrinsics.Object),
  ...
  scan (x, result = new Result(this)) {
    if (this.allowDeployablas && this.deployable(x)) {
      result.deployables.add(x)
    }

    if (this.allowTokens && Protocol.isToken(x)) {
      result.tokens.add(x)
    }
  ...
  deployable (x) { return typeof x === 'function' && x.toString().indexOf('[native code]') === -1 }
...
class Builder {
  ...
  allowDeployables () { this.options.alloDeployables = true; return this }
...
// TODO
// Arbitrary classes
// Sets and maps respect tokens in jigs
