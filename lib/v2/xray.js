...
class DedupScanner {
  ...
  cloneable (x, xray) { }
  clone (x, xray) { }
  serializable (x, xray) { return this.runAtTopLevel(() => xray.serializable(x)) }
  deserializable (x, xray) {}
  serialize (x, xray) { return this.runAtTopLevel(() => this.dedup(x, xray)) }
  deserialize (x, xray) {}
  ...
  runAtTopLevel (topLevelFunc, notTopLevelFunc = () => {}) {
    const topLevel = this.topLevel
    this.topLevel = false
    try {
      return topLevel ? topLevelFunc() : notTopLevelFunc()
    } finally {
