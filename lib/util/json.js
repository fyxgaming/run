...
const _newObject = options => new options._outputIntrinsics.Object() // eslint-disable-line
const _newArray = options => new options._outputIntrinsics.Array() // eslint-disable-line
...
function _fromTokenJson (x, options = {}) {
  options._hostIntrinsics = options._hostIntrinsics || _hostIntrinsics
  options._sandboxIntrinsics = options._sandboxIntrinsics || _hostIntrinsics
  options._outputIntrinsics = options._sandboxIntrinsics || _hostIntrinsics
  return _deserialize(x, options)
}

function _deserialize (x, options) {
  switch (typeof x) {
    case 'string': return x
    case 'boolean': return x
    case 'number': if (isNaN(x) || !isFinite(x)) break; return x
    ...
    case 'function': {
      if (!x) return null
      return _deserializeObject(x, options)
    }
  }
  throw new Error(`Cannot deserialize ${_display(x)}`)
}

function _deserializeObject (x, options) {
  const proto2 = Object.getPrototypeOf(Object.getPrototypeOf(x))
