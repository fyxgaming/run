...
class TokenJSON {
  ...
  static _serialize (x, opts = {}) {
    ...
    opts._sandboxIntrinsics = opts._sandboxIntrinsics || opts._hostIntrinsics
...
function _deserializeObject (y, opts) {
  ...
  if (isBasicObject(y)) {
    ...
    if ($ === '$set') {
      ...
      if (!(typeof y.props === 'undefined' || isBasicObject(y.props))) _throwDes(y)
...
const protoLen = x => { let n = 0; do { n++; x = Object.getPrototypeOf(x) } while (x); return n }
const isBasicObject = x => protoLen(x) === 2
...
function _reviveTokens (getToken) {
  return x => {
    if (isBasicObject(x) && x.$ref) {
      return getToken(x.$ref)
    }
  }
}
...
const _replaceArbitraryObjects = (x, opts) => {
  if (typeof x === 'object') {
    const y = { }
    opts._cache.set(x, y)
...
const _reviveArbitraryObjects = (x, opts) => {
  if (isBasicObject(x) && x.$arb && x.T) {
    const x = opts._preparedContainer || _newObject(opts)
    delete opts._preparedContainer
    if (opts._onlyContainer) return x

    Object.assign(x, TokenJSON._deserialize(x.$arb, opts))

    const T = TokenJSON._deserialize(x.T, opts)
