...
function _serializeObject (x, opts) {
  ...
  const isSet = (x instanceof opts._hostIntrinsics.Set ||
    x instanceof opts._sandboxIntrinsics.Set) && !proto3(x)
  if (isSet) {
    const y = _newObject(opts)
    opts._cache.set(x, y)
  ...
  const isMap = (x instanceof opts._hostIntrinsics.Map ||
    x instanceof opts._sandboxIntrinsics.Map) && !proto3(x)
  if (isMap) {
    const y = _newObject(opts)
    opts._cache.set(x, y)
...
const proto2 = x => Object.getPrototypeOf(Object.getPrototypeOf(x))
const proto3 = x => { const p2 = proto2(x); return p2 && Object.getPrototypeOf(p2) }
const isBasicObject = x => !proto2(x)
const isBasicArray = x => Array.isArray(x) && !proto3(x)
