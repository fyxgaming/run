...
function _toTokenJson (x, opts = {}) {
  ...
  opts._replacers = opts._replacers || []
...
function _serializeObject (x, opts) {
  ...
  for (const replacer of opts._replacers) {
    const y = replacer(x)
    if (y) return y
  }
...
function _deserializeObject (x, opts) {
  ...
  for (const reviver of opts._revivers) {
    const y = reviver(x)
    if (y) return y
  }
...
function _refifyTokens (getLocation) {
  const Protocol = require('../kernel/protocol')
  return x => Protocol.isToken(x) && { $ref: getLocation(x) }
}

function _tokenizeRefs (getToken) {
  return x => x.$ref && getToken(x.$ref)
}

function _findAllTokenRefsInTokenJson (json) {
  const refs = new Set()
  _scanJsonForObjects(json, x => {
    if (x.$ref) refs.add(x.$ref)
  })
  return refs
}

function _scanJsonForObjects (x, scanner) {
  if (typeof x === 'object' || typeof x === 'function') {
    scanner(x)
    Object.keys(x).forEach(key => _scanJsonForObjects(x[key], scanner))
  }
}
