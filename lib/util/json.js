...
const { TokenMap, TokenSet } = require('./datatypes')
...
class TokenJSON {
  ...
  static _serialize (x, opts = {}) {
    ...
    opts._hostIntrinsics = opts._hostIntrinsics || Sandbox._instance._hostIntrinsics
    opts._sandboxIntrinsics = opts._sandboxIntrinsics || Sandbox._instance._intrinsics
    ...
    opts._cache = opts._cache || new TokenMap() // X -> JSON
    opts._dups = opts._dups || new TokenSet() // [X]
...
function _serializeObject (x, opts) {
  ...
  const isSet = (x instanceof opts._hostIntrinsics.Set ||
    x instanceof opts._sandboxIntrinsics.Set) && protoLen(x) === 3
  if (isSet) {
    const y = _newObject(opts)
    opts._cache.set(x, y)
  ...
  const isMap = (x instanceof opts._hostIntrinsics.Map ||
    x instanceof opts._sandboxIntrinsics.Map) && protoLen(x) === 3
  if (isMap) {
    const y = _newObject(opts)
    opts._cache.set(x, y)
  ...
  const isUint8Array = (x instanceof opts._hostIntrinsics.Uint8Array ||
    x instanceof opts._sandboxIntrinsics.Uint8Array) && protoLen(x) === 4
  if (isUint8Array) {
    const keys = Object.keys(x)
    if (keys.length !== x.length) _throwSer(x)
    const valid = n => Number.isInteger(n) && n <= 255 && n >= 0
    if (Object.keys(x).some(key => !valid(x[key]))) _throwSer(x)
...
function _deserializeObject (y, opts) {
  ...
  if (isBasicObject(y)) {
    ...
    if ($ === '$arr') {
      if (!(isBasicObject(y.$arr) && y.$arr)) _throwDes(y)
    ...
    if ($ === '$obj') {
      if (!(isBasicObject(y.$obj) && y.$obj)) _throwDes(y)
    ...
    // Sets
    if ($ === '$set') {
      if (!isBasicArray(y.$set)) _throwDes(y)
      if (!(isUndefined(y.props) || isBasicObject(y.props))) _throwDes(y)
    ...
    // Maps
    if ($ === '$map') {
      if (!isBasicArray(y.$map)) _throwDes(y)
      if (!(isUndefined(y.props) || isBasicObject(y.props))) _throwDes(y)
      ...
      for (const val of y.$map) {
        if (!isBasicArray(val) || val.length !== 2) _throwDes(y)
    ...
    if ($ === '$dedup') {
      ...
      if (!isBasicArray(y.dups)) _throwDes(y)
      if (!isUndefined(opts._dups)) _throwDes(y)
    ...
    if ($ === '$dup') {
      if (!isBasicArray(opts._dups)) _throwDes(y)
  ...
  if (opts._reviver) {
    const x = opts._preparedContainer || opts._reviver(y, opts)
    delete opts._preparedContainer
    if (x) return x
  }
...
function _replaceTokens (getLocation) {
  return (x, opts) => {
    if (_tokenType(x)) {
      const y = _newObject(opts)
      y.$ref = getLocation(x)
...
function _reviveTokens (getToken) {
  return y => {
    if (isBasicObject(y) && !isUndefined(y.$ref)) {
      return getToken(y.$ref)
    }
  }
}
...
const _replaceArbitraryObjects = () => {
  return (x, opts) => {
    if (typeof x === 'object') {
      ...
      y.T = TokenJSON._serialize(x.constructor, opts)
      return y
    }
  }
}
...
const _reviveArbitraryObjects = () => {
  return (y, opts) => {
    if (isBasicObject(y) && !isUndefined(y.$arb) && !isUndefined(y.T)) {
      const x = opts._preparedContainer || _newObject(opts)
      delete opts._preparedContainer
      if (opts._onlyContainer) return x

      Object.assign(x, TokenJSON._deserialize(y.$arb, opts))

      const T = TokenJSON._deserialize(y.T, opts)
