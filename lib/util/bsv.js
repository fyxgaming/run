...
function _txify(tx) {
  // Check if hex
  if (typeof tx === 'string') {
    return new bsv.Transaction(tx)
  }

  // Check if buffer
  if (tx instanceof Uint8Array) {
    const buffer = new bsv.deps.Buffer.from(tx)
    return new bsv.Transaction(buffer)
  }

  // Check if bsv transaction
  if (tx instanceof bsv.Transaction) {
    return tx
  }

  throw new Error(`${_display(tx)} is not a transaction`)
}

// ------------------------------------------------------------------------------------------------

module.exports = { _populatePreviousOutputs, _scriptHash, _patchBsv, _txify }
