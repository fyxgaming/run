...
function _generateSignature (tx, vin, script, satoshis, privateKey, sighashType = Signature.SIGHASH_ALL) {
  sighashType |= Signature.SIGHASH_FORKID
  const satoshisBN = new BN(satoshis)
  const flags = Interpreter.SCRIPT_ENABLE_SIGHASH_FORKID
  const hashbuf = _sighash(tx, sighashType, vin, script, satoshisBN, flags)
...
module.exports = { _populatePreviousOutputs, _scripthash, _patchBsv, _generateSignature }
