...
class Proxy2 {
  ...
  ownKeys (target) {
    let keys = this._handler._ownKeys
      ? this._handler._ownKeys(this._target) : Reflect.ownKeys(this._target)

    // Safari and Firefox doen't like upgrading classes. They update the order of length
    // and name on the proxy, and only the proxy. We move these properties back to their
    // original position to keep jigs deterministic.
    if (typeof target === 'function') {
      keys = keys.filter(x => x !== 'length' && x !== 'name' && x !== 'prototype')
      keys.unshift('length', 'prototype', 'name')
    }
