 ...
 * not expected to be useful outside of this library. The underlying target may also be changed,
 * but this is an advanced operations. If the underlying target changes, it is important that
 * higher-level handlers are able to deal with the proxy invariants, which affects "prototype":
 *
 *    https://www.ecma-international.org/ecma-262/8.0/#sec-proxy-object-internal-methods-and-internal-slots-get-p-receiver
 *
 * It is not necessary to use Proxy2 everywhere in Run. Proxy2 is used only in jigs and
 * inner jig objects to ensure that intrinsics work and that we can look up targets.
 *
...
class Proxy2 {
  ...
  construct (target, args) {
    if (!this._handler._construct) return Reflect.construct(this._target, args)
    return this._handler._construct(this._target, args)
  }
  ...
  get (target, prop, receiver) {
    ...
    const isSet = _isBasicSet(this._target)
    const isMap = _isBasicMap(this._target)
    const isUint8Array = _isBasicUint8Array(this._target)
  ...
  static _setTarget (proxy, newTarget) {
    ...
    handler._target = newTarget
    _assert(oldTarget)
    PROXIES.delete(oldTarget)
    HANDLERS.delete(oldTarget)
    PROXIES.set(newTarget, proxy)
