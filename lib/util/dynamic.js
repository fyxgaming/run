...
const { _checkArgument, _text } = require('./misc')
...
class DynamicHandler {
  ...
  _setInnerType (T) {
    ...
    _checkArgument(typeof T === 'function', `Inner type must be a function type: ${_text(T)}`)
    ...
    const mismatchError = `Classes can only be changed to classes and functions to functions: ${_text(T)}`
