...
class DynamicHandler {
  ...
  deleteProperty (target, prop) {
    // Don't allow __type__ or prototype to be deleted
    if (prop === '__type__') return false
    if (prop === 'prototype') return false
  ...
  _setInnerType (T, proxy) {
    ...
    // Note: This doesn't check parents, and it probably should!
    _checkArgument(!Object.getOwnPropertyNames(T).includes('toString'), 'toString is a reserved property')
