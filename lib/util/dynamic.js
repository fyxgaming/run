  ...
  get (target, prop, receiver) {
    ...
    if (typeof target[prop] === 'function') {
      ...
      if (prop === 'toString') {
        ...
        // Create a special toString() method that does the above
        // Test persistent
        // If it's an outer type, can I get the inner type?

        const self = this

        function toString () { // eslint-disable-line
          const callingThis = this === self._outerType || this === self._proxy
          const toStringTarget = callingThis ? self._innerType : this
