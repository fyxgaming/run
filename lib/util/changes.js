...
class Changes {
  constructor () {
    this._origs = new Map() // Object -> State
    this._owners = new Map() // Object -> Owner
  }
  ...
  _setOriginalValue (owner, obj, prop, val) {
    this._preserve(obj)
    this._origs.get(obj)[prop] = val
  }
  ...
  _rollback () {
    return Membrane._sudo(() => {
      this._origs.forEach((before, after) => {
        ...
        if (after instanceof SI.Set || after instanceof HI.Set) {
          after.clear()
          before.forEach(value => after.add(value))
        }
        if (after instanceof SI.Map || after instanceof HI.Map) {
          after.clear()
          before.forEach((value, key) => after.set(key, value))
        }
      })
    })
  }
