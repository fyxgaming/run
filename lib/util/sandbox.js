...
const { _display, _sourceCode } = require('./misc')
...
class Sandbox {
  constructor () {
    ...
    this._intrinsics = compartment.evaluate(getIntrinsicsSrc)

    this._intrinsics.Map = this._sandboxType(TokenMap)[0]
    this._intrinsics.Set = this._sandboxType(TokenSet)[0]

    this._hostIntrinsics = eval(getIntrinsicsSrc) // eslint-disable-line
  }

  _sandboxType (T, env) {
    if (Sandbox.excludes.includes(T)) {
      Log._info(TAG, 'Excluding from sandbox', _display(T))
      return [T, null]
    }

    return this._evaluate(_sourceCode(T), env || T.deps)
  }

  _evaluate (code, env = {}) {
    Log._info(TAG, 'Evaluating', _display(code.replace(/\s+/g, ' ')))
...
let getIntrinsicsSrc = 'const x = {}\n'
intrinsicNames.forEach(name => {
  getIntrinsicsSrc += `x.${name} = typeof ${name} !== 'undefined' ? ${name} : undefined\n`
})
getIntrinsicsSrc += 'x'
 ...
 */
let instance = null
Object.defineProperty(Sandbox, '_instance', {
  get: () => {
