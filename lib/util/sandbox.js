...
const { _text, _sourceCode } = require('./type')
...
class Sandbox {
  constructor () {
    Log._debug(TAG, 'Creating deterministic realm')
    ...
    this._intrinsics.Map = this._sandboxType(SafeMap, SafeMap.deps, true)[0]
    this._intrinsics.Set = this._sandboxType(SafeSet, SafeSet.deps, true)[0]
  ...
  _sandboxType (T, env, builtin = false) {
    if ((process.env.COVER && builtin) || this.excludes.includes(T)) {
      Log._debug(TAG, 'Excluding', _text(T))
      const globalThis = typeof global !== 'undefined' ? global : window
      return [T, globalThis]
    }

    const code = _sourceCode(T)

    return this._evaluate(code, env)
  }

  _evaluate (code, env = {}) {
    Log._debug(TAG, 'Evaluating', _text(code.replace(/\s+/g, ' ')))
