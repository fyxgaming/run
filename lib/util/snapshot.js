...
const { _UNDEPLOYED } = require('./bindings')
  ...
  _rollback (e) {
    ...
    return _sudo(() => {
      ...
      if (this._bindingsOnly) {
        if (e) {
          const error = `error://A previous error occurred\n\n${e}`
          _setOwnProperty(this._jig, 'location', error)
        } else {
          _setOwnProperty(this._jig, 'location', _UNDEPLOYED)
        }
        return
      }
      ...
      if (e) {
        const Jig = require('../kernel/jig')
        if (this._jig instanceof Jig && this._props.location === _UNDEPLOYED) {
          const error = `error://Deploy failed\n\n${e}`
          _setOwnProperty(this._jig, 'origin', error)
          _setOwnProperty(this._jig, 'location', error)
        }
      }
    })
