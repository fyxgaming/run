...
const { RequestError, TimeoutError } = require('./errors')
 ...
 * @param {?number} options.cache How long to cache this response for
...
async function request (url, options = {}) {
  ...
  if (options.method === 'GET' && options.dedup) {
    return await _dedup(DEDUP, id, async () => {
      return await _cache(CACHE, id, options.cache, async () => {
 ...
 * @param {f} request Async function to perform
 */
async function _dedup (cache, key, f) {
...
async function _cache (cache, key, ms, request) {
  ...
  if (!ms) return await request()

  try {
    const result = await request()
    cache[key] = { expires: now + ms, result }
    return result
  } catch (error) {
    cache[key] = { expires: now + ms, error }
    throw error
  }
}
