...
const {
  _text, _isBasicObject, _isBasicArray, _isBasicSet, _isBasicMap, _isBasicUint8Array,
  _isUndefined, _isNegativeZero, _deterministicObjectKeys
} = require('./misc')
...
class Codec {
  ...
  _encodeAny (x, path) {
    switch (typeof x) {
      ...
      case 'number':
        if (isNaN(x) || !isFinite(x) || _isNegativeZero(x)) {
          const y = this._newObject()
          if (isNaN(x)) y.$nan = 1
          if (x === Infinity) y.$inf = 1
          if (x === -Infinity) y.$ninf = 1
          if (_isNegativeZero(x)) y.$n0 = 1
          return y
        }
        return x
      case 'symbol': break
