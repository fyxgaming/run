...
class Codec {
  ...
  _encode (x) {
    ...
    const dedup = this._shouldDedup
    try {
      this._shouldDedup = false
  ...
  _encodeObject (x) {
    ...
    if (isUint8Array) {
      ...
      if (keys.some(key => !valid(x[key]))) _throwEnc(x)
    ...
    if (this._jigSaver) {
      ...
      const y = { }
      this._cache.set(x, y)
      y.$arb = this._encode(Object.assign({}, x))
  ...
  // This check works on all intrinsics, host or sandbox
    ...
    // Check if there are any special props
      ...
      if ($ === '$top') {
        ...
        if (!_isUndefined(this._dups)) _throwDec(y)

        // Create all the dup containers first
        this._onlyContainer = true
        this._dups = y.dups.map(dup => this._decodeAny(dup))
      ...
      if ($ === '$dup') {
        if (!_isBasicArray(this._dups)) _throwDec(y)
        if (!(typeof y.$dup === 'number' && y.$dup >= 0 && y.$dup < this._dups.length)) _throwDec(y)
