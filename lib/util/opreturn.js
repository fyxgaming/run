...
function _checkRunTransaction (tx) {
  ...
  const protocol = tx.outputs[0].script.chunks[3].buf
  const isAllowedProtocol = protocol.length === 1 && protocol[0] === PROTOCOL_VERSION
  ...
  if (!isAllowedProtocol) {
    ...
    const details = `Txid: ${tx.hash}\nProtocol: ${protocol.toString('hex')}`
    throw new Error(`Unsupported run protocol: ${tx.hash}\n\n${details}\n\n${suggestion}`)
  }
}
