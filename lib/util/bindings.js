...
const { _text, _setOwnProperty, _assert, _defined } = require('./misc')
 ...
 * @returns {txid, vout, vdel, berry, hash, version, error, undeployed, commitid, recordid, native} Parsed locations. Fields are optional.
 */
...
function _compileLocation (parts) {
  ...
  _assert(_defined(prefix))

  // Suffix
  _assert(_defined(parts.vout) || _defined(parts.vdel))
  const suffix = _defined(parts.vout) ? `_o${parts.vout}` : `_d${parts.vdel}`
