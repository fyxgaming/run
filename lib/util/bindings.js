...
const CREATION_LOCATION = /^(?<txid>[a-f0-9]+)?_(?:(?:o(?<vout>[0-9]+))|(?:d(?<vdel>[0-9]+)))(?:_(?<berry>.*))?$/s
...
function _location (s) {
  ...
  const match = s.match(CREATION_LOCATION) || s.match(COMMIT_LOCATION) || s.match(RECORD_LOCATION) ||
