...
const RECORD_LOCATION = /^(?<protocol>record):\/\/(?<txid>[a-f0-9]+)?_(?:(?:o(?<vout>[0-9]+))|(?:i(?<vin>[0-9]+)))$/
 ...
 * @returns {txid, vout, vin, path, error, record} Parsed locations. Fields are optional.
 */
function _location (s, flags) {
  if (typeof s !== 'string') throw new Error(`Location is not a string: ${_text(s)}`)

  const match = s.match(RESOURCE_LOCATION) || s.match(RECORD_LOCATION) || s.match(ERROR_LOCATION)
  ...
  if (match) {
    ...
    if (ret.record && typeof ret.vout === 'undefined') {
      throw new Error(`Record locations may only be outputs: ${_text(s)}`)
    }
    ...
    if (flags & _USER && !('txid' in ret && 'vout' in ret && !ret.record)) {
...
module.exports = { _location, _nonce, _owner, _satoshis, _USER, _JIG, _BERRY, _BINDINGS, _Unbound }
