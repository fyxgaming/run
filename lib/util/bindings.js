...
const { _text, _setOwnProperty, _assert } = require('./misc')
...
const NATIVE_LOCATION = /^(?<protocol>native):\/\/(?<nativeid>[a-zA-Z0-9_$]+)/s
const RECORD_LOCATION = /^(?<protocol>record):\/\/(?<recordid>[a-f0-9]{64})_(?:o(?<vout>[0-9]+)|(?:d(?<vdel>[0-9]+)))$/
const COMMIT_LOCATION = /^(?<protocol>commit):\/\/(?<commitid>[a-f0-9]{64})_(?:o(?<vout>[0-9]+)|(?:d(?<vdel>[0-9]+)))$/
const CREATION_LOCATION = /^(?<txid>[a-f0-9]{64})?_(?:(?:o(?<vout>[0-9]+))|(?:d(?<vdel>[0-9]+)))(?:\?berry=(?<berry>[a-zA-Z0-9\-_.!~*'()%]*)(&hash=(?<hash>[a-f0-9]{64}))?(&version=(?<version>[1-9][0-9]*))?)?$/s
 ...
 * like URIs, and the supported prefixes are record://, commit://, and error://.
 *
 * record:// is also used before a commit is generated
 *
 ...
 * @returns {txid, vout, vdel, berry, hash, version, error, undeployed, commitid, recordid, nativeid} Parsed locations. Fields are optional.
 */
function _location (s) {
  if (typeof s !== 'string') throw new Error(`Location is not a string: ${_text(s)}`)

  const match = s.match(CREATION_LOCATION) ||
    s.match(COMMIT_LOCATION) ||
  ...
  if (match) {
    ...
    if (groups.protocol === 'record') { ret.recordid = 'record://' + groups.recordid }
    if (groups.protocol === 'error') { ret.error = groups.error }
    if (groups.protocol === 'native') { ret.nativeid = 'native://' + groups.nativeid }
    ...
    if (typeof groups.berry !== 'undefined') { ret.berry = decodeURIComponent(groups.berry) }
    if (typeof groups.hash !== 'undefined') { ret.hash = groups.hash }
    if (typeof groups.version !== 'undefined') { ret.version = parseInt(groups.version) }
...
function _compileLocation (parts) {
  if (typeof parts !== 'object' || !parts) throw new Error(`Bad parts: ${_text(parts)}`)

  // Errors
  if (parts.error) return `error://${parts.error}`

  // Native
  if (parts.nativeid) return parts.nativeid

  // Prefix
  const prefix = parts.commitid || parts.recordid || parts.txid
  _assert(prefix)

  // Suffix
  _assert('vout' in parts || 'vdel' in parts)
  const suffix = 'vout' in parts ? `_o${parts.vout}` : `_d${parts.vdel}`

  // Query params
  const params = []
  if ('berry' in parts) {
    params.push(`berry=${encodeURIComponent(parts.berry)}`)
    if ('hash' in parts) params.push(`hash=${parts.hash}`)
    if ('version' in parts) params.push(`version=${parts.version}`)
  }
