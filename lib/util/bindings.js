...
const { _text, _setOwnProperty, _defined } = require('./misc')
...
const COMMIT_LOCATION = /^(?<protocol>commit):\/\/(?<commit>[a-f0-9]{64})_(?:o(?<vout>[0-9]+)|(?:d(?<vdel>[0-9]+)))$/
 ...
 * @returns {txid, vout, vdel, berry, hash, version, error, undeployed, record, commit, native}
 */
function _location (s) {
  ...
  if (match) {
    ...
    if (groups.protocol === 'commit') { ret.commit = groups.commit }
 ...
 * 
 * @param {txid, vout, vdel, berry, hash, version, error, undeployed, record, commit, native} parts
...
function _compileLocation (parts) {
  ...
  if (_defined(parts.commit)) prefix = `commit://${parts.commit}`
