...
class REST {
  static async _get (url, timeout = REST.defaults.timeout, headers = {}) {
    const options = { method: 'GET', body: undefined, timeout, headers }
    return await REST._request(url, options)
  }

  static async _post (url, body, timeout = REST.defaults.timeout, headers = {}) {
    const options = { method: 'POST', body, timeout, headers }
    return await REST._request(url, options)
  }
  ...
  static async _request (url, options = {}) {
    options = Object.assign({}, REST.defaults, options)

    for (let i = 0; i <= options.retries; i++) {
      try {
        if (Log._infoOn) Log._info(TAG, options.method, url)

        let result = null
        try {
          result = await request(url, options.method, options.body, options.timeout, options.headers)
        } catch (e) {
        ...
        if (i === options.retries) throw e
        if (Log._warnOn) Log._warn(e.toString())
        if (Log._infoOn) Log._info(TAG, 'POST', url, `(Retry ${i + 1}/${options.retries})`)
      }
    }
  }
}
...
REST.defaults = {}
REST.defaults.method = 'GET'
REST.defaults.body = undefined
REST.defaults.headers = {}
REST.defaults.timeout = 30000
REST.defaults.retries = 2
