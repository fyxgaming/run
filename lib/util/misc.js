...
// A function that runs an async task in the background. It returns a promise that that resolves
/// when the task completes, regardless of success or failure. It avoids unhandled node errors.
function _backgroundTask (asyncTask, onSuccess, onFailure) {
  let resolveTask, rejectTask
  const task = new Promise((resolve, reject) => { resolveTask = resolve; rejectTask = reject })
  async function perform () {
    try {
      const ret = await asyncTask()
      onSuccess(ret, task)
      resolveTask(ret)
    } catch (e) {
      onFailure(e, task)
