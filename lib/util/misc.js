...
function _setOwnProperty (x, name, value) {
  ...
  if (!desc) desc = { configurable: true, enumerable: true, writable: true }
  desc.value = value
...
class _Timeout {
  constructor (kernel = _kernel(), method) {
    this._timeout = kernel._timeout
    this._method = method
    this._start = new Date()
  }

  _check () {
    if (new Date() - this._start > this._timeout) throw new TimeoutError(`${this._method} timeout`)
  }
}
