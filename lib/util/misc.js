...
const _RESERVED_PROPS = [
  // Not sure how these will be used yet, so setting aside for later
  'latest',
  'recent',
  'mustBeLatest',
  'mustBeRecent',
...
const _RESERVED_CODE_METHODS = [
  ...
  'load' // load is used on jigs and berries and not currently supported on static code
]
...
function _sameJig (a, b) {
  ...
  return _sudo(() => {
    ...
    if (a instanceof Berry || b instanceof Berry) {
      if (!(a instanceof Berry)) return false
      if (!(b instanceof Berry)) return false
      if (a.location.startsWith('error://')) return false
      if (b.location.startsWith('error://')) return false
