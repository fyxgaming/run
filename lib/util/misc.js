  ...
  _deepTraverseObjects([value], x => {
    ...
    JigControl._enableSpecialProps(() => {
      if (typeof x.$owner !== 'undefined' && x.$owner !== resource) {
        ...
        throw new Error(`${_display(x)} belongs to a different resource\n\n${suggestion}`)
      }
    })
 ...
 *
 * This is a relable way to traverse jig state, but it will not traverse every possible resource.
...
function _cloneForSandbox (x) {
  ...
  const opts = {
    _outputIntrinsics: Sandbox._instance._sandboxIntrinsics,
