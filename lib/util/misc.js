...
const { BadArgumentError, BadStateError, InternalError } = require('./errors')
...
function _checkArgument (condition, reason) {
  if (!condition) throw new BadArgumentError(reason)
}
...
function _checkState (condition, reason) {
  if (!condition) throw new BadStateError(reason)
}
