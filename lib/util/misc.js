...
function _deanonymizeSourceCode (src, name) {
  ...
  if (require('../util/sandbox')._cover.includes(name)) return src
...
// in ES2015 using Object.keys() [1], so JSON.stringify is too. This is bad. One solution is to use
// a deterministic key order ine stringify, like json-stable-stringify, but Run prefers to keep
// key order intact as much as possible. Therefore, internally sets on jigs use _setOwnProperty that keeps
// key order intact and _safeJSONStringify just checks for errors.
//
// In 2020, key order is deterministic to spec on Node, Chrome, Firefox, and Edge. In Safari, it is
// mostly correct, but using proxies it still returns wrong values. Run uses proxies. In theory,
// we could probably skip this check on browsers we trust, but we'll keep it on.
//
...
function _safeJSONStringify (x) {
  ...
  const x2 = JSON.parse(y)
  const { _deepEqual } = require('./deep')
  if (!_deepEqual(x, x2)) throw new Error('Error during JSON stringification')
  return y
}
...
function _safeObjectKeys (x) {
  const descs = Object.getOwnPropertyDescriptors(x)
  return Object.getOwnPropertyNames(x)
    .filter(name => descs[name].enumerable)
    .sort(_safeCompareKeys)
}
