...
const _RESERVED_PROPS = [
  ...
  'toString', 'upgrade', 'sync', 'auth', 'destroy', 'recent', 'latest', 'mustBeRecent',
  'mustBeLatest', 'pluck',
  // Future properties
...
function _deterministicCompareKeys (a, b) {
  ...
  const aIsInteger = parseInt(a).toString() === a
  const bIsInteger = parseInt(b).toString() === b
  if (aIsInteger && !bIsInteger) return -1
  if (bIsInteger && !aIsInteger) return 1
  if (aIsInteger && bIsInteger) return parseInt(a) - parseInt(b)
