...
const {
  ...
  _isArbitraryObject, _sameJig, _setOwnProperty, _deterministicObjectKeys, _assert
} = require('./misc')
...
function _deepReplace (x, replacer, visited = new Map()) {
  ...
  let recurse = true
  const x2 = replacer(x, y => { recurse = y }) || x
  visited.set(x, x2)
...
function _deepClone (x, intrinsics, replacer, visited = new Map()) {
  ...
  if (arbitraryObject && !_isArbitraryObject(y)) throw new StateError(`Cannot clone: ${_text(x)}`)
