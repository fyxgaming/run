...
const {
  _isBasicArray, _isBasicObject, _text, _isBasicSet, _isBasicMap, _isBasicUint8Array,
  _isArbitraryObject, _checkState, _sameJig, _setOwnProperty, _safeObjectKeys
} = require('./misc')
 ...
 *
 * This function preserves order because setting an existing value does not change order.
 *
 * @param {*} x Object to traverse
...
function _deepReplace (x, replacer, visited = new Map()) {
  ...
  Object.keys(x2).forEach(key => {
    _setOwnProperty(x2, key, _deepReplace(x2[key], replacer, visited))
  })
...
function _deepEqual (a, b) {
  ...
  for (let i = 0; i < aKeys.length; i++) {
    const aKey = aKeys[i]
    const bKey = bKeys[i]
    if (!_deepEqual(aKey, bKey)) return false
