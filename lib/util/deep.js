...
const {
  ...
  _arbitraryObject, _sameJig, _setOwnProperty, _deterministicObjectKeys, _assert
} = require('./misc')
 ...
 */
function _deepVisit (x, callback, parent = undefined, visited = new Set()) {
  return _sudo(() => {
    ...
    if (callback(x, parent) === false) return

    // Traverse set entries
    if (x instanceof HI.Set || x instanceof SI.Set) {
      for (const y of x) {
        _deepVisit(y, callback, x, visited)
      }
    }
...
function _deepClone (x, intrinsics, replacer, visited = new Map()) {
  ...
  if (arbitraryObject && !_arbitraryObject(y)) throw new StateError(`Cannot clone: ${_text(x)}`)
