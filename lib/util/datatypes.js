...
class ResourceMap {
  ...
  get [Symbol.species] () { return ResourceMap }
...
ResourceMap.deps = { internal, uniqueKey }
...
/**
 * The Set version of ResourceMap
 *
 * This essentially wraps ResourceMap and makes values the same as keys.
 */
class ResourceSet {
  constructor (iterable) {
    internal(this).map = new ResourceMap()
    if (iterable) { for (const x of iterable) this.add(x) }
  }

  get size () { return internal(this).map.size }
  get [Symbol.species] () { return ResourceSet }
...
ResourceSet.deps = { internal, ResourceMap }

// ------------------------------------------------------------------------------------------------
// uniqueKey
// ------------------------------------------------------------------------------------------------

const resourceOrigin = x => JigControl._disableSafeguards(() => x.origin || x.location)
const resourceLocation = x => JigControl._disableSafeguards(() => x.location)
