...
class TokenMap {
  ...
  get (x) {
    return internal(this).map.get(uniqueKey(internal(this), x))
  }

  set (x, y) {
    internal(this).map.set(uniqueKey(internal(this), x), y); return this
  }

  has (x) {
    return internal(this).map.has(uniqueKey(internal(this), x))
  }

  get size () {
    return internal(this).map.size
  }
  ...
  entries () {
    return internal(this).map.entries()
  }

  keys () {
    return internal(this).map.keys()
  }

  forEach (callback, thisArg) {
    return internal(this).map.forEach(callback, thisArg)
  }

  values () {
    return internal(this).map.values()
  }

  [Symbol.iterator] () {
    return internal(this).map[Symbol.iterator]()
  }
}
...
const tokenOrigin = x => JigControl.disableSafeguards(() => x.origin || x.location)
const tokenLocation = x => JigControl.disableSafeguards(() => x.location)
...
function uniqueKey (internals, x) {
  ...
  if (!_tokenType(x)) return x

  const xOrigin = tokenOrigin(x)
  const xLocation = tokenLocation(x)

  // Case1: Undeployed token

  if (!xOrigin || !Location.parse(xOrigin).txid) {
    internals.undeployed.add(x)
    return x
  }

  // Case 2: Deployed token
 ...
 */
function internal (x) {
  const target = JigControl.enableSpecialProps(() => {
    return x.$target || x
  })
