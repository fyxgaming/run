...
function _serializeObject (x, options) {
  ...
  if (isBasicObject) {
    const y = Object.create(options._outputIntrinsics.Object)
    options._cache.set(x, y)
    let $ = false
    Object.keys(x).forEach(key => {
      $ = $ || key.startsWith('$')
      y[key] = _serializeInternal(x[key], options)
    })
    if (!$) return y
    const z = Object.create(options._outputIntrinsics.Object)
    z.$obj = y
    return z
  }
  ...
  if (isBasicArray) {
    const y = options._outputIntrinsics.Array.from([])
    options._cache.set(x, y)
    Object.keys(x).forEach(key => { y[key] = _serializeInternal(x[key], options) })
    return y
  }
  ...
  if (isSet) {
    ...
    for (const v of x) { y.$set.push(_serializeInternal(v, options)) }
    // Serialize properties on the set, if they exist
    if (Object.keys(x).length) {
      y.props = Object.create(options._outputIntrinsics.Object)
      Object.keys(x).forEach(key => { y.props[key] = _serializeInternal(x[key], options) })
    }
    return y
  }
  ...
  if (isMap) {
    ...
    for (const [k, v] of x) {
      y.$map.push([
        _serializeInternal(k, options),
        _serializeInternal(v, options)
      ])
    }
...
function _dedup (x, options) {
  ...
  y.dups = options._outputIntrinsics.Array.from([])
  for (const dup of options._dups) {
    const dedup = Object.create(options._outputIntrinsics.Object)
...
function _deserialize (x) {
  return x
}

module.exports = { _serialize, _deserialize }
