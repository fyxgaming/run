...
const { _sandboxSourceCode, _anonymizeSourceCode, _defineGetter } = require('../util/misc')
...
class Sandbox {
  ...
  _sandboxType (T, env, native = false, anonymize = false) {
    // If in cover mode, we can't evaluate. Save the type too to avoid any future evaluates.
    if ((process.env.COVER && native) || this._cover.includes(T.name)) {
      const globalThis = typeof global !== 'undefined' ? global : window
      return [T, globalThis]
    }

    // Create the source code
    const src = _sandboxSourceCode(T.toString(), T)
    const src2 = anonymize ? _anonymizeSourceCode(src) : src
  ...
  _evaluate (code, env = {}) {
    ...
    compartment.global.console = {
      log: (...args) => {
        const { _sudo } = require('../util/admin')
        _sudo(() => console.log(...args))
      }
    }
