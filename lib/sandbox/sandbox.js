...
class Sandbox {
  ...
  _sandboxType (T, env, native = false, anonymize = false) {
    ...
    if (cover) {
      ...
      const covsRegex = /(cov_[a-zA-Z0-9]+\(\).[a-zA-Z0-9\[\]]+\+\+,?)/g // eslint-disable-line
      originalSource = originalSource.replace(covsRegex, '')
    }

    // Create the source code
    const src = _sandboxSourceCode(originalSource, T)
