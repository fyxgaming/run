...
class Owner extends Sign {
  ...
  async queryLatest () {
    ...
    if (typeof queryString === 'object') {
      const script = queryString.getBuffer()
      const scriptHash = bsv.crypto.Hash.sha256(Buffer.from(script))
      queryString = scriptHash.toString('hex')
    }
  ...
  sameOwner (x, y) {
    ...
    const xbuf = Buffer.from(x.getBuffer())
    const ybuf = Buffer.from(y.getBuffer())
