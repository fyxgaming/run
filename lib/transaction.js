...
class Transaction {
  ...
  async load (location, options = {}) {
    ...
    // Attempt to load this first using Run
    let runError = null
    try {
      return await this.loadRunToken(location)
    } catch (e) { runError = e }

    // Attempt to load this using berries
    if (this.protocol.berryProtocols.size) {
      try {
        return await this.protocol.pluckBerry(location, this.blockchain, this.code)
      } catch (berryError) {
        throw new Error(`Failed to load ${location}\n\n${runError}\n\n${berryError}`)
      }
    } else { throw new Error(`Failed to load ${location}\n\n${runError}`) }
  }
