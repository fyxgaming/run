...
const { UniqueSet } = require('./unique')
...
class ProtoTransaction {
  ...
  async import (tx, run, preexistingJig, immutable, vout, cache) {
    ...
    */
    ...
    this.outputs.forEach((o, n) => {
      ...
      const hex1 = Buffer.from(util.getOwner(o.owner).toBuffer()).toString('hex')
  ...
  buildBsvTransaction (run) {
    ...
    this.code.forEach(def => {
      const script = new bsv.Script(util.getOwner(def.owner).getBuffer())
    ...
    this.outputs.forEach(jig => {
      ...
      const script = new bsv.Script(util.getOwner(restored.owner).getBuffer())
