...
class ProtoTransaction {
  ...
  async import (tx, run, preexistingJig, immutable, vout, cache) {
    ...
    for (const [refId, refLocation] of refs) {
      if (preexistingJig && refLocation === preexistingJig.location) {
        ...
        try {
          refs.set(refId, await run.load(refLocation))
        } catch (e) {
          run.logger.error(e)
          throw new Error(`bad ref ${refId}\n\n${e.toString()}`)
        }
      }
    }
    ...
    for (const action of data.actions) {
      ...
      if (action.method === 'init') {
        ...
        const T = await run.load(action.target[0] === '_' ? tx.hash + action.target : action.target)
  ...
  rollback (lastPosted, run, error, unhandled) {
    ...
    this.outputs.forEach(jig => {
      ...
      run.owner._update(this.proxies.get(jig))
    })
  ...
  buildBsvTransaction (run) {
    ...
    const code = this.code.map(def => {
      ...
      return { text: util.codeText(def.type), deps, props, owner: def.owner }
    })
...
class Transaction {
  ...
  async load (location, options = {}) {
    ...
    if (isNaN(vout)) throw new Error(`bad location: ${location}`)

    // check the state cache so we only have to load each jig once
    const cachedState = await this.state.get(location)
    if (cachedState) {
      // bump this state because it was recently accessed
      await this.state.set(location, cachedState)
