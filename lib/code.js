...
module.exports = class Code {
  ...
  deploy (type) {
    ...
    try {
      ...
      if (parentClass !== Object.getPrototypeOf(Object)) {
        ...
        if (realdeps[parentClass.name] && realdeps[parentClass.name] !== parentClass) {
      ...
      const [sandbox, sandboxGlobal] = this.sandbox(type, util.codeText(type), type.name, env, run.sandbox)
  ...
  async installFromTx (def, location, tx, run, bsvNetwork, partiallyInstalledCode = new Map()) {
    ...
    partiallyInstalledCode.set(location, sandbox)
  ...
  installJig () {
    ...
    this.Jig = this.sandbox(Jig, Jig.toString(), 'Jig', env, !TEST_COVER)[0]
