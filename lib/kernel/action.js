...
const { _assert, _text, _parent, _hasJig, _hasOwnProperty, _checkState } = require('../util/misc')
...
function _upgrade (jig, snapshot) {
  ...
  _checkState(!destroyed, 'Cannot upgrade destroyed jig')

  // If non-upgradable, then we can't upgrade
  const nonupgradableError = `${_text(jig)} is non-upgradable`
  _checkState(_sudo(() => !_hasOwnProperty(jig, 'upgradable') || jig.upgradable), nonupgradableError)
...
function _auth (jig) {
  ...
  const destroyed = 'vdel' in _location(_sudo(() => jig.location))
  _checkState(!destroyed, 'Cannot auth destroyed jigs')
