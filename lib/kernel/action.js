...
const { _assert, _kernel, _text, _parent, _hasOwnProperty, _checkState } = require('../util/misc')
...
function _prepareArgs (thisArg, args) {
  ...
  if (!kernel._manual) {
    ...
    _sudo(() => {
      const worldview = new Map() // Origin -> Universal Jig

      worldview.set(thisArg.origin, thisArg)

      if (thisArg instanceof Jig) {
        worldview.set(thisArg.constructor.origin, thisArg.constructor)
      }
    ...
    _sudo(() => {
      const worldview = new Map() // Location -> Universal Jig

      worldview.set(thisArg.location, thisArg)

      if (thisArg instanceof Jig) {
        worldview.set(thisArg.constructor.location, thisArg.constructor)
      }
