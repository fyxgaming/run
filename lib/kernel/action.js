...
class _SignAction extends _Action {
  toString () {
    return `Sign ${_text(this._creation)}`
  }
...
function _deploy (creations) {
  ...
  CURRENT_RECORD._capture(() => {
    ...
    // Add parent classes to the SIGN set
    creations.forEach(C => signParents(C, 'deploy'))
...
function _sign (creation) {
  ...
  if (Log._infoOn) Log._info(TAG, 'Sign', _text(creation))

  // If already destroyed, then we can't sign
  const destroyed = 'vdel' in _location(_sudo(() => creation.location))
  if (destroyed) throw new StateError('Cannot sign destroyed jigs')

  CURRENT_RECORD._capture(() => {
    CURRENT_RECORD._sign(creation)

    // Only add the action if there is not already an action in progress
    if (CURRENT_RECORD._stack.length) return

    const action = new _SignAction(creation)
    CURRENT_RECORD._action(action)
  })
}
