...
const { _assert, _text, _parent, _hasOwnProperty, _checkState, _hasJig } = require('../util/misc')
...
function _auth (jig) {
  ...
  // If already destroyed, then we can't auth
  const destroyedBefore = 'vdel' in _location(_sudo(() => jig.location))
  const destroyedInRecord = _hasJig(CURRENT_RECORD._deletes, jig)
  const destroyed = destroyedBefore || destroyedInRecord
