...
class _NewAction extends _Action {
  ...
  toString () { return `New ${_text(this._jig)}` }
}
...
function _new (classJig, jig, args) {
  ...
  Log._info(TAG, 'New', _text(classJig))
  ...
  CURRENT_RECORD._capture(() => {
    ...
    // Deploy all of the args before the NEW action starts recording.
    _deepVisit(args, x => {
    ...
    // Create the action
    const action = new _NewAction(classJig, jig, args)

    // Sandwich the action in a push/pop stack update
    CURRENT_RECORD._push(action)
    const ret = jig.init(...args)
