...
module.exports = class Syncer {
  ...
  _publish (record) {
    for (const [key, value] of record.locations) {
  ...
  async _publishNext () {
    ...
    next._outputs.forEach(jig => inventory._notify(next.proxies.get(jig)))
  ...
  async _updateJigWithNextTransaction (jig, tx) {
    ...
    const jigProxies = Array.from(record.proxies.values())
