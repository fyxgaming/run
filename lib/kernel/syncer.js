...
module.exports = class Syncer {
  ...
  _publish (record) {
    for (const [key, value] of record._locations) {
  ...
  async _publishNext () {
    // next is the record to publish
    const next = this._queued[0]
    if (next._empty()) return this._finish()
    ...
    try {
      ...
      if (!next._paid) await next.pay(this._kernel)
      ...
      // TODO: Do after rollback
      // const rollbackErrors = []
      // const record = this._queued[i]
      // record.rollback(this._lastPosted, this._kernel, e, unhandled)
      // }
      this._queued.forEach(record => record.rollback(this._lastPosted, this._kernel, e, unhandled))
    ...
    next._outputs.forEach((target, index) => {
      const vout = 1 + next._code.length + index
      ...
      next._after.get(target).restore().origin = target.origin
      next._after.get(target).restore().location = `${tx.hash}_o${vout}`
    })

    next._code.forEach((def, index) => def.success(`${tx.hash}_o${index + 1}`))
    ...
    for (const jig of next._outputs) {
      const after = next._after.get(jig)
    ...
    next._code.forEach(def => inventory._notify(def.S))
    next._outputs.forEach(jig => inventory._notify(next._proxies.get(jig)))
