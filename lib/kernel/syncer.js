...
const { error } = util
const { _toTokenJson, _refifyTokens } = require('../util/json')
...
module.exports = class Syncer {
  ...
  async publishNext () {
    ...
    const net = util.networkSuffix(this._kernel._blockchain.network)
    ...
    try {
      ...
      tx = await this._kernel._owner.sign(tx)
...
owner: ${spentJigs[i].owner}`)
      ...
      error(this._kernel._logger, `Unhandled ${e.toString()}`)
    ...
    for (const jig of next.outputs) {
      ...
      const serialized = JigControl.disableProxy(() => {
        ...
        return _toTokenJson(restored, {
          _sandboxIntrinsics: this._kernel._code._sandboxIntrinsics,
          _replacers: [_refifyTokens(tokenSaver)]
        })
      })
    ...
    // notify the owner
    const update = x => { if (this._kernel._owner instanceof Owner) this._kernel._owner.update(x) }
      ...
      if (eString.indexOf('Missing inputs') !== -1 || eString.indexOf('txn-mempool-conflict') !== -1) {
        ...
        for (const input of tx.inputs) {
          ...
          if (prevOutput.spentTxId) {
            ...
            const type = util._outputType(prevTx, input.outputIndex)

            let typeString = 'Payment'
            switch (type) {
              case 'code':
                try {
                  typeString = (await this._kernel.load(prevLocation)).name
                } catch (e) { typeString = 'Code' }
                break
              case 'jig':
                try {
                  typeString = `${await this._kernel.load(prevLocation)}`
