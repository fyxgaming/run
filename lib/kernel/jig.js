...
class Jig {
  constructor (...args) {
    ...
    if (!(this.constructor instanceof Code)) {
      return CURRENT_RECORD._capture(() => {
        ...
        if (!_kernel()._manual) Code._editor(C)._deploy()
        return new C(...args)
      })
    } else {
      if (!_kernel._manual) Code._editor(this.constructor)._deploy()
    }
  ...
  auth () {
    ...
    if (_hasJig(Record._CURRENT_RECORD._creates, this)) throw new Error('auth unavailable')
