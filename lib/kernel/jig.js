...
class Jig {
  constructor (...args) {
    ...
    handler.get = function (target, prop, receiver) {
      ...
      if (typeof target[prop] === 'object') {
        ...
        if (prop === 'owner' && targetIsAJig) {
          if (JigControl.stack.length && !fromWithin()) {
    ...
    this.owner = JigControl.stack.length ? JigControl.stack[JigControl.stack.length - 1].owner : Context._nextOwner()
    this.satoshis = 0
...
const JigControl = { // control state shared across all jigs, similar to a PCB
  ...
  _special: false, // enable reading special props on the token. Currently, $target and $owner
