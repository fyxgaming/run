...
class Jig {
  constructor (...args) {
    ...
    if (creator) {
      // Try reading the owner first to be sure its bound
      creator.owner // eslint-disable-line
  ...
  sync (options = {}) {
    ...
    if (!(this instanceof NativeJig)) throw new Error('Can only sync jigs')
  ...
  destroy () {
    ...
    Log._debug(TAG, 'Destroy', text(this))

    if (!(this instanceof NativeJig)) throw new Error('Can only destroy jigs')
