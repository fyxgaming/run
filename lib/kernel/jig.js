...
class Jig {
  constructor (...args) {
    ...
    const creator = stack.length && stack[stack.length - 1]._creation
    if (creator) {
  ...
  sign () {
    ...
    const Action = JigDeps._Action
    const Record = JigDeps._Record
    const _hasCreation = JigDeps._misc._hasCreation

    if (Log._debugOn) Log._debug(TAG, 'Sign', text(this))

    // sign cannot be applied to a non-jig
    if (!(this instanceof NativeJig)) throw new Error('sign unavailable on native jigs')

    // We cannot sign jigs just created because there is no input
