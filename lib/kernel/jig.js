...
class Jig {
  constructor (...args) {
    ...
    if (!run.internal.code.isSandboxed(this.constructor)) {
      run.transaction.begin()
      try {
        const T = run.internal.code.deploy(this.constructor)
        return new T(...args)
    ...
    handler.apply = function (target, thisArg, args) {
      ...
      try {
        ...
        const argsCheckpoint = parentIsAJig ? new Context.Checkpoint(args, run.internal.code) : null

        if (!JigControl.before.has(this.checkpointTarget)) {
          const checkpoint = new Context.Checkpoint(Object.assign({}, this.checkpointTarget), run.internal.code)
        ...
        if (!JigControl.stack.length) {
          ...
          const changed = JigControl.disableSafeguards(() => {
            ...
            objectsToSave.forEach(target => {
              after.set(target, new Context.Checkpoint(Object.assign({}, target), run.internal.code))
            })
  ...
  static [Symbol.hasInstance] (target) {
    ...
    let T = run.internal.code.getSandboxed(this)
    if (!T) {
      const net = Context._networkSuffix(run.blockchain.network)
      T = run.internal.code.getSandboxed(this[`origin${net}`])
      if (!T) return false
    }
