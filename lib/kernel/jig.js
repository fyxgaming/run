...
const JigControl = { // control state shared across all jigs, similar to a PCB
  ...
  enforce: true, // enable safeguards for the user. (This is not the same as a target. ie. $$$belongs_to$$$)
...
class Jig {
  constructor (...args) {
    ...
    handler.get = function (target, prop, receiver) {
      ...
      if (prop === '$$$belongs_to$$$') return proxy
    ...
    handler.apply = function (target, thisArg, args) {
      ...
      try {
        ...
        // But the args we use in the method have to be the original, for the proxies
        // and $$$belongs_to$$$ to take effect. Internal methods do not need a checkpoint.
        //
        // TODO: Do we even need to do this for any internal jig calls? Can jigs modify
        // the outside through changes to args? Sure. Add a test and fix this.
        ...
        const ret = target.call(parentIsAJig ? proxy : this.parent, ...args)
        ...
        if (!JigControl.stack.length) {
          ...
          if (JigControl.creates.size || changed.length) {
            ...
            const argsCopy = argsCheckpoint ? argsCheckpoint.restore() : args
            run.transaction.storeAction(original, this.name, argsCopy, inputs, outputs,
  ...
  async sync (options) {
    const syncer = Context.activeRunInstance()._kernel._syncer
