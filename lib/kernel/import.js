...
async function _import (tx, payload, kernel, published, jigToSync, importLimit) {
  ...
  try {
    ...
    // Replace the current record with ours while we execute commands
    Record._CURRENT_RECORD = record

    // Execute each command
    for (const entry of payload.cmds) {
      const { cmd, data } = entry

      _checkState(Object.keys(entry).length === 2, 'Invalid cmds')
      _checkState(typeof cmd === 'string', `Invalid cmd: ${cmd}`)
      _checkState(typeof data === 'object' && data, `Invalid data: ${data}`)

      const masterList = _addJigs(incoming, record._creates)

      _execute(cmd, data, masterList)
    }
