...
const { _checkState, _addJigs, _Timeout } = require('../util/misc')
 ...
 */
async function _import (tx, payload, kernel, published, jigToSync, timeout) {
  const _execute = require('./execute')

  if (Log._infoOn) Log._info(TAG, 'Import', tx.hash)
  ...
  if ((deploys || upgrades) && !kernel._trusts.has(tx.hash) && !kernel._trusts.has('*')) {
  ...
  // We will trust every input and reference if we trust this transaction
  const trusted = deploys || upgrades
  const trust = txid => { if (trusted && !kernel._trusts.has(txid)) kernel._trusts.add(txid) }
  ...
  _checkState(!jigToSync || inputs.some(x => x.location === jigToSyncLocation), 'Jig not spent in the transaction')
  ...
  for (let i = 0; i < inputs.length; i++) {
    if (inputs[i].location === jigToSyncLocation) {
      inputs[i] = jigToSync
      break
    }
  }
...
function unifyIncoming (incoming, jigToSync) {
  ...
  _sudo(() => {
    _deepVisit(incoming, x => {
      if (x instanceof Universal) {
        ...
        const y = worldview[key] || x
        if (x.nonce >= y.nonce) worldview[key] = y
      }
    })
  })
  ...
  const refmap = {}
  Object.keys(worldview).forEach(origin => {
    refmap[origin] = [worldview[origin].location, worldview[origin].nonce]
  })
  ...
  _sudo(() => {
    ...
    // Unify the jig to sync with the worldview, potentially going back
