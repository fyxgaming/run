 ...
 */
async function _import (tx, txid, payload, kernel, published, jigToSync, timeout) {
  ...
  if ((deploys || upgrades) && !kernel._trusts.has(txid) && !kernel._trusts.has('*')) {
  ...
  _checkState(!jigToSync || inputs.some(x => x.location === jigToSync.location), 'Jig not spent in the transaction')
  ...
  try {
    ...
    // Verify the commit
    await verify(commit, tx, txid, payload, published, timeout)
