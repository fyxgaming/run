...
const { _location, _lockify } = require('../util/resource')
...
function checkPresets (T, network) {
  ...
  if (anyJigPresets) {
    if (_location(npresets.location).onchain) {
      throw new InstallFailedError(T, `The ${network} location preset is not an onchain location`)
    }
    if (_location(npresets.origin).onchain) {
      throw new InstallFailedError(T, `The ${network} origin preset is not an onchain location`)
    }

    try {
      _lockify(npresets.owner)
    } catch (e) {
      throw new InstallFailedError(T, `The ${network} owner preset is not a valid owner\n\n${e.message}`)
    }
  }
}
