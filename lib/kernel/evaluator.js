...
const { getIntrinsics, intrinsicNames, Intrinsics } = require('./intrinsics')
const { FriendlySet, FriendlyMap, createSandboxedFriendlyMap, createSandboxedFriendlySet } = require('../util/friendly')
...
class Evaluator {
  ...
  evaluate (code, env = {}) {
    ...
    this._kernel._logger.info(`Evaluating code starting with: "${code.slice(0, 20)}"`)
...
class SandboxEvaluator {
  constructor () {
    ...
    compartment.global.intrinsicNames = intrinsicNames
    this.intrinsics = compartment.evaluate(`(${getIntrinsics.toString()})()`)
  }
...
class GlobalEvaluator {
  ...
  setGlobalDescriptor (key, descriptor) {
    ...
    if (!(key in this.savedGlobalDescriptors)) {
      ...
      this._kernel._logger.warn(`${warning}\n\n${reason}`)
    }
