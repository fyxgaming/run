...
async function _replay (tx, txid, payload, kernel, published, jigToSync, timeout, preverify) {
  ...
  try {
    ...
    commit._base = new bsv.Transaction(payload.base)
...
async function verify (commit, tx, txid, txpayload, published, timeout, preverify) {
  ...
  const feePerKb = 0
  const partialtx = _createPartialTx(commit, payload, outputScripts, feePerKb)
  ...
  for (let i = 0; i < payload.out.length; i++) {
    const txout1 = tx.outputs[i + payload.vrun + 1]
    const txout2 = partialtx.outputs[i + payload.vrun + 1]
    const script1 = txout1.script.toString('hex')
    const script2 = txout2.script.toString('hex')
    if (script1 !== script2) throw new StateError(`Script mismatch on output ${i + payload.vrun + 1}`)
    if (txout1.satoshis < txout2.satoshis) {
      const hint = `Hint: Transaction has ${txout1.satoshis} but expected ${txout2.satoshis}`
      throw new StateError(`Satoshis mismatch on output ${i + payload.vrun + 1}\n\n${hint}`)
    }
  }
...
function _assignOwnersFromPayload (commit, txpayload, masterList) {
  ...
  if (commit._record._creates._size !== txpayload.cre.length) throw new StateError('Invalid locks')
