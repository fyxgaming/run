...
const { _text, _deterministicJSONStringify, _Timeout } = require('../util/misc')
...
async function _replay (tx, txid, metadata, kernel, published, jigToSync, timeout, preverify) {
  ...
  if (anythingToTrust) {
    await kernel._checkTrusted(txid, 'replay')
  }
  ...
  try {
    ...
    if (!commit) throw new StateError('Invalid metadata: no commit generated')
    ...
    // Verify the commit
    await verify(commit, tx, txid, metadata, published, timeout, preverify)
...
async function verify (commit, tx, txid, txmetadata, published, timeout, preverify) {
  ...
  for (let i = 0; i < metadata.out.length; i++) {
    ...
    if (script1 !== script2) throw new StateError(`Script mismatch on output ${i + metadata.vrun + 1}`)
    if (txout1.satoshis < txout2.satoshis) {
      const hint = `Hint: Transaction has ${txout1.satoshis} but expected ${txout2.satoshis}`
      throw new StateError(`Satoshis mismatch on output ${i + metadata.vrun + 1}\n\n${hint}`)
    }
  }
...
function _assignOwnersFromMetadata (commit, txmetadata, masterList) {
  ...
  // Inflate the owners
  const owners = txmetadata.cre.map(lock => ownersCodec._decode(lock))

  // Check that the owners list length matches the number of creates
  if (commit._record._creates._size !== txmetadata.cre.length) throw new StateError('Invalid locks')
