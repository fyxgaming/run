...
async function _replay (tx, txid, payload, kernel, published, jigToSync, timeout, preverify) {
  ...
  if (!preverify && (deploys || upgrades) && !kernel._trustlist.has(txid) && !kernel._trustlist.has('*')) {
    const hint = 'Hint: Trust this txid using run.trust if you know it is safe'
    throw new Error(`Cannot load untrusted code via replay: ${txid}\n\n${hint}`)
  }
  ...
  const trust = txid => {
    if (preverify) return
    if (!kernel._trustlist.has(txid)) kernel._trustlist.add(txid)
  }
...
async function verify (commit, tx, txid, txpayload, published, timeout, preverify) {
  ...
  const dustAmount = 0
  const partialtx = _createPartialTx(commit, payload, outputScripts, dustAmount)
