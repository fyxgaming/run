...
const { _text, _addJigs, _deterministicJSONStringify, _Timeout } = require('../util/misc')
...
async function _replay (tx, txid, payload, kernel, published, jigToSync, timeout, preverify) {
  ...
  record._version = payload.version
...
function _assignOwnersFromPayload (commit, txpayload, masterList) {
  ...
  if (commit._creates.length !== txpayload.cre.length) throw new StateError('Invalid locks')
