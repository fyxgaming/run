...
class Membrane {
  ...
  static _configure (proxy) {
    ...
    if (typeof m._target === 'function' && topLevel) {
      const isStatic = !_extendsFrom(m._target, Jig)

      const isNative =
        _hasOwnProperty(m._target, 'location') &&
        m._target.location &&
        m._target.location.startsWith('native://')
    ...
    // Jig instances
    if (typeof m._target === 'object' && topLevel && _extendsFrom(m._target.constructor, Jig)) {
      m._bindings = true
      m._private = true
      m._record = true
      return
    ...
    // Berry instances
    if (typeof m._target === 'object' && topLevel && _extendsFrom(m._target.constructor, Berry)) {
      m._immutable = true
      m._record = true
      return
    }
    ...
    if (typeof m._target === 'object' && !topLevel) {
