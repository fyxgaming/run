...
class Membrane {
  ...
  _apply (target, thisArg, args) {
    ...
    return this._captureErrors(() => {
      ...
      // Jig.init is not recordable, even through the jig records calls, because native code
      // is not recordable. This is by design. Instantiation is a special top-level action.
      let passThrough = !membrane || !membrane._rules._recordableTarget || !this._rules._recordCalls
      ...
      if (passThrough && typeof thisArg === 'function' && !(thisArg instanceof Jig)) {
        ...
        passThrough = !membrane || !membrane._rules._recordableTarget || !this._rules._recordCalls
      }
