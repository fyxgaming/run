...
const { _assert, _hasOwnProperty, _extendsFrom } = require('../util/misc')
...
class Membrane {
  ...
  constructor (target, parentJig = null) {
    ...
    // Whether the target is an inner object, or otherwise top-level jig
    this._inner = false
    // Whether this jig should record reads and updates
    this._record = false
    // Whether this jig cannot be changed
    this._immutable = false
    // Kind of membrane: jig, code, berry, inner object, inner method
    this._kind = null
    // Whether this is static code or object (not supported)
    this._static = false
    // Whether this is an inner object or method of another jig
    this._inner = false
  ...
  static _configure (jig, parentJig) {
    ...
    if (typeof m._target === 'function' && !parentJig) {
      m._kind = 'code'
      m._jig = m._proxy
      m._record = true
    ...
    // Jig instances
    if (typeof m._target === 'object' && !parentJig && _extendsFrom(m._target.constructor, Jig)) {
      m._kind = 'jig'
      m._jig = m._proxy
      m._record = true
      return
    ...
    // Berry instances
    if (typeof m._target === 'object' && !parentJig && _extendsFrom(m._target.constructor, Berry)) {
      m._kind = 'berry'
      m._jig = m._proxy
      m._record = true
      m._immutable = true
    ...
    // Inner objects
    if (typeof m._target === 'object' && parentJig) {
      m._kind = 'inner object'
      m._inner = true
      m._record = true
    }
    ...
    // Inner methods
    if (typeof m._target === 'function' && parentJig) {
      m._kind = 'inner method'
      m._jig = parentJig
      m._record = true
      m._inner = true
    }

    throw new Error('Unknown membrane target')
  }
