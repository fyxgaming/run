...
class Membrane {
  ...
  _apply (target, thisArg, args) {
    ...
    return this._trap(() => {
      ...
      // If this function isn't replayable, then we run it directly. This is used for
      ...
      if (!this._rules._replayable) return Reflect.apply(target, thisArg, args)
      ...
      const callArgs = this._wrapForImport(args)
  ...
  _wrapForImport (value) {
    // If import rules are disabled, then just return the value
    if (!this._rules._import) return value
    ...
    if (!stack.length) {
      ...
      this._unifyWorldview(this._jig, clonedValue)

      // No membranes required since we cloned. Return.
      return clonedValue
    }

    // Determine if we are crossing a boundary from another jig to his one
    const crossing = stack[stack.length - 1]._jig === this._jig

    // If not crossing, then we're an internal method calling ourself. No import.
    if (!crossing) return value

    // If this is a primitive type, it needs no import
    if (!this._isObjectType(value)) return value
