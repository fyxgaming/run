...
function getLatestMethod (thisArg, target, proxy, jig) {
  ...
  // If the method was upgraded but its still part of the same class, we can call it
  const container = typeof thisArg === 'function' ? thisArg : thisArg.constructor
  if (thisArg[name] && _sudo(() => container.origin === jig.origin)) {
    return [Proxy2._getTarget(thisArg[name]), container]
  }
