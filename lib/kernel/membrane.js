...
const {
  ...
  _RESERVED_PROPS, _getOwnProperty, _deterministicCompareKeys
} = require('../util/misc')
  ...
  _apply (target, thisArg, args) {
    ...
    return this._captureErrors(() => {
      ...
      return RECORD()._capture(() => {
        ...
        if (!stack.length) {
          const { _prepare } = require('./args')
          args = _prepare(thisArg, args)
        }
        ...
        Action._call(thisArg, target.name, args, () => {
          ...
          // OR, if we are in one of our jig methods already, and this method is in our class chain
          const errorMessage = `Cannot call ${target.name} on ${_text(thisArg)}`
          const allowed = isAllowed(thisArg, target.name, this._proxy)
  ...
  _checkCanGetBinding (target, prop) {
    ...
    try {
      ...
      if (prop === 'owner' || prop === 'satoshis') {
        ...
        if (prop === 'owner') _owner(val instanceof Unbound ? val._value : val, true)
        if (prop === 'satoshis') _satoshis(val instanceof Unbound ? val._value : val, true)
      }
    } catch (e) {
...
function isAllowed (thisArg, name, methodProxy) {
  // No this, always allowed
  if (!thisArg) return true

  // Only jigs can have jig methods called on them
  if (!(thisArg instanceof Universal)) return false

  // If this is our method, then we can call it
  if (thisArg[name] === methodProxy) return true

  // If we are in our jig already, and this is in our class chain, it's OK
  const stack = RECORD()._stack
  if (stack.length < 2 || stack[stack.length - 2]._jig !== thisArg) return false
  let prototype = Object.getPrototypeOf(thisArg)
  while (prototype) {
    const prototypeMethod = _getOwnProperty(prototype, name)
    if (prototypeMethod === methodProxy) return true
