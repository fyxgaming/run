...
class Membrane {
  ...
  _checkCanGetUtxoBinding (prop) {
    ...
    try {
      ...
      if (prop === 'owner' || prop === 'satoshis') {
        ...
        if (prop === 'satoshis') _satoshis(val instanceof Unbound ? val._value : val)
      }
    } catch (e) {
  ...
  _checkCanChangeUtxoBinding (prop, value, method) {
    ...
    const target = Proxy2._getTarget(this._proxy)
    const destroyed = target.owner === null && target.satoshis === 0
    if (destroyed) throw new StateError(`Cannot ${method} ${prop}`)
  ...
  _finalizePending () {
    ...
    _deepReplace(PENDING, (x, recurse) => {
