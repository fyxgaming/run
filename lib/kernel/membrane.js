...
class Membrane {
  ...
  _apply (target, thisArg, args) {
    ...
    return this._captureErrors(() => {
      ...
      return RECORD()._capture(() => {
        ...
        const callArgs = crossing ? targetMembrane._exportCow(args) : args
        ...
        try {
          ...
          return this._exportOwned(ret, returning)
        } finally {
  ...
  _get (target, prop, receiver) {
    ...
    return this._captureErrors(() => {
      ...
      value = parentMembrane ? parentMembrane._exportOwned(value, returning) : value
  ...
  _getOwnPropertyDescriptor (target, prop) {
    ...
    return this._captureErrors(() => {
      ...
      // Wrap this object with a membrane that enforces parent rules
      const returning = false
      desc.value = this._exportOwned(desc.value, returning)
  ...
  _intrinsicOut (value) {
    ...
    return this._captureErrors(() => {
      ...
      // Wrap this object with a membrane that enforces parent rules
      const returning = false
      value = this._exportOwned(value, returning)
  ...
  _exportOwned (value, returning, unclaimedSet = new Set()) {
    ...
    if (!status._claimed) {
      ...
      const wrapInner = x => this._exportOwned(x, returning, unclaimedSet)
