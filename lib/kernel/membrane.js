...
class Membrane {
    ...
    return this._captureErrors(() => {
      ...
      return RECORD()._capture(() => {
        ...
        // Push a call action onto the stack and then call the method
          ...
          if (crossing) this._finalizePendingMembranes()
          ...
          if (crossing) {
            _deepVisit(Proxy2._getTarget(this._jig), x => {
  ...
  _finalizePendingMembranes () {
    _sudo(() => _deepReplace(PENDING_MEMBRANES, (x, recurse) => {
      ...
      // Primitives are left intact
      if (this._isBasicType(x)) return
      ...
      // Non-proxied objects may be left intact, but we have to traverse to see their inners.
      // These would be objects created and assigned in the current method, or unclaimed
      // and out of another jig's method.
      ...
      if (membrane._rules._parentJig === this._jig) return target

      // The parent jig is not us. It may be cow arg, or another property, but in
      // both cases the assignment to ourselves should make us a clone. And in
      return _deepClone(target, SI)
    }))
  }
