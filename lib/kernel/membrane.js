...
class MethodHandler {
  ...
  apply (target, thisArg, args) {
    ...
    console.log('apply', STACK, RECORD, this._name)

    // Check that the user isn't doing something they shouldn't
    if (thisArg !== this._jig) throw new Error('Jig methods must only be applied to themselves')

    // If this is a built-in Jig or Code function, then return it directly. No wrapping.
    const builtin = (this._jig instanceof Code &&
      Object.getOwnPropertyNames(Code.prototype).includes(this._name)) ||
      (this._jig instanceof Jig &&
        Object.getOwnPropertyNames(Jig.prototype).includes(this._name))
    if (builtin) return Reflect.apply(target, this._jig, args)

    // Special handle toString because it is not a jig method that needs to be wrapped
    // if (this._name === 'toString') {
    // return '123'
    // }
    // if (this._jig instanceof Code && this._name === 'toString') {
    // return 'abc'
    // return Function.prototype.toString.call(this._jig)
    ...
    if (STACK.length) {
      const clonedArgs = _deepClone(args, SI)

      console.log('in')

      return this._applyInternal(target, thisArg, clonedArgs)
    }
    ...
    _recordMultiple(() => {
      ...
      _record(record => {
        ...
        RECORD = record
        try {
          ret = this._applyInternal(target, thisArg, clonedArgs)

          // Record the action and args
        } finally {
          RECORD = null
        }
      })
  ...
  _applyInternal (target, thisArg, clonedArgs) {
    const Command = require('./command')

    STACK.push(this._jig)

    let ret = null

    try {
      ret = Reflect.apply(target, this._jig, clonedArgs)
    } catch (e) {
      ERROR = e
    } finally {
      STACK.pop()
    ...
    if (!STACK.length) {
      ...
      Command._call(RECORD, this._name, clonedArgs)
    }
...
class Membrane {
  ...
  get (target, prop, receiver) {
    ...
    // If in admin mode, return properties and methods directly
    // if (ADMIN) { return this._target[prop] }
    ...
    if (typeof val === 'function') {
      ...
      const methodHandler = new MethodHandler(this._proxy, prop)
  ...
  _checkSettable (prop, value) {
    ...
    if (this._proxy instanceof Code) {
      ...
      _assert(!reserved.includes(prop), 'Must not set a reserved property')
    }
