...
const {
  _assert, _text, _hasOwnProperty, _setOwnProperty, _serializable,
...
// Jigs whose membrane is created but is not yet returned in gets. The PENDING_MEMBRANES will
// contain any objects assigned and also all their sub-objects. Local objects created in a method
// and then assigned to the jig are in this state. They will leave the PENDING_MEMBRANES
...
class Membrane {
    ...
    return this._captureErrors(() => {
      ...
      return RECORD()._capture(() => {
        ...
        // Push a call action onto the stack and then call the method
          ...
          if (!_serializable(ret)) throw new StateError('Return value not serializable')
  ...
  _defineProperty (target, prop, desc) {
    ...
    return this._captureErrors(() => {
      ...
      if (this._isFinalCodeProp(prop)) throw new Error(`Cannot define ${prop}`)
      if (this._isFinalJigProp(prop)) throw new Error(`Cannot define ${prop}`)
      if (this._isFinalBerryProp(prop)) throw new Error(`Cannot define ${prop}`)
  ...
  _deleteProperty (target, prop) {
    ...
    return this._captureErrors(() => {
      ...
      if (this._isFinalCodeProp(prop)) throw new Error(`Cannot delete ${prop}`)
      if (this._isFinalJigProp(prop)) throw new Error(`Cannot delete ${prop}`)
      if (this._isFinalBerryProp(prop)) throw new Error(`Cannot delete ${prop}`)
  ...
  _has (target, prop) {
    ...
    return this._captureErrors(() => {
      ...
      if (this._isFinalCodeProp(prop)) return true
      if (this._isFinalJigProp(prop)) return true
      if (this._isFinalBerryProp(prop)) return true
  ...
  _set (target, prop, value, receiver) {
    ...
    return this._captureErrors(() => {
      ...
      if (this._isFinalCodeProp(prop)) throw new Error(`Cannot set ${prop}`)
      if (this._isFinalJigProp(prop)) throw new Error(`Cannot set ${prop}`)
      if (this._isFinalBerryProp(prop)) throw new Error(`Cannot set ${prop}`)
  ...
  _wrapForExport (value, parentJig, returning, unclaimedSet = new Set()) {
    ...
    // const skipPendingClaim = exportingInside && pendingClaim
    // if (skipPendingClaim) return value

    // When returning from a method, claimed objects get a membrane. Objects still unclaimed
    // are returned without one, allowing methods to return regular objects with information.
    const claimStatus = this._claimStatus(value, parentJig)
    if (exportingInside && claimStatus._pending) return value

    // function unclaimed() {
    // if (exportingInside) return false
    // }

    // The above logic makes

    // const claimed = exportingInside || !returning || pendingMembranes.has(value)
    // console.log(value, claimed, exportingInside, returning, pendingMembranes.has(value))
    // if (claimed) {
    // value = this._addParentRules(value, parentJig)
    // return value
    // }
    ...
    // if (pendingMembranes.has(value)) return true
    // }
    ...
    // const unclaimed = !exportingInside && (returning && !pendingClaim)
    // console.log(unclaimed, value, returning, exportingInside)

    if (!claimStatus._claimed) {
      unclaimedSet.add(value)
      _sudo(() => shallowReplace(value, x => this._wrapForExport(x, parentJig, returning, unclaimedSet)))
      return value
    }

    // Add parent rules no matter whether returning to us or them
    // Parent rules handle reads and writes. Only exception is PENDING_MEMBRANES,
    // which means this jig is already a write if a value is in it.
    value = this._addParentRules(value, parentJig)

    // Add a cow membrane if returning outside and cow props are enabled.
    const cow = !exportingInside && this._rules._cowProps
    if (cow) value = this._addCowPropMembrane(value)

    return value
  }

  _claimStatus (value, parentJig) {
    // For simplicity, always work on targets without membranes. CHILD_MEMBRANES uses that too.
    value = Proxy2._getTarget(value)

    // If we've already extracted this value out of the jig, we know it is claimed
    const existingClaim = CHILD_MEMBRANES.has(parentJig) && CHILD_MEMBRANES.get(parentJig).has(value)
    if (existingClaim) return { _claimed: true, _pending: false }

    // If we've put this value into the jig in the current method, we know its claimed and pending
    const pendingClaim = PENDING_MEMBRANES.has(value)
    if (pendingClaim) return { _claimed: true, _pending: true }

    // Because we do not add membranes to pending claims, adds to pending claims are not tracked.
    // The only way then to know is to deeply visit all members of the parentJig to see.
    const ret = { _claimed: false, _pending: true }

    _sudo(() => _deepVisit(parentJig, (x, parent) => {
      // Don't traverse into other creations because we know they won't own this value
      if (x instanceof Creation) return false

      // If not a match, then continue
      if (x !== value) return true

      // We know now it is claimed
      ret._claimed = true

      // Is it pending? if we added it to any non-pending parent, then no. Otherwise, yes.
      if (ret._pending) {
        const parentClaimStatus = this._claimStatus(parent, parentJig)
        if (!parentClaimStatus._pending) {
          ret._pending = false
        }
      }

      // If we are pending, remember so for next time
      if (ret._pending) PENDING_MEMBRANES.add(value)

      // We found our object. Don't need to go deeper.
      return false
    }))
  ...
  _addCowPropMembrane (value) {
    ...
    // Primitive values are not COWs because they are value types.
    if (typeof value !== 'object' || value === null) return value

    // If inside a method of the current jig, never add a cow membrane
    if (this._inside()) return value

    // Remove the pre-existing membrane if there is one
    value = Proxy2._getTarget(value) || value
  ...
  _claimOwnership (obj) {
    return _deepReplace(obj, (value, recurse) => {
      ...
      if (this._isBasicType(value)) return
