...
class Membrane {
  ...
  _checkSerializable (prop, value) {
    ...
    _checkState(_isSerializable(value), `Not serializable: ${_text}`)
  }
  ...
  _copyOnWriteIfNecessary (target) {
    ...
    // deepClone will ensure serializability
    const clonedTarget = _deepClone(oldTarget, SI)
    Proxy2._setTarget(this._proxy, clonedTarget)
    // The jig is no longer copy-on-write since we just copied it
