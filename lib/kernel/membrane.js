...
function CODE_METHOD_NAMES () {
  if (!this._cache) {
    ...
    this._cache = Object.getOwnPropertyNames(proto).concat(Object.getOwnPropertySymbols(proto))
  }
  return this._cache
}
  ...
  _checkCanGetCreationBinding (target, prop) {
    ...
    try {
      ...
      if (prop === 'location' || prop === 'origin' || prop === 'nonce') {
        ...
        if (_defined(loc.undeployed)) throw new StateError('Hint: Sync the jig to deploy it')
        if (_defined(loc.error)) throw new StateError(`A previous error occurred\n\n${loc.error}`)
        ...
        if (!_defined(loc.txid)) throw new StateError('Hint: Sync the jig to assign it in a transaction')
      }
    } catch (e) {
