...
class Membrane {
  ...
  _apply (target, thisArg, args) {
    ...
    return this._captureErrors(() => {
      ...
      return RECORD()._capture(() => {
        ...
        const callArgs = crossing ? targetMembrane._cow(args) : args
  ...
  _get (target, prop, receiver) {
    ...
    return this._captureErrors(() => {
      ...
      const owned = true
      value = parentMembrane ? parentMembrane._export(value, owned) : value
  ...
  _getOwnPropertyDescriptor (target, prop) {
    ...
    return this._captureErrors(() => {
      ...
      // Wrap this object with a membrane that enforces parent rules
      const owned = true
      desc.value = this._export(desc.value, owned)
  ...
  _intrinsicOut (value) {
    ...
    return this._captureErrors(() => {
      ...
      // Wrap this object with a membrane that enforces parent rules
      const owned = true
      value = this._export(value, owned)
  ...
  _export (value, owned = undefined, unclaimedSet = new Set()) {
    ...
    const status = claimStatus(value, owned)
    ...
    if (!status._claimed) {
      ...
      const wrapInner = x => this._export(x, owned, unclaimedSet)
  ...
  _addParentRules (value) {
    ...
    if (!CHILD_MEMBRANES.has(this._jig)) CHILD_MEMBRANES.set(this._jig, new WeakMap())
    if (CHILD_MEMBRANES.get(this._jig).has(value)) return CHILD_MEMBRANES.get(this._jig).get(value)
    ...
    CHILD_MEMBRANES.get(this._jig).set(value, membrane)
  ...
  /**
   * Takes ownership of the object and returns it as an unproxied target
   */
  _claim (obj) {
    return _deepReplace(obj, (value, recurse) => {
      ...
      if (!membrane) {
        if (RECORD()._stack.length) PENDING_MEMBRANES.add(target)
...
function finalizePendingMembranes (thisArg) {
  ...
  _sudo(() => _deepReplace(PENDING_MEMBRANES, (x, recurse) => {
    ...
    return _deepClone(target, SI, y => claimStatus(y)._pending ? y : undefined)
  }))
}
...
function claimStatus (value, owned) {
  ...
  if (owned) return { _claimed: true, _pending: false }
