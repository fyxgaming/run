...
let PENDING_MEMBRANES = null

// If the owner or satoshis changed, they will be marked unbound in the record
// at the end of all the jig's methods. Unbound creations cannot be changed.
let PENDING_UNBIND = null
...
class Membrane {
  ...
  _apply (target, thisArg, args) {
    ...
    return this._captureRecordErrors(() => {
      ...
      return RECORD()._capture(() => {
        ...
        const savedPendingUnbind = PENDING_UNBIND

        try {
          if (crossing || !PENDING_MEMBRANES) {
          ...
          PENDING_UNBIND = savedPendingUnbind
        }
      })
    })
  }
  ...
  _defineProperty (target, prop, desc) {
    ...
    return this._captureRecordErrors(() => {
      ...
      if (this._isUtxoBinding(prop)) PENDING_UNBIND = true
  ...
  _checkCanGetUtxoBinding (prop) {
    ...
    try {
      ...
      if (prop === 'owner' || prop === 'satoshis') {
        ...
        if (undetermined) throw new StateError('Hint: Sync the jig to bind it in a transaction')
  ...
  _checkCanChangeLocationBinding (prop, value, method) {
    ...
    if (this._isLocationBinding(prop)) throw new StateError(`Cannot ${method} ${prop}`)
  }
  ...
  _checkCanChangeUtxoBinding (prop, value, method) {
    ...
    if (this._deleted()) throw new StateError(`Cannot ${method} ${prop}`)
  ...
  _claim (value) {
    ...
    if (!membrane) {
      if (STACK().length && PENDING_MEMBRANES) PENDING_MEMBRANES.add(target)
      return target
    }
  ...
  _finalizePending () {
    ...
    _deepReplace(PENDING_MEMBRANES, (x, recurse) => {
    ...
    // Deleted creations are unbound
    if (this._deleted()) PENDING_UNBIND = true

    // If either the owner or satoshis were changed, the creation becomes unbound
    if (PENDING_UNBIND) RECORD()._unbind(this._creation)
  }
  ...
  _pending (value) {
    ...
    if (!PENDING_MEMBRANES) return false
    ...
    const stack = RECORD()._stack
    const inside = stack.length && stack[stack.length - 1]._creation === this._creation
    if (!inside) return false

    // Return claimed and pending membranes we know about. We might not know them all.
    if (PENDING_MEMBRANES.has(value)) return true

    // We may have assigned to a pending claim. It would not be in our PENDING_MEMBRANES set,
    ...
    // proxy would already be accessible from non-pending objects in the PENDING_MEMBRANES set.
    ...
    if (pending) PENDING_MEMBRANES.add(value)
  ...
  _isLocationBinding (prop) { return this._rules._locationBindings && _CREATION_BINDINGS.includes(prop) }
