...
const {
  ...
  _RESERVED_PROPS, _RESERVED_CODE_METHODS, _RESERVED_JIG_METHODS, _getOwnProperty,
  _deterministicCompareKeys, _isBasicSet, _isBasicMap
} = require('../util/misc')
...
function makeCodeMethod (name) {
  const Code = require('./code')
  const key = name === Symbol.hasInstance ? 'Symbol.hasInstance' : `'${name}'`
  const funcname = name === Symbol.hasInstance ? 'hasInstance' : name
  const script = `function ${funcname} (...args) { return Code.prototype[${key}].apply(this, args) }`
...
const CODE_METHODS = {
  ...
  [Symbol.hasInstance]: makeCodeMethod(Symbol.hasInstance)
}

// destroy and auth are not protected
const CODE_METHOD_NAMES = ['toString', 'sync', 'upgrade', 'destroy', 'auth', Symbol.hasInstance]

const JIG_METHOD_NAMES = ['init', 'sync'] // destroy and auth are not protected
...
class Membrane {
  ...
  _apply (target, thisArg, args) {
    ...
    return this._captureErrors(() => {
      ...
      let membrane = Proxy2._getHandler(thisArg)
      const disabledMethods = membrane && membrane._rules._disabledMethods
      const disabled = disabledMethods && disabledMethods.includes(target.name)
      _checkState(!disabled, `${target.name} disabled`)

      // TODO: Jig.init() ... is not callable and recordable. So doesn't get unified.
      let callable = membrane && membrane._rules._callable && this._rules._recordable

      // If not callable, see if we can make it callable. Check both if thisArg is
      // a function and whether it is not a Jig instance to support mocks in tests.
      const Jig = require('./jig')
      if (!callable && typeof thisArg === 'function' && !(thisArg instanceof Jig)) {
        thisArg = new Code(thisArg)
        membrane = Proxy2._getHandler(thisArg)
        callable = membrane && membrane._rules._callable && this._rules._recordable
      }

      // If this object isn't callable, then we run it directly. This is used for
      ...
      if (!callable) return Reflect.apply(target, thisArg, args)
      ...
      return RECORD()._capture(() => {
        ...
        Action._call(thisArg, target.name, args, () => {
          ...
          _checkState(latestTarget, errorMessage)

          // Calling a function requires a read of the code jig being called
          // We perform this again in case it wasn't captured above.
          if (this._shouldRecord()) RECORD()._read(latestTargetJig)
  ...
  _defineProperty (target, prop, desc) {
    ...
    return this._captureErrors(() => {
      ...
      if (!(desc.value instanceof Universal)) {
  ...
  _deleteProperty (target, prop) {
    ...
    return this._captureErrors(() => {
      ...
      // Code and jig methods are permanent by design
      if (this._isCodeMethod(prop)) throw new Error(`Cannot delete ${prop}`)
      if (this._isCodeDeps(prop)) throw new Error(`Cannot delete ${prop}`)
      if (this._isJigMethod(prop)) throw new Error(`Cannot delete ${prop}`)

      // Bindings cannot be deleted by design
      if (this._isBinding(prop)) throw new Error(`Cannot delete ${prop}`)
  ...
  _get (target, prop, receiver) {
    ...
    return this._captureErrors(() => {
      ...
      // Code and Jig methods are specially handled. They are not reads since they don't change.
      const hasInstanceOverride = prop === Symbol.hasInstance &&
        target[Symbol.hasInstance] !== SI.Function.prototype[Symbol.hasInstance]
      if (this._isCodeMethod(prop) && !hasInstanceOverride) return CODE_METHODS[prop]
      if (this._isJigMethod(prop) && prop !== 'init') {
        const Jig = require('./jig')
  ...
  _getOwnPropertyDescriptor (target, prop) {
    ...
    return this._captureErrors(() => {
      ...
      // Code and jig methods are not owned properties and are not overrideable
      if (this._isCodeMethod(prop)) return undefined
      if (this._isJigMethod(prop)) return undefined
  ...
  _has (target, prop) {
    ...
    return this._captureErrors(() => {
      ...
      // Code and jig methods are part of the object, but not owned properties
      if (this._isCodeMethod(prop)) return true
      if (this._isJigMethod(prop)) return true
  ...
  _checkIfSetInMethod () {
    if (!this._rules._owned) return
    const stack = RECORD()._stack
    if (stack.length && stack[stack.length - 1]._jig === this._jig) return
    throw new Error('Updates must be performed in the jig\'s methods')
  }
  ...
  _checkCanGetBinding (target, prop) {
    ...
    try {
      ...
      if (prop === 'owner' || prop === 'satoshis') {
        ...
        _checkState(!undetermined, undeterminedHint)
  ...
  _checkCanSetBinding (target, prop, value) {
    ...
    _checkState(prop !== 'origin', 'Cannot set origin')
    _checkState(prop !== 'location', 'Cannot set location')
    _checkState(prop !== 'nonce', 'Cannot set nonce')
    ...
    const unboundError = `Cannot set ${prop}\n\n${unboundHint}`
  ...
  _checkCanSetCodeOption (prop, value) {
    ...
    if (prop === 'sealed') Code._checkSealedOption(value)
    if (prop === 'upgradable') Code._checkUpgradableOption(value)
  }
  ...
  _checkReserved (prop) {
    ...
    _checkState(!_RESERVED_PROPS.includes(prop), error)
    if (this._rules._jigMethods) _checkState(!_RESERVED_JIG_METHODS.includes(prop), error)
    if (this._rules._codeMethods) _checkState(!_RESERVED_CODE_METHODS.includes(prop), error)
  }
  ...
  _wrapForImport (thisMembrane, value) {
    ...
    if (!thisMembrane._rules._owned) return value
  ...
  _claimOwnership (value) {
    _deepVisit(value, value => {
      ...
      if (value instanceof Universal) return false
  ...
  _getParentJig (prop) {
    ...
    while (!_hasOwnProperty(target, prop)) {
      ...
      if (parentJig && this._shouldRecord() && Proxy2._getHandler(parentJig)._shouldRecord()) {
  ...
  _isJigMethod (prop) { return this._rules._jigMethods && JIG_METHOD_NAMES.includes(prop) }
...
function getLatestMethod (thisArg, target, proxy, jig) {
  ...
  if (!(thisArg instanceof Universal)) return [null, null]
  ...
  if (_sudo(() => thisContainer.origin === jig.origin)) {
    _sudo(() => _checkState(thisContainer.nonce >= jig.nonce, 'Method time travel'))
  ...
  while (prototype) {
    ...
    if (prototypeMethod && _sudo(() => prototypeMethodContainer.origin === jig.origin)) {
      _sudo(() => _checkState(prototypeMethodContainer.nonce >= jig.nonce, 'Method time travel'))
