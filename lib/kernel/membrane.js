...
class Membrane {
  ...
  _apply (target, thisArg, args) {
    ...
    return this._trap(() => {
      ...
      // If we are crossing a jig boundary, this will COW the args
      ...
      const callMethod = () => {
        ...
        try {
          ...
          if (!stack.length || thisArg !== stack[stack.length - 1]._jig) {
      ...
      Action._call(thisArg, target.name, actionArgs, () => callMethod())
  ...
  _wrapForExport (value, parentJig) {
    ...
    if (!inside) value = this._addCowMembrane(value)
