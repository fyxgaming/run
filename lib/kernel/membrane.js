...
class Membrane {
  ...
  constructor (target, rules = new Rules()) {
    ...
    // Determine the jig that the target is owned by
    if (!_defined(rules._parentJig)) {
      this._creation = this._proxy
    } else {
      this._creation = rules._parentJig
    }

    // If we have a parent jig, copy its child and pending membranes for ease of access
    if (rules._parentJig) {
      const parentMembrane = Proxy2._getHandler(rules._parentJig)
      this._pendingMembranes = parentMembrane._pendingMembranes
      this._childMembranes = parentMembrane._childMembranes
    }
  ...
  _apply (target, thisArg, args) {
    ...
    return this._captureErrors(() => {
      ...
      const crossing = !stack.length || thisArg !== stack[stack.length - 1]._creation
      ...
      return RECORD()._capture(() => {
        ...
        try {
          ...
          Action._call(thisArg, target.name, args, () => {
            ...
            const [latestTarget, latestTargetJig] = getLatestMethod(thisArg, target, this._proxy, this._creation)
  ...
  _get (target, prop, receiver) {
    ...
    return this._captureErrors(() => {
      ...
      const parentMembrane = parentJig && Proxy2._getHandler(parentJig)

      // Wrap this object with a membrane that enforces parent rules
      const claimed = true
      value = parentMembrane ? parentMembrane._export(value, claimed) : value
  ...
  _throwIfJigErrors () {
    ...
    const jigTarget = Proxy2._getTarget(this._creation)
  ...
  _checkIfSetInMethod () {
    ...
    const stack = RECORD()._stack
    if (stack.length && stack[stack.length - 1]._creation === this._creation) return
  ...
  _finalizePendingMembranes () {
    ...
    _sudo(() => _deepReplace(this._pendingMembranes, (x, recurse) => {
      ...
      if (membrane._rules._parentJig === this._creation) return target
  ...
  _claimStatus (value, claimed) {
    // Cow args do not have parent jigs and are not claimed
    const membrane = Proxy2._getHandler(value)
    if (membrane && !membrane._rules._parentJig) return { _claimed: false, _pending: false }
  ...
  /**
   * Takes ownership of the object and all subobjects and returns it as an
   * unproxied target suitable for storage
   */
  _claim (obj) {
    return _deepReplace(obj, (value, recurse) => {
      ...
      if (!membrane) {
        if (RECORD()._stack.length) this._pendingMembranes.add(target)
      ...
      if (membrane._rules._parentJig === this._creation) {
        recurse(false)
        return target
      }
  ...
  _inside () { const s = RECORD()._stack; return s.length && s[s.length - 1]._creation === this._creation }
}
