...
class Membrane {
  ...
  _apply (target, thisArg, args) {
    ...
    return this._captureErrors(() => {
      ...
      return RECORD()._capture(() => {
        ...
        // to use when exporting the return value.
        let methodPendingMembranes
        ...
        Action._call(thisArg, target.name, args, () => {
          ...
          try {
            ...
            methodPendingMembranes = PENDING_MEMBRANES
        ...
        return this._wrapForExport(ret, thisArg, owned, returning, methodPendingMembranes)
      })
    })
  }
  ...
  _get (target, prop, receiver) {
    ...
    return this._captureErrors(() => {
      ...
      if (this._isFinalCodeProp(prop) && _RESERVED_CODE_METHODS.includes(prop)) {
        ...
        method = this._addParentRules(method, Code, true)
        return method
      }
      ...
      value = this._wrapForExport(value, parentJig, owned, returning)
  ...
  _getOwnPropertyDescriptor (target, prop) {
    ...
    return this._captureErrors(() => {
      ...
      const owned = true
      const returning = false
      desc.value = this._wrapForExport(desc.value, this._jig, owned, returning)
  ...
  _intrinsicOut (value) {
    ...
    return this._captureErrors(() => {
      ...
      const owned = true
      const returning = false
      value = this._wrapForExport(value, this._jig, owned, returning)
  ...
  _wrapForExport (value, parentJig, owned, returning,
    methodPendingMembranes, unclaimedSet = new Set()) {
    ...
    if (exportingInside && PENDING_MEMBRANES.has(value)) return value
    ...
    const unclaimed = !exportingInside && returning && !methodPendingMembranes.has(value)

    if (unclaimed) {
      unclaimedSet.add(value)

      _sudo(() => {
        const replacer = x => this._wrapForExport(x, parentJig,
          owned, returning, methodPendingMembranes, unclaimedSet)
  ...
  _claimOwnership (obj) {
    return _deepReplace(obj, (value, recurse) => {
      ...
      const method = typeof value === 'function'
      const owned = true
      const rules = Rules._childProperty(this._jig, method, owned)
