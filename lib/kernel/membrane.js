...
class Membrane {
  ...
  _apply (target, thisArg, args) {
    ...
    return this._captureErrors(() => {
      ...
      return RECORD()._capture(() => {
        ...
        const owned = false
        const returned = true
        const ownerBinding = false
        return this._wrapForExport(ret, ownerBinding, thisArg, owned, returned, methodPendingMembranes)
      })
    })
  }
  ...
  _get (target, prop, receiver) {
    ...
    return this._captureErrors(() => {
      ...
      const ownerBinding = this._isUtxoBinding(prop) && prop === 'owner'
      value = this._wrapForExport(value, ownerBinding, parentJig, owned, returned)
  ...
  _getOwnPropertyDescriptor (target, prop) {
    ...
    return this._captureErrors(() => {
      ...
      const returned = false
      const ownerBinding = this._isUtxoBinding(prop) && prop === 'owner'
      desc.value = this._wrapForExport(desc.value, ownerBinding, this._jig, owned, returned)
  ...
  _intrinsicOut (value) {
    ...
    return this._captureErrors(() => {
      ...
      const returned = false
      const ownerBinding = false
      value = this._wrapForExport(value, ownerBinding, this._jig, owned, returned)
  ...
  _wrapForExport (value, ownerBinding, parentJig, owned, returned,
    ...
    if (unclaimed) {
      ...
      _sudo(() => {
        const replacer = x => this._wrapForExport(x, false, parentJig,
          owned, returned, methodPendingMembranes, unclaimedSet)
    ...
    // Also add a cow prop for the owner UTXO binding, because it's often assigned.
    const cow = !inside && (this._rules._cowProps || ownerBinding)
