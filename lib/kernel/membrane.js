...
class Membrane {
  ...
  _apply (target, thisArg, args) {
    ...
    return this._trap(() => {
      ...
      if (this._rules._records) RECORD()._read(this._jig)
      ...
      if (!this._rules._actions) return callMethod()
  ...
  _defineProperty (target, prop, desc) {
    ...
    return this._trap(() => {
      ...
      if (this._rules._records) RECORD()._update(this._jig)
  ...
  _getParentJig (prop) {
    ...
    while (!_hasOwnProperty(target, prop)) {
      ...
      if (this._rules._records) RECORD()._read(parentJig)
    }
