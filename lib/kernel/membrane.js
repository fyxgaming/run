...
class Membrane {
  ...
  _apply (target, thisArg, args) {
    ...
    return this._captureErrors(() => {
      ...
      const callable = membrane && membrane._rules._callable
      ...
      return RECORD()._capture(() => {
        ...
        const { _NewAction } = require('./action')
        if (!stack.length || (stack.length === 1 && stack[0] instanceof _NewAction)) {
  ...
  _get (target, prop, receiver) {
    ...
    return this._captureErrors(() => {
      ...
      if (this._isJigMethod(prop) && prop !== 'init') return Reflect.get(target, prop, receiver)
