...
const { _checkState, _hasOwnProperty } = require('../util/misc')
...
CODE_TRAPS.construct = function (target, args, newTarget) {
  checkNotErrored(target)
...
CODE_TRAPS.deleteProperty = function (target, prop) {
  checkNotErrored(target)
...
CODE_TRAPS.get = function (target, prop, receiver) {
  ...
  if (Object.keys(CODE_METHODS).includes(prop)) return CODE_METHODS[prop]
...
CODE_TRAPS.getPrototypeOf = function (target) {
  checkNotErrored(target)
...
CODE_TRAPS.has = function (target, prop) {
  checkNotErrored(target)
...
CODE_TRAPS.ownKeys = function (target) {
  checkNotErrored(target)
...
CODE_TRAPS.preventExtensions = function (target) {
  checkNotErrored(target)

  // Prevent extensions is not allowed on any jigs. Jigs can always be changed.
...
CODE_TRAPS.setPrototypeOf = function (target, prototype) {
  checkNotErrored(target)

  // Users cannot change prototypes directly
  if (!_admin()) throw new Error('Cannot change prototype')
...
METHOD_TRAPS.apply = function (target, thisArg, args) {
  // TODO
}
