...
class Membrane {
  ...
  _checkCanGetBinding (target, prop) {
    ...
    if (!this._isBinding(prop)) return
  ...
  _isBinding (prop) {
    if (!this._rules._bindings) return false
    if (this._rules._berryMethods && prop !== 'location') return false
    return _BINDINGS.includes(prop)
  }
