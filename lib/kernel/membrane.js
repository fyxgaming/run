...
class Membrane {
  ...
  _defineProperty (target, prop, desc) {
    ...
    // Check if this value is settable
    desc.value = this._checkSettable(target, prop, desc.value, 'define')
  ...
  _set (target, prop, value, receiver) {
    ...
    // Check if this value is settable
    value = this._checkSettable(target, prop, value, 'set')
  ...
  _checkSettable (target, prop, value, handler) {
    // Code methods are permanent
    if (this._isCodeMethod(prop)) throw new Error(`Cannot ${handler} ${prop}`)
    ...
    if (!this._hasPrivateAccess(prop)) {
      throw new Error(`Cannot ${handler} private property ${prop}`)
    }
    ...
    // Enforce immutability
    if (this._immutable) throw new Error(`${handler} disabled`)
