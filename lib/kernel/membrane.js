...
class Membrane {
  ...
  _init (jig, jigTarget) {
    ...
    this._jig = jig
    // Unwrapped jig object that can be accessed without admin mode
    this._jigTarget = jigTarget
    // Whether this membrane is for a jig, meaning top level, or some inner object
    this._topLevel = true
  ...
  _throwIfJigErrors () {
    ...
    if (!_hasOwnProperty(this._jigTarget, 'location')) return

    // Undeployed jigs can still be used because they will be deployed after the action completes.
    const { error, undeployed } = _location(this._jigTarget.location)
  ...
  _isCodeMethod (prop) {
    return this._codeMethods && CODE_METHOD_NAMES.includes(prop)
  }
}
