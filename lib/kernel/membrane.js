...
const CHILD_MEMBRANES = new WeakMap() // Target -> Proxcy
...
// set once the current method completes. The goal is to allow a consistent worldview where
...
class Membrane {
  ...
  _apply (target, thisArg, args) {
    ...
    return this._trap(() => {
      ...
      if (this._rules._recordable) RECORD()._read(this._jig)
      ...
      // TODO: COW wrapping in get when going external
      // TEST You can't change a jig's inner properties. They are cow-wrapped.
      // Only if replayable.

      // a.arr.find(..) // not wrapped
      // but intrinsicout is wrapped, so is get. it'll be wrapped.

      // Wrap the args if we need to
      const callArgs = this._wrapForImport(membrane, args)

      // Helper to call the method whether this is an aciton or not
      const callMethod = () => {
        const savedPendingMembranes = PENDING_MEMBRANES
        try {
          PENDING_MEMBRANES = new Set()
          const ret = Reflect.apply(target, thisArg, callArgs)
          return this._wrapForExport(ret)
        } finally {
      ...
      /*
      Return value... add parent rules? No. just COW.

      Leaving .. COW
      What if to another internal method.

      AddParentRules after action ends if returning internally.
      If returning externally, COW

      PENDING_MEMBRANES DOES NOT APPLY TO STATIC CODE

      If not replayable, return directly.

      Clone -> Replayable
      Unify -> replayable
      No. Everything becomes serializable.

      Everything is COW-cloned out. Method ... deal with it
      COW clone every non-method.
      */

      // If not recording actions, then pass the method call through. This is used for static code.
      ...
      const thisArgReplayable = thisArgMembrane && thisArgMembrane._rules._replayable
      if (!thisArgReplayable) return callMethod()

      // Push a call action onto the stack
      const Action = require('./action')
      let ret = null
      const actionArgs = _deepClone(args)
      Action._call(thisArg, target.name, actionArgs, () => { ret = callMethod() })
  ...
  _defineProperty (target, prop, desc) {
    ...
    return this._trap(() => {
      ...
      if (this._rules._recordable) RECORD()._update(this._jig)
  ...
  _get (target, prop, receiver) {
    ...
    return this._trap(() => {
      ...
      // Add COW membrane if going external
      value = this._addCowMembrane(value)
  ...
  _getOwnPropertyDescriptor (target, prop) {
    ...
    return this._trap(() => {
      ...
      // Wrap this object with a membrane that enforces parent rules
      desc.value = this._addParentRules(desc.value, this._jig)

      // Add COW membrane if going external
      desc.value = this._addCowMembrane(desc.value)
  ...
  _intrinsicOut (value) {
    ...
    return this._trap(() => {
      ...
      // Wrap this object with a membrane that enforces parent rules
      value = this._addParentRules(value, this._jig)
  ...
  _claimOwnership (value) {
    // If this is a top-level jig, then it can't be claimed
    ...
    if (membrane) {
      ...
      _assert(!membrane._rules._parentJig)
  ...
  _addParentRules (value, parentJig) {
    ...
    if (PENDING_MEMBRANES.has(value)) return value
    ...
    if (CHILD_MEMBRANES.has(value)) return CHILD_MEMBRANES.get(value)
    ...
    CHILD_MEMBRANES.set(value, membrane)
  ...
  _addCowMembrane (value) {
    ...
    const stack = RECORD()._stack
    if (stack._length && stack[stack._length - 1]._jig === this._jig) return value
  ...
  // Converts a value into one safe to use in this any replayable action. For now
  // we only use this on the method args, because any other way of getting data in
  // would already be in a method where its arguments were wrapped. In the future,
  // if we support top-level getters and setters, then we would wrap there too.
  _wrapForImport (thisMembrane, value) {
    ...
    // Determine the special case if we are entering the jig from outside entirely
    // This may be from calling a method or setting a property (a future setter).
    const stack = RECORD()._stack
    if (!stack.length) {
      // Clone the value using sandbox intrinsics
      const clonedValue = _deepClone(value, SI)

      // Unify the worldview so that this jig is in harmony with the args
      thisMembrane._unifyWorldview(this._jig, clonedValue)

      // No membranes required since we cloned. Return.
      return clonedValue
    }

    // Determine if we are crossing a boundary from another jig to this one
    const crossing = stack[stack.length - 1]._jig !== thisMembrane._jig
  ...
  _wrapForExport (value) {
    // Add a COW membrane if leaving the jig
    // Unique each time. No save.
    // Same for inputs
    ...
    // If two stack actions are different => crossing out
    // If one or less stack action, leaving
  ...
  _getParentJig (prop) {
    ...
    while (!_hasOwnProperty(target, prop)) {
      ...
      if (this._rules._recordable) RECORD()._read(parentJig)
    }
