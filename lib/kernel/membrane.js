...
class Membrane {
  ...
  _defineProperty (target, prop, desc) {
    ...
    return this._trap(() => {
      ...
      // If this is a naked object and we're in a method, add to our pending membranes set
      const membrane = Proxy2._getHandler(desc.value)
      if (!membrane && RECORD()._stack.length) PENDING_MEMBRANES.add(desc.value)

      // If is a COW object, then make a copy
      if (membrane) membrane._copyOnWriteIfNecessary()
      if (membrane) membrane._assignParentJig(this._jig)
  ...
  _checkOwnership (value) {
    ...
    // If there is no membrane, then there is no owner
    if (!membrane) return
  ...
  _copyOnWriteIfNecessary (target) {
    // If don't need to copy, then return
    if (!this._rules._cow) return target
