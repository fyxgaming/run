...
class Membrane {
  ...
  _apply (target, thisArg, args) {
    ...
    return this._captureRecordErrors(() => {
      ...
      return RECORD()._capture(() => {
        ...
        try {
          if (crossing || !PENDING) PENDING = new Set()
          ...
          Action._call(thisArg, target.name, args, () => {
            ...
            if (!latestFunction) throw new StateError(`Cannot call ${target.name} on ${_text(thisArg)}`)
  ...
  _defineProperty (target, prop, desc) {
    ...
    return this._captureRecordErrors(() => {
      ...
      desc.value = this._claim(desc.value, this._isUtxoBinding(prop))

      // When utxo bindings are set, they are unbound until the record is committed
  ...
  _set (target, prop, value, receiver) {
    ...
    return this._captureRecordErrors(() => {
      ...
      value = this._claim(value, this._isUtxoBinding(prop))
  ...
  _checkCanChangeProp (prop, value, method) {
    ...
    if (this._isFinalProp(prop)) throw new Error(`Cannot ${method} ${prop}: property is final`)
    ...
    this._checkCanChangeCreationBinding(prop, value, method)
  ...
  _checkCanGetProp (prop, method) {
    ...
    this._checkCanGetCreationBinding(prop)
  ...
  _checkCanGetCreationBinding (prop) {
  ...
  // --------------------------------------------------------------------------
  // _checkCanChangeCreationBinding
  // --------------------------------------------------------------------------

  _checkCanChangeCreationBinding (prop, value, method) {
    ...
    if (this._isCreationBinding(prop)) throw new StateError(`Cannot ${method} ${prop}`)
  }
  ...
  _checkCanChangeUtxoBinding (prop, value, method) {
    ...
    if (method === 'delete') throw new StateError(`Cannot ${method} ${prop}`)
    ...
    if (isUnbound) {
      ...
      throw new StateError(`Cannot ${method} ${prop}\n\n${unboundHint}`)
    }
  ...
  _export (value) {
    ...
    const Unbound = require('../util/unbound')
    value = value instanceof Unbound ? value._value : value
  ...
  _return (value, crossing, unclaimed = new Set()) {
    ...
    // If it already has a membrane, which should only happen for prototype methods on native
    // code, and also properties of other jigs, and creations, then return it directly, because
  ...
  // unproxied target suitable for storing in the creation
  _claim (value, unbound) {
    return _deepReplace(value, (value, recurse) => {
      ...
      if (!membrane) {
        if (STACK().length && PENDING) PENDING.add(target)
      ...
      // it is read. We don't need to mark the creation as a read because we would have already.
  ...
  _finalizePending () {
    ...
    _sudo(() => _deepReplace(PENDING, (x, recurse) => {
  ...
  _pending (value) {
    ...
    const stack = RECORD()._stack
    if (stack.length && stack[stack.length - 1]._creation !== this._creation) return false

    // Return claimed and pending membranes we know about. We might not know them all.
    if (PENDING.has(value)) return true

    // We may have assigned to a pending claim. It would not be in our PENDING set,
    ...
    // proxy would already be in the PENDING set. This includes creations.
    let pending = false
    _deepVisit(PENDING, x => {
  ...
  _isNativeProp (prop) {
    return (this._isFinalCodeProp(prop) && _RESERVED_CODE_METHODS.includes(prop)) ||
        (this._isFinalJigProp(prop) && _RESERVED_JIG_METHODS.includes(prop)) ||
        (this._isFinalBerryProp(prop) && _RESERVED_BERRY_METHODS.includes(prop))
  }
...
function checkValidDescriptor (desc, configurable = true) {
  ...
  if (configurable && !desc.configurable) throw new StateError('Descriptor must be configurable')
  if (!configurable && desc.configurable) throw new StateError('Descriptor must be non-configurable')
...
function checkValidPropFields (x) {
  _sudo(() => {
    ...
    // Uint8array elements must all be non-configurable
    const nonconfigurable = x instanceof SI.Uint8Array || x instanceof HI.Uint8Array
    ...
    Object.getOwnPropertyNames(x)
      ...
      .forEach(desc => checkValidDescriptor(desc, !nonconfigurable))
  })
}
...
function getLatestFunction (thisArg, functionProxy) {
  ...
  if (thisArgFunctionCreationOrigin === functionCreationOrigin) {
    if (thisArgFunctionCreationNonce < functionCreationNonce) throw new StateError('Method time travel')
  ...
  const stack = RECORD()._stack
  const inside = stack.length >= 2 && stack[stack.length - 2]._creation === thisArg
  ...
  while (prototype) {
    ...
    if (prototypeMethod && _sudo(() => prototypeMethodContainer.origin === functionCreationOrigin)) {
      _sudo(() => {
        if (prototypeMethodContainer.nonce < functionCreationNonce) throw new StateError('Method time travel')
      })
