...
class Membrane {
  ...
  _apply (target, thisArg, args) {
    ...
    // If not recording, then pass through
    if (!this._rules._record) return Reflect.apply(target, thisArg, args)
    ...
    return RECORD()._multiple(() => {
      ...
      // Record that we will call the method
      const Action = require('./action')
      Action._call(thisArg, target.name, methodArgs)

      // Call the method
      const ret = Reflect.apply(target, thisArg, methodArgs)
