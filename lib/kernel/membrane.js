...
const { _assert, _checkState, _text, _hasOwnProperty, _setOwnProperty } = require('../util/misc')
...
// Set on a function
// Apply without record .. allowed to pass anything as args, and return value is intact
//      -> Or static code tests
...
class Membrane {
  ...
  _apply (target, thisArg, args) {
    ...
    return this._captureErrors(() => {
      ...
      return RECORD()._multiple(() => {
        // We can only apply jig methods to jigs they belong to. Also trigger a read.
  ...
  _construct (target, args, newTarget) {
    ...
    return this._captureErrors(() => {
      ...
      return RECORD()._multiple(() => {
        ...
        // TODO: AND ALL OF THE PARENTS

        // If not recording, then pass the construct through. This is used for static code.
        // Arguments will not be cloned. The call will not be recorded anywhere. It's a helper.
        if (!this._rules._recordConstructs) return Reflect.construct(target, args, newTarget)
  ...
  _defineProperty (target, prop, desc) {
    ...
    return this._captureErrors(() => {
      ...
      if (this._recordUpdates()) RECORD()._update(this._jig)
  ...
  _get (target, prop, receiver) {
    ...
    return this._captureErrors(() => {
      ...
      // Record this read
      if (this._recordReads()) RECORD()._read(this._jig)
      ...
      // Determine the owner. The owner may be on a prototype.
        ...
        while (!_hasOwnProperty(container, prop)) {
          ...
          // If we are at the top, then we didn't find the property
          if (!container) { parentJig = null; break }
          ...
          if (parentJig instanceof Code) { parentJig = null; break }

          // Record this read
          if (this._recordReads()) RECORD()._read(parentJig)
        }

        // If the child property is an object owned by a jig, mark it as such
        if (parentJig) {
          const rules = Rules._childProperty(parentJig)
  ...
  _captureErrors (f) {
    ...
    // Check if an error has already occurred during this action
  ...
  _recordReads () { return this._rules._recordReads && RECORD()._stack.length }
  _recordUpdates () {
    if (!this._rules._recordUpdates) return false
    _checkState(RECORD()._stack.length, 'Cannot update jig outside of an action')
    return true
  }
}
