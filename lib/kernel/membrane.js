...
class Membrane {
  ...
  _construct (target, args, newTarget) {
    ...
    // How to handle hasInstance...
    // Membrane doesn't like this.
    // Or any property that is not on this directly
  ...
  _defineProperty (target, prop, desc) {
    ...
    if (this._rules._bindings && _BINDINGS.includes(prop) && !(desc.value instanceof Unbound)) {
  ...
  _deleteProperty (target, prop) {
    ...
    if (this._rules._bindings && _BINDINGS.includes(prop)) {
  ...
  _get (target, prop, receiver) {
    ...
    if (isFunctionPrototype) return Reflect.get(target, prop, receiver)

    // Symbol properties we return directly and they aren't settable by the user
    ...
    if (typeof value === 'object' || typeof value === 'function') {
      const rules = Rules._innerProperty(this._jig)
  ...
  _getOwnPropertyDescriptor (target, prop) {
    ...
    // Function prototypes must be returned directly. No changes. Always available.
    const isFunctionPrototype = typeof target === 'function' && prop === 'prototype'
    if (isFunctionPrototype) return Reflect.getOwnPropertyDescriptor(target, prop)
    ...
    // If this is an unbound binding, return its actual inner value
    const Unbound = require('../util/unbound')
    const isUnboundValue = _BINDINGS.includes(prop) && desc.value instanceof Unbound
...
class Rules {
  ...
  static _innerProperty (parentJig) {
    ...
    rules._errors = parentRules._admin
    rules._bindings = false
    rules._codeMethods = false
    rules._private = parentRules._private
    rules._immutable = parentRules._private
    rules._record = parentRules._private
    return rules
  }

  equals (rules) {
    _assert(Object.keys(rules).length === Object.keys(this).length)
