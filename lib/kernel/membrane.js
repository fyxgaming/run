...
// Proxies for inner property so that we don't create different membranes for the same objects
...
// but whose membrane has not yet been created. In gets in the current jig's method, these
...
// externally will return the membrane.
...
class Membrane {
  ...
  _apply (target, thisArg, args) {
    ...
    return this._captureErrors(() => {
      ...
      let passThrough = !targetMembrane || !targetMembrane._rules._recordableTarget ||
        !this._rules._recordCalls
      ...
      if (passThrough && typeof thisArg === 'function') {
        ...
        passThrough = !targetMembrane || !targetMembrane._rules._recordableTarget ||
          !this._rules._recordCalls
      }
      ...
      return RECORD()._capture(() => {
        ...
        // Even internal method args need to be serializable though
        ...
        // Push a call action onto the stack and then call the method
          ...
          if (crossing) this._claimPendingMembranes()
  ...
  _wrapForExport (value, parentJig, returning, unclaimedSet = new Set()) {
    ...
    // Get the claim status of this value
    const claimStatus = this._claimStatus(value, returning)

    // If this value is unclaimed, then we'll leave it intact, but we need to check inner
    // objects that might have gone undetected and if any are claimed then wrap them.
  ...
  _claimStatus (value, returning) {
    // All fully claimed values will have membranes
    if (Proxy2._getTarget(value)) return { _claimed: true, _pending: false }

    // Pending membranes were applied directly onto the jig
    ...
    // property of another jig, a creation, etc. It would go undetected, so we deep traverse.
    let found = false
  ...
  // --------------------------------------------------------------------------
  // _claimPendingMembranes
  // --------------------------------------------------------------------------

  _claimPendingMembranes () {
    // _sudo(() => {
    // for (const x of PENDING_MEMBRANES) {
    // this._checkSerializable('', x)
    // shallowReplace(x, y => this._claimOwnership(y))
    // }
    // })
  }
