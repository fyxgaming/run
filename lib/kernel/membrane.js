...
const {
  ...
  _RESERVED_PROPS, _getOwnProperty, _safeCompareKeys
} = require('../util/misc')
...
class Membrane {
  ...
  _ownKeys (target) {
    ...
    return this._captureErrors(() => {
      ...
      let keys = Reflect.ownKeys(target).sort(_safeCompareKeys)

      // Filter out private keys if we are not able to view them
      keys = keys.filter(key => this._hasPrivateAccess(key))

      // Make sure bindings are always at the front
      if (this._rules._bindings) {
        keys = keys.filter(key => !_BINDINGS.includes(key))
        keys.unshift(..._BINDINGS)
      }
