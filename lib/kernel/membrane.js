...
const { _location, _owner, _satoshis, _LOCATION_BINDINGS, _UTXO_BINDINGS } = require('../util/bindings')
...
class Membrane {
  ...
  _checkNotReserved (prop, method) {
    ...
    if (this._rules._jigMethods && _RESERVED_JIG_PROPS.includes(prop)) throwReservedError()
    if (this._rules._codeMethods && _RESERVED_CODE_PROPS.includes(prop)) throwReservedError()
    if (this._rules._berryMethods && _RESERVED_BERRY_PROPS.includes(prop)) throwReservedError()
  }
