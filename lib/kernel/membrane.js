...
// itself will be membranes. If the owner or satoshis changed, they will be marked unbound in the
// record at the end of all the jig's methods. Unbound creations cannot be changed. The PENDING
// object captures both. _membranes is a Set, and _unbind is a boolean
let PENDING = null
...
class Membrane {
  ...
  _apply (target, thisArg, args) {
    ...
    return this._captureRecordErrors(() => {
      ...
      return RECORD()._capture(() => {
        ...
        try {
          if (crossing || !PENDING) {
            PENDING = { _membranes: new Set(), _unbind: false }
          }
          ...
          Action._call(thisArg, target.name, args, () => {
            ...
            _sudo(() => console.log('RET', crossing, target.name, thisArgMembrane._creation.amount))
  ...
  _defineProperty (target, prop, desc) {
    ...
    return this._captureRecordErrors(() => {
      ...
      console.log('a', this._isUtxoBinding(prop))
  ...
  _set (target, prop, value, receiver) {
    ...
    return this._captureRecordErrors(() => {
      ...
      // When utxo bindings are set, the creation becomes unbound
      console.log('b', this._isUtxoBinding(prop), prop, value)
      if (this._isUtxoBinding(prop)) PENDING._unbind = true
  ...
  _checkChangable (method, prop) {
    ...
    if (RECORD()._unbound.includes(this._creation)) throw new Error(`Cannot ${method} ${prop}: unbound`)
  }
  ...
  _inside () {
    const s = STACK()
    _sudo(() => console.log('STACK', s))
    return s.length && s[s.length - 1]._creation === this._creation
  }
