...
class Membrane {
  ...
  _apply (target, thisArg, args) {
    ...
    return this._captureErrors(() => {
      ...
      const disabledError = `${target.name} disabled`
      _checkState(!membrane._rules._disabledMethods.includes(target.name), disabledError)
  ...
  _get (target, prop, receiver) {
    ...
    return this._captureErrors(() => {
      ...
      if (this._isJigMethod(prop)) return Reflect.get(target, prop, receiver)
