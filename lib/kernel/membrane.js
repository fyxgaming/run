...
const { _assert, _checkState, _hasOwnProperty, _extendsFrom, _setOwnProperty } = require('../util/misc')
...
class Membrane {
  ...
  constructor (target, jig = null) {
    // If this object already has a membrane, return it
  ...
  static _configure (proxy) {
    const Jig = require('./jig')
    const Berry = require('./berry')

    // Get the membrane and parent membrane
    const m = Proxy2._getHandler(proxy)
    _assert(m)
    const pm = Proxy2._getHandler(m._jig)
    _assert(pm)

    // Detect if this is an top-level jig target
    const topLevel = m._jig === m._proxy
    const target = Proxy2._getTarget(proxy)

    // Jig code: normal, static, and native
    if (typeof target === 'function' && topLevel) {
      const isStatic = !_extendsFrom(target, Jig)

      const isNative =
        _hasOwnProperty(target, 'location') &&
        target.location &&
        target.location.startsWith('native://')

      m._admins = true
      m._errors = true
      m._codeMethods = true
      m._bindings = true
      m._private = !isStatic && !isNative
      m._immutable = isStatic || isNative
      m._recording = true
      return
    }

    // Jig instances
    if (typeof target === 'object' && topLevel && _extendsFrom(target.constructor, Jig)) {
      m._admins = true
      m._errors = true
      m._bindings = true
      m._private = true
    ...
    // Berry instances
    if (typeof target === 'object' && topLevel && _extendsFrom(target.constructor, Berry)) {
      m._admins = true
      m._errors = true
      m._immutable = true
      m._recording = true
    ...
    // Inner objects - inherit properties
    if (typeof target === 'object' && !topLevel) {
      m._admins = true
      m._errors = true
      m._private = pm._private
      m._immutable = pm._immutable
      m._recording = true
      return
    }

    // Inner methods - inherit properties
    if (typeof target === 'function' && !topLevel) {
      m._admins = true
      m._errors = true
      m._private = pm._private
  ...
  _apply (target, thisArg, args) {
    if (this._admins && _admin()) return Reflect.apply(target, thisArg, args)
  ...
  _construct (target, args, newTarget) {
    if (this._admins && _admin()) return Reflect.construct(target, args, newTarget)
    this._throwIfJigErrors()
  ...
  _defineProperty (target, prop, desc) {
    if (this._admins && _admin()) return Reflect.defineProperty(target, prop, desc)
    this._throwIfJigErrors()
    ...
    if (this._bindings && _BINDINGS.includes(prop) && !(desc.value instanceof Unbound)) {
      desc.value = new Unbound(desc.value)
    }

    // Enforce immutability
    if (this._immutable) throw new Error('defineProperty disabled')
  ...
  _deleteProperty (target, prop) {
    if (this._admins && _admin()) return Reflect.deleteProperty(target, prop)
    this._throwIfJigErrors()
  ...
  _get (target, prop, receiver) {
    if (this._admins && _admin()) return Reflect.get(target, prop, receiver)
    this._throwIfJigErrors()

    // Function prototypes must be returned directly. No changes. Always available.
    if (typeof target === 'function' && prop === 'prototype') {
      return Reflect.get(target, prop, receiver)
    }

    // A parent jig may be trying to override a non-jig child class's gets
    ...
    // Add the membrane if this is an object type
    if (typeof value === 'object' || typeof value === 'function') {
      value = new Membrane(value, this._jig)
    }
  ...
  _getOwnPropertyDescriptor (target, prop) {
    if (this._admins && _admin()) return Reflect.getOwnPropertyDescriptor(target, prop)
    this._throwIfJigErrors()
    ...
    if (typeof desc.value === 'object' || typeof desc.value === 'function') {
      desc.value = new Membrane(desc.value, this._jig)
    }
  ...
  _getPrototypeOf (target) {
    if (this._admins && _admin()) return Reflect.getPrototypeOf(target)
    this._throwIfJigErrors()
  ...
  _has (target, prop) {
    if (this._admins && _admin()) return Reflect.has(target, prop)
    this._throwIfJigErrors()
  ...
  _isExtensible (target) {
    if (this._admins && _admin()) return Reflect.isExtensible(target)
    this._throwIfJigErrors()
  ...
  _ownKeys (target) {
    if (this._admins && _admin()) return Reflect.ownKeys(target)
    this._throwIfJigErrors()
  ...
  _preventExtensions (target) {
    if (this._admins && _admin()) return Reflect.preventExtensions(target)
    this._throwIfJigErrors()
  ...
  _set (target, prop, value, receiver) {
    if (this._admins && _admin()) return Reflect.set(target, prop, value, receiver)
    this._throwIfJigErrors()
    ...
    // When bindings are set, they are unbound until the record is committed
    const Unbound = require('../util/unbound')
    if (this._bindings && _BINDINGS.includes(prop) && !(value instanceof Unbound)) {
    ...
    // Reflect.set will call defineProperty
  ...
  _setPrototypeOf (target, prototype) {
    if (this._admins && _admin()) return Reflect.setPrototypeOf(target, prototype)
    this._throwIfJigErrors()
  ...
  _intrinsicOut (value) {
    if (this._admins && _admin()) return value
    this._throwIfJigErrors()
