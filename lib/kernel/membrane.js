...
class MethodHandler {
  ...
  apply (target, thisArg, args) {
    ...
    _assert(thisArg instanceof Jig || thisArg instanceof Code, 'Jig methods must only be called on jigs')
    // if (thisArg !== this._jig) throw new Error('Jig methods must only be applied to themselves')

    // Check that thisArg can call this method. How? That that target is in thisArg's chain
    // Check that its in the prototype
...
class Membrane {
  ...
  get (target, prop, receiver) {
    ...
    if (typeof val === 'function') {
      ...
      const methodHandler = new MethodHandler(prop)
