...
class Membrane {
  ...
  _get (target, prop, receiver) {
    ...
    return this._trap(() => {
      ...
      if (parentJig && !PENDING_MEMBRANES.has(value) && this._isObjectType(value)) {
  ...
  _getOwnPropertyDescriptor (target, prop) {
    ...
    return this._trap(() => {
      ...
      if (this._isObjectType(desc.value) && !PENDING_MEMBRANES.has(desc.value)) {
        if (CHILD_MEMBRANES.has(desc.value)) return CHILD_MEMBRANES.get(desc.value)
        const method = typeof desc.value === 'function'
        const rules = Rules._childProperty(this._jig, method)
        const membrane = new Membrane(desc.value, rules)
        CHILD_MEMBRANES.set(desc.value, membrane)
        desc.value = membrane
      }
  ...
  _claimOwnership (value) {
    ...
    // If there is a membrane, we can only claim it if it is COW
    const membrane = Proxy2._getHandler(value)
    if (membrane) {
      _assert(membrane._jig === membrane._parentJig)
      _assert(membrane._cow)
