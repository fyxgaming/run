...
async function _capture (jig, commit, timeout) {
  ...
  let beforeRefmap = {}
  if (!_hasJig(commit._creates, jig)) {
    const beforeLocation = commit._before.get(jig)._props.location
    beforeRefmap = await refmap(beforeLocation, commit._kernel, timeout)
  }
