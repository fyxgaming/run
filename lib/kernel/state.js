...
const {
  _kernel, _assert, _parentName, _sameCreation, _text, _setOwnProperty,
  _deterministicJSONStringify, _defined
} = require('../util/misc')
...
const { _JIG_OBJECTS, _BERRIES } = require('./creation')
...
async function _captureJig (jig, commit, timeout) {
  ...
  const codec = new Codec()._saveJigs(x => {
    const vout = commit._outputs.findIndex(y => _sameCreation(x, y))
    if (vout >= 0) return `_o${vout + 1}`

    const vdel = commit._deletes.findIndex(y => _sameCreation(x, y))
    if (vdel >= 0) return `_d${vdel}`

    const ref = commit._refs.find(y => _sameCreation(x, y))
  ...
  const vout = commit._outputs.findIndex(x => _sameCreation(x, jig))
  const vdel = commit._deletes.findIndex(x => _sameCreation(x, jig))
  const localLocation = vout === -1 ? `_d${vdel}` : `_o${vout + 1}`
  _assert(!props.origin.startsWith('commit://') || props.origin.startsWith(`commit://${commit._id}`))
  if (props.origin.startsWith(`commit://${commit._id}`) || props.origin.startsWith(commit._txid)) props.origin = localLocation
...
function _captureBerry (berry, version) {
  ...
  const codec = new Codec()._saveJigs(x => {
    ...
    _assert(_defined(loc.txid) && !_defined(loc.record) && !_defined(loc.commit) && !_defined(loc.error))
    return xLocation
  })
...
async function _recreate (key, hash, loader) {
  ...
  // If the state is code, make sure it is trusted
  const kernel = loader._kernel
  const txid = _location(key.split('jig://')[1]).txid
  if (state.kind === 'code' && !kernel._trusts.has(txid) && !kernel._trusts.has('*')) {
  ...
  // If we manually trust this state, trust all of its references too
  if (kernel._trusts.has(txid)) {
    for (const ref of refs.keys()) {
      const reftxid = _location(ref).txid
      if (reftxid) kernel._trusts.add(reftxid)
    }
  }
...
async function _recreateCode (state, decodedProps, refs, txid, loader) {
  ...
  if (parentName) {
    ...
    const parentFullLocation = _compileLocation(Object.assign({ txid }, _location(parentLocation)))
  ...
  const complete = async () => {
    ...
    for (const ref of refs.keys()) {
      ...
      const fullLocation = _compileLocation(Object.assign({ txid }, _location(ref)))
    ...
    function jigLoader (x) {
      ...
      const fullLocation = _compileLocation(Object.assign({ txid }, _location(x)))
    ...
    _sudo(() => {
      C.location = _compileLocation(Object.assign({ txid }, _location(C.location)))
      C.origin = _compileLocation(Object.assign({ txid }, _location(C.origin)))
    })
...
async function _recreateJig (state, decodedState, refs, txid, loader) {
  ...
  async function complete () {
    ...
    _sudo(() => {
      jig.location = _compileLocation(Object.assign({ txid }, _location(jig.location)))
      jig.origin = _compileLocation(Object.assign({ txid }, _location(jig.origin)))
    })
...
async function _recreateBerry (state, decodedState, refs, hash, loader) {
  ...
  const rules = Rules._berryObject()
  ...
  async function complete () {
    ...
    for (const ref of refs.keys()) {
      if (refs.get(ref)) continue
      const fullLocation = _compileLocation(Object.assign({ hash }, _location(ref)))
      const jig = await loader._load(fullLocation, undefined, true)
      refs.set(ref, jig)
    }
    ...
    function jigLoader (x) {
      const jig = refs.get(x)
      const fullLocation = _compileLocation(Object.assign({ hash }, _location(x)))
      if (!jig) throw new StateError(`Jig not loaded: ${fullLocation}`)
      return jig
    }
    ...
    _sudo(() => {
      berry.location = _compileLocation(Object.assign({ hash }, _location(berry.location)))
      berry.origin = _compileLocation(Object.assign({ hash }, _location(berry.origin)))
    })
