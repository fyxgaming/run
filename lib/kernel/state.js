...
function _captureBerry (berry) {
  ...
  // props
  const props = Object.assign({}, berry)
  state.props = codec._encode(props)
...
async function _recreate (key, hash, loader) {
  ...
  if (state.version !== STATE_VERSION) throw new StateError(`Unsupported state version: ${state.version}`)
  ...
  if (hash) {
    ...
    if (stateHashHex !== hash) throw new StateError(`State hash mismatch for ${key}`)
  }
  ...
  switch (state.kind) {
    ...
    case 'berry': return _recreateBerry(state, decodedProps, refs, loader)
...
async function _recreateCode (state, decodedProps, refs, txid, loader) {
  ...
  const complete = async () => {
    ...
    function jigLoader (x) {
      ...
      if (!jig) throw new StateError(`Jig not loaded: ${refFullLocation}`)
      return jig
    }
...
async function _recreateBerry (state, decodedState, refs, loader) {
  ...
  async function complete () {
    ...
    for (const ref of refs.keys()) {
      if (refs.get(ref)) continue
      const jig = await loader._load(ref, undefined, true)
      refs.set(ref, jig)
    }
    ...
    function jigLoader (x) {
      const jig = refs.get(x)
      if (!jig) throw new StateError(`Jig not loaded: ${x}`)
      return jig
    }
