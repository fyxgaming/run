...
async function _captureJig (jig, commit, timeout) {
  ...
  const codec = new Codec()._saveJigs(x => {
    const vout = record._outputs._indexOf(x)
    if (vout >= 0) return `_o${vout + 1}`

    const vdel = record._deletes._indexOf(x)
  ...
  const vout = record._outputs._indexOf(jig)
  const vdel = record._deletes._indexOf(jig)
