...
async function _capture (jig, commit, timeout) {
  ...
  const beforeRefmap = await commit._getBeforeRefmap(timeout)

  // Generate a refmap that contains relative origins and locations
  const localRefmap = {}
  ...
  const codec = new Codec()._saveJigs(x => {
    ...
    if (vout >= 0) {
      const localLocation = `_o${vout + 1}`
      const localOrigin = origin.startsWith('commit://') ? localLocation : origin
      const after = commit._after.get(commit._outputs[vout])
      localRefmap[localOrigin] = [localLocation, after._props.nonce]
    ...
    if (vdel >= 0) {
      const localLocation = `_d${vdel}`
      const localOrigin = origin.startsWith('commit://') ? localLocation : origin
      const after = commit._after.get(commit._deletes[vdel])
      localRefmap[localOrigin] = [localLocation, after._props.nonce]
      return localLocation
    }
    ...
    if (ref) {
      ...
      localRefmap[before._props.origin] = [before._props.location, before._props.nonce]
      return before._props.location
    }

    // Native
    if (origin.startsWith('native://')) {
      localRefmap[origin] = [origin, 0]
      return origin
    }

    // Unused
    const beforeRefLocation = beforeRefmap[origin] && beforeRefmap[origin][0]
    _assert(beforeRefLocation)
    localRefmap[origin] = beforeRefmap[origin]
  ...
  // Save the local location now that we know it for this jig
  localRefmap[props.origin] = [props.location, props.nonce]

  return [state, localRefmap]
}
