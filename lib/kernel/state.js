...
const { _kernel, _assert, _parentName, _sameJig, _hasJig, _checkState, _text, _setOwnProperty, _safeJSONStringify } = require('../util/misc')
...
async function _capture (jig, commit, timeout) {
  ...
  // Create the state data to encode. The order here is part of the protocol. It goes:
  //
  //    [version, kind, props, cls, src]
  //
  // and props always has its first bindings as [location, origin, nonce, owner, satoshis]
  ...
  state.props = {}
  let keys = Object.getOwnPropertyNames(after._props)
  keys = keys.filter(key => !_BINDINGS.includes(key))
  keys.unshift(..._BINDINGS)
  keys.forEach(key => { state.props[key] = after._props[key] })
...
async function _recreate (key, hash, loader) {
  ...
  if (hash) {
    const stateString = _safeJSONStringify(state)
