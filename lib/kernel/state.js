...
const {
  _assert, _parentName, _text, _setOwnProperty,
...
async function _recreateCode (state, decodedProps, refs, txid, kernel, session, timeout) {
  ...
  if (parentName) {
    ...
    const Parent = await _load(parentFullLocation, undefined, kernel, session, timeout, true)
  ...
  const complete = async () => {
    ...
    for (const ref of refs.keys()) {
      ...
      const _load = require('./load')
      const jig = await _load(fullLocation, undefined, kernel, session, timeout, true)
      refs.set(ref, jig)
    }
