...
async function _captureJig (jig, commit, timeout) {
  ...
  if (props.origin.startsWith(`record://${record._id}`) || props.origin.startsWith(commit._txid)) props.origin = localLocation
...
function _captureBerry (berry, version) {
  ...
  const codec = new Codec()._saveJigs(x => {
    ...
    _assert(_defined(loc.txid) && !_defined(loc.record) && !_defined(loc.error))
    return xLocation
  })
