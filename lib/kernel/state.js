...
const { _kernel, _assert, _parentName, _sameJig, _hasJig, _checkState, _text, _setOwnProperty } = require('../util/misc')
...
async function _recreate (key, hash, loader) {
  ...
  switch (state.kind) {
    ...
    case 'jig': return _recreateJig(decodedProps, txid)
...
async function _recreateCode (state, decodedProps, refs, txid, loader) {
  ...
  const complete = async () => {
    ...
    // TODO: SGlobal is not valid after my changes!! Because it is not the sandbox.
    // Would be nice not to have to evaluate twice. Meaning here, we would create
    // the sandboxed code.
    Object.assign(SGlobal, redecodedProps.deps)

    // Apply the props to the code
    _sudo(() => {
      Object.keys(redecodedProps).forEach(key => {
        _setOwnProperty(C, key, redecodedProps[key])
      })
...
async function _capture (jig, commit) {
  ...
  // Create the state data to encode
  // The order here is part of the protocol. It goes: [version, kind, props, cls, src]
  const state = {}
  ...
  state.props = codec._encode(state.props)
  if (state.cls) state.cls = codec._encode(state.cls)
...
async function createReferenceMap (jig, commit) {
  ...
  _deepVisit(prev, x => {
    if (x instanceof Universal) {
      _sudo(() => map.set(x.origin || x.location, x.location))
    }
  })
