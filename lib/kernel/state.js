...
async function _recreate (key, hash, loader) {
  ...
  const origin = decodedProps.origin.startsWith('_') ? txid + decodedProps.origin : decodedProps.origin
  const origintxid = origin.slice(0, 64)
  if (state.kind === 'code' && !kernel._trustlist.has(txid) && !kernel._trustlist.has('*') && !kernel._trustlist.has(origintxid)) {
    const hint = 'Hint: Trust this txid using run.trust if you know it is safe'
    throw new Error(`Cannot load untrusted code from cache: ${txid}\n\n${hint}`)
  }
  ...
  if (kernel._trustlist.has(txid)) {
    for (const ref of refs.keys()) {
      const reftxid = _location(ref).txid
      if (reftxid) kernel._trustlist.add(reftxid)
    }
  }
