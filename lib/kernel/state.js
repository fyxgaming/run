...
async function _captureJig (jig, commit, timeout) {
  ...
  const codec = new Codec()._saveJigs(x => {
    const vout = record._outputs.findIndex(y => _sameCreation(x, y))
    if (vout >= 0) return `_o${vout + 1}`

    const vdel = record._deletes.findIndex(y => _sameCreation(x, y))
    if (vdel >= 0) return `_d${vdel}`

    const ref = record._refs.find(y => _sameCreation(x, y))
  ...
  const vout = record._outputs.findIndex(x => _sameCreation(x, jig))
  const vdel = record._deletes.findIndex(x => _sameCreation(x, jig))
