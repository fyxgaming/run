...
const {
  ...
  _setOwnProperty, _sandboxSourceCode
} = require('../util/misc')
...
class File {
  ...
  _upgrade (T, local = true) {
    ...
    try {
      ...
      const { _record, _recordMultiple } = require('./commit')
  ...
  static _deployMultiple (...jigs) {
    ...
    // const { _record } = require('./commit')
    // const Command = require('./command')
    const { _recordDeploy } = require('./action')
    _recordDeploy(Array.from(allJigsToDeploy))
