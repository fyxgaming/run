...
const {
  ...
  _setOwnProperty, _sandboxSourceCode, _checkState
} = require('../util/misc')
...
class File {
  ...
  _upgrade (T, local = true) {
    ...
    _checkState(wasClass === willBeClass, badUpgradeError)
    ...
    _checkState(!(T instanceof Code), 'Cannot upgrade to a code jig')
...
function checkInstallable (T) {
  _checkState(typeof T === 'function', 'Only functions and classes are supported')
  _checkState(T.name, 'Anonymous functions and classes cannot be jigs')
  _checkState(!T.prototype || T.prototype.constructor === T, 'Prototypal inheritance not supported')
  _checkState(T.toString().indexOf('[native code]') === -1, 'Cannot install native code')
}
...
function checkDeps (T, ParentCode) {
  ...
  if (_hasOwnProperty(T, 'deps')) {
    _checkState(_isBasicObject(T.deps), 'deps must be an object')
    if (ParentCode) {
      const DepParent = T.deps[ParentCode.name]
      const DepParentFile = File._find(DepParent)
      _checkState(DepParentFile && DepParentFile._jig === DepParent, 'Parent dependency mismatch')
    }
  }
}
...
function checkPresets (T) {
  ...
  if (_hasOwnProperty(T, 'presets')) {
    ...
    _checkState(_isBasicObject(presets), 'presets must be an object')

    for (const network of Object.keys(presets)) {
      const npresets = presets[network]
      _checkState(_isBasicObject(npresets), `${network} presets must be an object`)
      ...
      _checkState(!anyBindings || allBindings, `${network} presets are not fully defined`)
      ...
      _checkState(!('deps' in npresets), `${network} presets must not contain deps`)
      _checkState(!('presets' in npresets), `${network} presets must not contain presets`)
...
function checkOptions (T) {
  ...
  if (_hasOwnProperty(T, 'sealed')) {
    ...
    _checkState(T.sealed === true || T.sealed === false || T.sealed === 'owner', badSealed)
  }
}
...
function checkBindings (T) {
  ...
  _checkState(!badBinding, `Must not have any bindings: ${badBinding}`)
}
...
function checkForReservedCodeWords (props) {
  ...
  _checkState(!badWord, `Must not have any reserved words: ${badWord}`)
}
