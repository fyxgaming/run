...
const {
  _kernel, _assert, _text, _parent, _isBasicObject, _isUndefined, _hasOwnProperty, _sourceCode
} = require('../util/misc')
...
const DIRECTORY = new Map() // Network -> WeakMap<Local|Code, File>
...
class File {
  ...
  constructor (T = undefined) {
    // Save our network. Each file is tied to one, although locals can be shared.
    this._network = _kernel()._blockchain.network

    // Check if this file was created and not internal
    const prev = DIRECTORY[this._network].get(T)
    ...
    // Our code is not native nor internal by default
    this._native = false
  ...
  /**
   * Installs a new local type onto the file. Installs all dependencies as needed too.
   */
  _installLocal (T) {
    ...
    try {
      DIRECTORY[this._network].delete(oldLocal)
      DIRECTORY[this._network].set(this._local, this)
      ...
      DIRECTORY[this._network].delete(T)
      DIRECTORY[this._network].set(oldLocal, this)
    }
  }
  ...
  _setInnerType (T) {
    ...
    if (!_parent(T)) Object.setPrototypeOf(T, Code.prototype)
    ...
    Membrane._sudo(() => {
      for (const [T] of repository._localToCode) {
        Bindings._BINDINGS.forEach(x => {
          _setOwnProperty(T, x, preset(T, x, network))
        })
      }
    })
  }
