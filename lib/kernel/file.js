...
// Set of installed local code. Used to change presets on activate().
...
class File {
   ...
   */
  constructor (T = undefined, local = true) {
  ...
  _installLocal (T, local = true) {
    ...
    try {
      ...
      const ParentCode = Parent && new File(Parent, local)._jig
      ...
      const makeCode = x => typeof x === 'function' ? new File(x, local)._jig : undefined
  ...
  _upgrade (T, local = true) {
    ...
    try {
      ...
      _recordMultiple(() => {
        ...
        // Upgrade the class
        _record(record => Command._upgrade(record, this._jig, snapshot))
      })
