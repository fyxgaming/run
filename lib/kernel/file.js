...
class File {
   ...
   */
  constructor (T = undefined, customDirectory = undefined) {
    // Check if this file was created and not internal
    try {
      const network = _kernel()._blockchain.network
      const directory = customDirectory || DIRECTORY[network]
   ...
   */
  _installLocal (T, customDirectory = undefined) {
    Log._info(TAG, 'Install', _text(T))

    _assert(!this._local, 'Code cannot be installed twice')

    // Check if T is valid
    _assert(typeof T === 'function', 'Only functions and classes are supported')
    ...
    const directory = customDirectory || DIRECTORY[this._network]
    ...
    try {
      ...
      if (!customDirectory) LOCALS.add(this._local)

      // Create the parent first
      const Parent = _parent(T)
      const ParentCode = Parent && new File(Parent, customDirectory)._jig
      ...
      if (_hasOwnProperty(T, 'presets')) {
        ...
        for (const network of Object.keys(presets)) {
          ...
          checkForReservedCodeWords(Object.getOwnPropertyNames(npresets))
        }
      }
      ...
      // Check for reserved words
      checkForReservedCodeWords(propNames)
      ...
      const makeCode = x => typeof x === 'function' ? new File(x, customDirectory)._jig : undefined
  ...
  static _deployMultiple (...jigs) {
    const allJigsToDeploy = new Set()

    // Find all dependent jigs to deploy
    jigs.forEach(jig => {
      const file = File._find(jig)
      const jigs2 = whatNeedsToBeDeployed(file._jig)
      jigs2.forEach(jig2 => allJigsToDeploy.add(jig2))
    })

    // Check if there is anything to deploy
    if (!allJigsToDeploy.size) return

    const { _record } = require('./record')
    const Command = require('./command')
    _record(record => Command._deploy(record, ...Array.from(allJigsToDeploy)))
  }
...
function checkForReservedCodeWords (propNames) {
  const Code = require('./code')
