...
class Publisher {
  constructor (kernel) {
    ...
    this._onReadyListeners = [] // callbacks for when sync completes (Array<{resolve,reject}>)
  ...
  async _publishNext () {
    ...
    this._onReadyListeners.forEach(c => c.resolve())
  ...
  async _buildAndBroadcast (record) {
    ...
    if (!record._paid) await record.pay(this._kernel)
    await record.sign(this._kernel)
  ...
  _rollbackAndNotifyListenersAfterPublishError (e) {
    ...
    const unhandled = this._onReadyListeners.length === 0
    ...
    this._onReadyListeners.forEach(c => c.reject(err))
