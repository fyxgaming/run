  ...
  async _buildAndBroadcast (record) {
    ...
    try {
      refSpends = await Promise.all(refs.map(ref => {
        ...
        return this._kernel._blockchain.spends(txid, vout)
      }))
    } catch (e) {
    ...
    if (typeof this._kernel._purse.broadcast === 'function') {
      try {
        await this._kernel._purse.broadcast(tx.toString('hex'))
      } catch (e) {
    ...
    try {
      await this._kernel._blockchain.broadcast(tx.toString('hex'))
    } catch (e) {
    ...
    if (eString.indexOf('Missing inputs') !== -1 || eString.indexOf('txn-mempool-conflict') !== -1) {
      ...
      for (const input of tx.inputs) {
        try {
          ...
          if (prevspend) {
            const prevraw = await this._kernel._blockchain.fetch(prevtxid)
