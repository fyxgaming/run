...
class Publisher {
  ...
  async _publishNext () {
    ...
    await this._addToCache(record, tx)
    this._updateInventoryAfterPublish(record)
  ...
  async _buildAndBroadcast (record) {
    ...
    const purse = this._kernel._purse
    ...
    // check that each read references are plausibly the latest
    let refSpends = null
    try {
      refSpends = await Promise.all(refs.map(ref => {
        const txid = ref.slice(0, 64)
        const vout = parseInt(ref.slice(66))
        return blockchain.spends(txid, vout)
      }))
    } catch (e) {
      throw new Error(`Aborting broadcast. A referenced jig may not be the latest.\n\n${e}`)
    }
    for (let i = 0; i < refSpends.length; i++) {
      if (refSpends[i]) {
        throw new Error(`Read ${refs[i]} is not the latest. Must sync() jigs`)
      }
    }
  ...
  async _addToCache (record, tx) {
    ...
    for (const jig of record._outputs) {
      ...
      const restoredLocation = 'jig://' + restored.location
      ...
      // the cached state of the jig. For now, I would just deserialize and serialize again.
      ...
      await this._kernel._cache.set(restoredLocation, cachedState)
    }
  }

  _updateInventoryAfterPublish (record) {
  ...
  _rollbackAndNotifyListenersAfterPublishError (e) {
    ...
    if (rollbackErrors.length) {
      e.message += '\n\nThese errors also occurred while rolling back:'
      rollbackErrors.forEach(e => { e.message += '\n\n' + e.toString() })
    }

    // Unblock each listener that is waiting for sync() with the error.
    if (unhandled) Log._error(TAG, 'Unhandled', e)
    this._readyPromises.forEach(promise => promise.reject(e))
    this._readyPromises = []
