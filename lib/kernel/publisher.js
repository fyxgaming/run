...
class Publisher {
  ...
  async _publish () {
    try {
      ...
      const states = await calculateStates(this._commit, this._kernel, this._importLimit)
...
async function calculateStates (commit, kernel, importLimit) {
  const states = new Map()
  const jigs = commit._outputs.concat(commit._deletes)

  for (const jig of jigs) {
    const state = await calculateState(commit, jig, kernel, importLimit)
...
async function calculateState (commit, jig, kernel, importLimit) {
  ...
  const referenceMap = await createReferenceMap(commit, jig, kernel, importLimit)
...
async function createReferenceMap (commit, jig, kernel, importLimit) {
