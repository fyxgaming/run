...
const { _checkState, _deterministicJSONStringify } = require('../util/misc')
...
async function verify (commit, tx, txid, txpayload, published, timeout) {
  ...
  if (_deterministicJSONStringify(payload) !== _deterministicJSONStringify(txpayload)) {
    _throwPayloadMismatchError(txpayload, payload, commit, states)
  }
...
function _throwPayloadMismatchError (expected, actual, commit, states) {
  ...
  function detectBadState (expectedHash, actualHash, jig) {
    if (expectedHash === actualHash) return

    const state = states.get(jig)
    if (Log._errorOn) Log._error(TAG, 'State mismatch:', JSON.stringify(state, 0, 3))
  }

  if (expected.out.length === actual.out.length && expected.del.length === actual.del.length) {
    expected.out.forEach((expectedHash, n) => {
      detectBadState(expectedHash, actual.out[n], commit._outputs[n])
    })
    expected.del.forEach((expectedHash, n) => {
      detectBadState(expectedHash, actual.del[n], commit._deletes[n])
    })
  }
