...
const {
  ...
  _createInstructions, _createPayload, _createPartialTx, _finalizeLocationsAndOrigins, _cacheStates
} = require('./publish')
...
async function verify (commit, tx, txpayload, published) {
  ...
  // Compare payload
  if (JSON.stringify(payload) !== JSON.stringify(txpayload)) {
...
function _throwPayloadMismatchError (expected, actual) {
  Log._error(TAG, 'Expected payload:', JSON.stringify(expected, 0, 3))
  Log._error(TAG, 'Actual payload:', JSON.stringify(actual, 0, 3))
