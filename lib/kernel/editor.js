...
const {
  _text, _kernel, _parent, _isAnonymous, _isUndefined, _sameJig,
  _isBasicObject, _hasOwnProperty, _setOwnProperty, _assert, _extendsFrom,
  _deanonymizeSourceCode, _RESERVED_PROPS, _RESERVED_CODE_METHODS, _RESERVED_JIG_METHODS
} = require('../util/misc')
...
const { _CREATION_BINDINGS, _BINDINGS, _location, _nonce, _owner, _satoshis } = Bindings
...
class Editor {
  ...
  _install (T, local = true, newCode = [], src) {
    ...
    try {
      ...
      // If there were presets, we have a couple other things to do because we won't deploy
      if (_hasOwnProperty(T, 'presets')) {
        // First, make sure all jigs referenced by this jig are also deployed
        _deepVisit(this._C, x => {
          if (x instanceof Code && x !== this._C) {
            if (_location(x.location).undeployed) {
   ...
   */
  _uninstall () {
    if (this._native) throw new StateError('Cannot uninstall native code')
  ...
  _setupBehavior (T, local = false, newCode = []) {
    ...
    if (this._installed) {
      ...
      if (beforeJig !== afterJig) throw new StateError('Cannot change staticness of code in upgrade')
    }

    // Make sure we do not allow berries to be upgraded from or to
    if (this._installed && _extendsFrom(this._T, Berry)) {
      throw new StateError(`Cannot upgrade from berry class: ${_text(this._T)}`)
    }
    if (this._installed && _extendsFrom(T, Berry)) {
      throw new StateError(`Cannot upgrade to berry class: ${_text(T)}`)
    }
  ...
  _installNative (T, internal = false) {
    ...
    if (_hasOwnProperty(S, 'upgradable')) _setOwnProperty(S, 'upgradable', upgradable)
  ...
  _deploy () {
    ...
    if (this._native) throw new StateError('Cannot deploy native code')
...
/**
   ...
   */
function _newCode (T, local = true, newCode = []) {
  // Check if T is already installed as code
  const prevCode = lookupCodeByType(T)
...
function checkType (T) {
  ...
  if (_isAnonymous(T)) throw new ArgumentError(`Anonymous types not supported: ${_text(T)}`)
  if (T === Code) throw new ArgumentError('The Code class itself cannot be used in code')
...
function checkDeps (T, ParentCode) {
  ...
  if (!_isBasicObject(T.deps)) throw new ArgumentError('deps must be a basic object')
  ...
  if (ParentCode) {
    ...
    if (DepParent && !_sameJig(DepParentCode, ParentCode)) throw new ArgumentError('Parent dependency mismatch')
  }
...
function checkPresets (T) {
  ...
  if (!_isBasicObject(presets)) throw new ArgumentError('presets must be a basic object')

  for (const network of Object.keys(presets)) {
    const npresets = presets[network]
    if (!_isBasicObject(npresets)) throw new ArgumentError(`Presets for ${network} network must be an object`)
    ...
    if (anyBindings && !allBindings) throw new ArgumentError(`${network} presets not fully defined`)

    // Check that the preset bindings are valid if they exist
    if (anyBindings) {
      const loc = _location(npresets.location)
      if (!(loc.txid && ('vout' in loc || 'vdel' in loc) && !('berry' in loc))) {
        throw new ArgumentError('Bad location')
      }
      const orig = _location(npresets.origin)
      if (!(orig.txid && ('vout' in orig || 'vdel' in orig) && !('berry' in orig))) {
        throw new ArgumentError('Bad origin')
      }
...
function checkNoReservedWords (props) {
  ...
  const badWord = _RESERVED_PROPS.find(word => propNames.includes(word)) ||
   _RESERVED_CODE_METHODS.find(word => propNames.includes(word))
...
function checkValidJigClass (T) {
  ...
  const badWord = _RESERVED_PROPS.find(word => propNames.includes(word)) ||
   _RESERVED_JIG_METHODS.find(word => propNames.includes(word)) ||
...
function checkValidBerryClass (T) {
  ...
  const propNames = Object.getOwnPropertyNames(T.prototype)
  const badWord = _RESERVED_PROPS.find(word => propNames.includes(word)) ||
    _CREATION_BINDINGS.find(word => propNames.includes(word))
...
function checkNoSymbolMethods (T) {
  _sudo(() => {
    if (Object.getOwnPropertySymbols(T).length ||
      ...
      throw new StateError('Symbol methods not supported')
    }
  })
}
...
function checkNoAccessors (T) {
  const check = desc => {
    if ('get' in desc || 'set' in desc) {
      throw new StateError('Getters and setters not supported')
    }
  }
...
function checkUpgradable (T, editor) {
  ...
  if (editor._native) throw new StateError('Cannot upgrade native code')
  ...
  if (_hasOwnProperty(T, 'presets')) {
    ...
    const checkNoPresets = x => {
      if (x in npresets) {
        throw new StateError('Preset bindings not supported for upgrades')
      }
    }
  ...
  if (origin === Bindings._UNDEPLOYED_LOCATION) throw new StateError('Cannot upgrade undeployed code')
}
...
function makeSandbox (C, T, local = false, newCode = undefined) {
  ...
  const ParentCode = Parent && Editor._newCode(Parent, local, newCode)
  if (ParentCode) Editor._get(ParentCode)._postinstall()
  ...
  const makeCode = x => typeof x === 'function' ? Editor._newCode(x, local, newCode) : undefined
...
function lookupCodeByNativeId (id) {
...
Editor._newCode = _newCode
Editor._deployMultiple = _deployMultiple
Editor._lookupCodeByType = lookupCodeByType
Editor._lookupCodeByNativeId = lookupCodeByNativeId
...
Editor._get = C => EDITOR.get(lookupCodeByType(C))
