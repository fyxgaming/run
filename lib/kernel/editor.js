...
const FUNCTION_REGEX = /^\s*function\s+[a-zA-Z0-9$_]+\s*\(.*}\s*$/ms
const CLASS_REGEX = /^\s*class\s+[a-zA-Z0-9$_]+\s*{.*}\s*$/ms
const CHILD_REGEX = /^\s*class\s+[a-zA-Z0-9$_]+\s+extends.*}\s*$/ms
const TYPE_REGEX = /(function\s+[a-zA-Z0-9$_]+\s*\()|(class\s+[a-zA-Z0-9$_]+\s*{)|(class\s+[a-zA-Z0-9$_]+\s+extends)/msg

function checkSourceCode (src) {
  if (!src.match(FUNCTION_REGEX) && !src.match(CLASS_REGEX) && !src.match(CHILD_REGEX)) {
    throw new Error(`Bad source code: ${src}`)
  }

  // Check that there are not multiple classes or functions
  let count = 0
  while (TYPE_REGEX.exec(src)) count++
  if (count > 1) throw new Error(`Multiple types not permitted in source code: ${src}`)
}
...
Editor._checkSourceCode = checkSourceCode
