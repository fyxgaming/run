...
function checkPresets (T) {
  ...
  for (const network of Object.keys(presets)) {
    ...
    if (anyBindings) {
      ...
      if (!(loc.txid && (!_isUndefined(loc.vout) || !_isUndefined(loc.vdel)) && _isUndefined(loc.berry))) {
        throw new ArgumentError('Bad location')
      }
      const orig = _location(npresets.origin)
      if (!(orig.txid && (!_isUndefined(orig.vout) || !_isUndefined(orig.vdel)) && _isUndefined(orig.berry))) {
...
Editor._deployMultiple = deployMultiple
