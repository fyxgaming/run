...
const {
  _text, _kernel, _parent, _anonymous, _defined, _sameJig,
...
function checkPresets (T) {
  ...
  for (const network of Object.keys(presets)) {
    ...
    const allBindings = !_BINDINGS.some(prop => !_defined(npresets[prop]))
    ...
    if (anyBindings) {
      ...
      if (!(loc.txid && (_defined(loc.vout) || _defined(loc.vdel)) && _defined(loc.berry))) {
        throw new ArgumentError('Bad location')
      }
      const orig = _location(npresets.origin)
      if (!(orig.txid && (_defined(orig.vout) || _defined(orig.vdel)) && _defined(orig.berry))) {
