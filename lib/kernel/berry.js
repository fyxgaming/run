...
class Berry {
  ...
  constructor (...args) {
    ...
    const BERRIES = BerryDeps._Creation._BERRIES
    const NativeBerry = BerryDeps._NativeBerry
    const Membrane = BerryDeps._Membrane
    const claimBerry = BerryDeps._Loader._claimBerry
    ...
    // Claim the berry
    const partialLocation = claimBerry(this.constructor)

    // Assign the location which comes from the loader
    this.location = partialLocation
    this.origin = partialLocation
    this.nonce = 0
  ...
  static [Symbol.hasInstance] (x) {
    ...
    if (!BerryDeps._Creation._BERRIES.has(x)) return false
  ...
  static load (location) {
    const { _kernel, _text, _extendsFrom } = BerryDeps._misc
    const Loader = BerryDeps._Loader
    const NativeBerry = BerryDeps._NativeBerry
    ...
    const promise = loader._load(location, B)
