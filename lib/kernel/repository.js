...
function _install (T, options = {}) {
  ...
  // Delete the existing methods on the method table
  Object.getOwnPropertyNames(methodTable).forEach(method => { delete methodTable[method] })

  // Add the methods to the method table
  ...
  try {
    ...
    Membrane._sudo(() => {
      const dontDelete = ['length', 'prototype', 'name'].concat(Bindings._BINDINGS)
      Object.getOwnPropertyNames(C).filter(x => !dontDelete.includes(x)).forEach(x => { delete C[x] })
    ...
    if (upgrading) {
      ...
      _record(record => record._upgrade())
    }
