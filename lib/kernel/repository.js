...
function _install (T, options = {}) {
  ...
  console.log('PREV', PrevCode, presetLocation)
  ...
  */
  ...
  try {
    ...
    if (ParentCode) {
      ...
      console.log('----------------1222')
      console.log(T.name, Sprops, Parent.name)
      console.log('----------------1222')
    }
...
function _deploy (...Cs) {
  ...
  Cs.forEach(C => {
    ...
    console.log(C, Xs.size, C.location)
...
function _notify (jig) {
  ...
  if (!Object.getOwnPropertyNames(T).includes('presets')) {
    Object.defineProperty(T, 'presets', { enumerable: true, configurable: true, value: {} })
  }
  if (!Object.getOwnPropertyNames(T.presets).includes(network)) {
    Object.defineProperty(T.presets, network, { enumerable: true, configurable: true, value: {} })
  }

  // Set each binding
  Membrane._sudo(() => {
    Bindings._BINDINGS.forEach(binding => {
      T[binding] = jig[binding]
      T.presets[network][binding] = jig[binding]
    })
  })
}
...
module.exports = { _install, _installNative, _get, _isNative, _isCode, _deploy, _activate, _notify }
