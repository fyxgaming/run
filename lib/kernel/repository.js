...
function _install (T, options = {}) {
  ...
  class V { }
  const [U] = Sandbox._sandboxType(V, {})
  ...
  const C = new Sandbox._intrinsics.Proxy(U, membrane)
  membrane._init(S, C)

  const _src = _sourceCode(S)
  const metadata = { _src, _membrane: membrane }
  ...
  try {
    ...
    const Uprops = _deepClone(props, Sandbox._intrinsics, makeCode)

    if (ParentCode) {
      if (!Object.getOwnPropertyNames(Uprops).includes('deps')) {
        const value = new Sandbox._intrinsics.Object()
        Object.defineProperty(Uprops, 'deps', { enumerable: true, configurable: true, value })
      }
      Uprops.deps[Parent.name] = ParentCode
    }

    if (Object.getOwnPropertyNames(Uprops).includes('deps')) {
      Object.assign(SGlobal, Uprops.deps)
    }

    Membrane._sudo(() => {
      Object.assign(C, Uprops)
      delete C.presets
    })

    if (hasPresets(T, network)) {
      Membrane._sudo(() => Object.assign(C, Uprops.presets[network]))
    }
...
function _upgrade (C, T) {
  ...
  // TODO: The original class should be swapped out. Don't need it.
