...
function _install (T, options = {}) {
  ...
  const PrevCode = _get(T, repository)
  ...
  checkValid(T, ParentCode, false /* native */, repository)
...
function _destroy (C) {
  _assert(!_isNative(C), 'Native code may not be destroyed')
...
function _get (T, repository) {
  const Code = require('./code')
  if (T instanceof Code) return T
  repository = repository || ACTIVE_REPOSITORY()
 ...
 */
function checkValid (T, ParentCode, native, repository) {
  ...
  checkDeps(T, ParentCode, repository)
  checkPresets(T)
  checkOptions(T)
  checkBindings(T)
 ...
 */
function checkDeps (T, ParentCode, repository) {
  ...
  if (ParentCode) {
    ...
    const DepParentCode = _get(DepParent, repository)
