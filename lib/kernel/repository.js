...
const CODE_METADATA = new WeakMap() // C -> { _src, _T, _S, _repository, _membrane, _native }
...
class Repository {
  ...
  _install (T, options = {}) {
    ...
    try {
      ...
      this._localToCode.delete(OldS)
      this._localToCode.set(T, C)
      this._localToCode.set(S, C)
  ...
  _get (T) {
    ...
    if (T instanceof Code) {
      ...
      if (metadata._repository !== this && !metadata._native) return
      return T
    }
    return this._localToCode.get(T) || this._sandboxToCode.get(T)
  }
  ...
  _checkBindings (T) {
    ...
    for (const binding of Bindings._BINDINGS) {
  ...
  static _activate (network) {
    ...
    Membrane._sudo(() => {
      for (const [T] of repository._localToCode) {
        Bindings._BINDINGS.forEach(x => {
          _setOwnProperty(T, x, preset(x, T, network))
        })
      }
    })
  }
