...
class Loader {
  ...
  _loadNative (location) {
    ...
    const file = File._findNative(native)
    _assert(file, `Native code not found: ${location}`)

    // Make sure it's not internal
    _assert(!file._internal, `${_text(file._jig)} cannot be a dependency`)
  ...
  async _recreate (key, hash = undefined) {
    ...
    const { _PROTOCOL_VERSION } = require('./record')
    const version = Buffer.from(_PROTOCOL_VERSION).toString('hex')
    _assert(state.version === version, `Unsupported state version: ${state.version}`)
  ...
  async _recreateCode (decodedState, refs, txid, state) {
    ...
    const [S] = Sandbox._evaluate(decodedState.src, env)
    ...
    const complete = async () => {
      ...
      const [S2, S2Global] = Sandbox._evaluate(decodedState.src, env)
      Object.assign(S2Global, redecodedState.props.deps)
      file._setInnerType(S2)
