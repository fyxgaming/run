...
const { _assert, _extendsFrom, _text, _checkState, _checkArgument } = require('../util/misc')
...
class Loader {
  ...
  async _loadJigViaReplay (tx, payload, location) {
    ...
    const oldRecord = await _import(tx, payload, this._kernel, published, jigToSync, this._importLimit)

    // Assume empty record
    const record = await _replay(tx, payload)

    // Restore old record
    const commit = record._commit()

    await _verify(commit, tx, payload, published)

    await _verify(record, tx, payload, published)
  ...
  _loadNative (location) {
    ...
    const file = File._findNativeById(nativeid)
    _checkArgument(file, `Native code not found: ${nativeid}`)
