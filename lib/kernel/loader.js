...
class Loader {
  ...
  async _loadFromLocationWithBanCheck (location, loadFunction) {
    ...
    if (banned && bannedValue.untrusted && await this._kernel._trusted(bannedValue.untrusted)) {
      try {
    ...
    try {
      ...
      // Ban jigs that fail to load. We assume these are deterministic failures.
      try {
