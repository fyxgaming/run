...
const { ArgumentError, StateError, ClientModeError } = require('../util/errors')
...
class Loader {
  ...
  async _loadJigFromLocation (location) {
    ...
    const hasVout = typeof vout === 'number' && vout > payload.vrun && vout < payload.out.length + payload.vrun + 1
    ...
    // Get the state hash
    const hash = hasVout ? payload.out[vout - payload.vrun - 1] : payload.del[vdel]
  ...
  async _loadJigViaReplay (tx, payload, location) {
    ...
    if (typeof vout === 'number' && vout > payload.vrun && vout < record._outputs._size + payload.vrun + 1) {
      return record._outputs._arr()[vout - payload.vrun - 1]
    }
...
function _payload (tx) {
  ...
  if (!chunks) throw new StateError(BAD_PROTOCOL_ERROR)
  ...
  try {
    ...
    payload.base = base.toString('hex')
    payload.vrun = base.outputs.length
