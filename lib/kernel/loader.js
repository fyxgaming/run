...
let UNCLAIMED_BERRY_CLASS = null
let UNCLAIMED_BERRY_LOCATION = null
...
class Loader {
  ...
  async _loadBerryFromLocation (location) {
    ...
    const loc = _location(location)
    const classLocation = loc.txid + (_defined(loc.vout) ? `_o${loc.vout}` : `'_d${loc.vdel}`)
    const berry = loc.berry
    const hash = loc.hash
    const version = loc.version || _PROTOCOL_VERSION

    // Try loading from the cache first
    const partialLocation = _compileLocation({ txid: loc.txid, vdel: loc.vdel, vout: loc.vout, berry, version })
    const key = `jig://${partialLocation}`
  ...
  async _loadJigViaReplay (tx, payload, location) {
    ...
    record._outputs.forEach(jig => this._kernel._emit('load', jig))
    record._deletes.forEach(jig => this._kernel._emit('load', jig))
  ...
  async _loadWithUserBerryClass (path, BerryClass) {
    ...
    if (berryClassDeployed) {
      const opts = { berry: path, version: _PROTOCOL_VERSION }
  ...
  async _pluckBerry (path, hash, version, B) {
    ...
    const savedUnclaimedBerryClass = UNCLAIMED_BERRY_CLASS
    const savedUnclaimedBerryLocation = UNCLAIMED_BERRY_LOCATION
    ...
    try {
      ...
      if (berryClassDeployed) {
        partialLocation = _compileLocation(Object.assign(_location(B.location), { berry: path, version }))
      }

      // Set the pluck class and location to be taken inside pluck()
      UNCLAIMED_BERRY_CLASS = B
      UNCLAIMED_BERRY_LOCATION = partialLocation
      PLUCK_COUNT++
      ...
      if (berryClassDeployed) {
        ...
        const fullLocation = _compileLocation(Object.assign({ hash: stateHashHex }, _location(partialLocation)))
      ...
      UNCLAIMED_BERRY_CLASS = savedUnclaimedBerryClass
      UNCLAIMED_BERRY_LOCATION = savedUnclaimedBerryLocation
      PLUCK_COUNT--
...
function _payload (tx) {
  const { _parsePayloadVersion } = require('../util/version')

  const BAD_PROTOCOL_ERROR = 'Not a run transaction: invalid op_return protocol'
  const BAD_PAYLOAD_ERROR = 'Not a run transaction: invalid run payload'

  if (!tx.outputs.length) throw new StateError(BAD_PROTOCOL_ERROR)
  ...
  if (badProtocol) throw new StateError(BAD_PROTOCOL_ERROR)

  const version = _parsePayloadVersion(chunks[3].buf.toString('hex'))
  ...
  try {
    ...
    const badPayload =
      ...
      !Array.isArray(payload.out) ||
      !Array.isArray(payload.del) ||
      !Array.isArray(payload.cre) ||
      !Array.isArray(payload.exec) ||
      payload.ref.some(ref => typeof ref !== 'string') ||
      payload.out.some(hash => typeof hash !== 'string') ||
      payload.del.some(hash => typeof hash !== 'string') ||
      payload.exec.some(hash => typeof hash !== 'object')

    if (badPayload) throw new StateError(BAD_PAYLOAD_ERROR)
    ...
    throw new StateError(BAD_PAYLOAD_ERROR)
  }
}
...
function _claimBerry (B) {
  // Check that the berry class matches and we haven't already plucked it
  if (UNCLAIMED_BERRY_CLASS !== B) throw new Error('Must only create berry from its berry class')

  const partialLocation = UNCLAIMED_BERRY_LOCATION

  UNCLAIMED_BERRY_CLASS = null
  UNCLAIMED_BERRY_LOCATION = null
