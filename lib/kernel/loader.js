...
class Loader {
  ...
  async _loadJigFromLocation (location) {
    ...
    // Get the state hash
    const hash = hasVout ? metadata.out[vout - metadata.vrun - 1] : metadata.del[vdel]
    ...
    return this._loadJigViaReplay(tx, metadata, location)
  }
  ...
  async _loadJigViaReplay (tx, metadata, location) {
    this._timeout._check()
...
function _metadata (tx) {
  ...
  try {
    ...
    if (badMetadata) throw new StateError(BAD_METADATA_ERROR)
    ...
    throw new StateError(BAD_METADATA_ERROR)
  }
}
