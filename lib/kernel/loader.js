...
const { _assert, _extendsFrom, _text, _Timeout, _deterministicJSONStringify, _isUndefined } = require('../util/misc')
...
class Loader {
  ...
  async _loadBerryFromLocation (location) {
    ...
    const loc = _location(location)
    const classLocation = loc.txid + (!_isUndefined(loc.vout) ? `_o${loc.vout}` : `'_d${loc.vdel}`)
