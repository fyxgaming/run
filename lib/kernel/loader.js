...
class Loader {
  ...
  async _loadWithUserBerryClass (path, BerryClass) {
    ...
    _checkArgument(typeof path === 'string', 'Berry path must be a string')
  ...
  async _pluckBerry (path, B) {
    ...
    const fetchCode = 'async function(txid) { return await b.fetch(txid) }'
    ...
    try {
      ...
      const berryLocation = berryClassDeployed ? `${_sudo(() => B.location)}_${path}` : _UNDEPLOYED
      ...
      const asyncError = 'pluck method must be async'
      _checkState(promise instanceof SI.Promise || promise instanceof HI.Promise, asyncError)
      const berry = await promise

      // Check the berry is allowed for this berry class
      _checkState(berry && berry.constructor === B, `Berry must be an instance of ${_text(B)}`)
