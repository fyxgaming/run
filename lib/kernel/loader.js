...
const { _assert, _parentName, _extendsFrom, _text, _checkState } = require('../util/misc')
...
class Loader {
  ...
  async _loadFresh (location, BerryClass) {
    ...
    _assert(!loc.commit, badLocation)
  ...
  async _loadWithUserBerryClass (location, BerryClass) {
    ...
    const hasLocation = !Bloc.commit && !Bloc.error && Bloc.txid
