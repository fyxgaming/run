...
class Loader {
  ...
  async _loadJigViaReplay (tx, payload, location) {
    ...
    const trustlist = this._kernel._trustlist
    if ((deploys || upgrades) && trustlist && !trustlist.includes(tx.hash) && !trustlist.includes('*')) {
      const hint = 'Hint: Add this location to run.trustlist if you know it is safe'
