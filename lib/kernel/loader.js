...
class Loader {
  constructor (kernel, importLimit = undefined) {
    ...
    this._importLimit = importLimit || new _ImportLimit('load')
  }
  ...
  async _loadJigViaReplay (tx, payload, location) {
    ...
    const whitelist = this._kernel._codeWhitelist
    if ((deploys || upgrades) && whitelist && !whitelist.includes(tx.hash)) {
      throw new Error(`Transaction not whitelisted for code: ${tx.hash}`)
    }
    ...
    const commit = await _import(tx, payload, this._kernel, published, jigToSync, this._importLimit)
