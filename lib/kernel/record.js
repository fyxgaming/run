...
const { SafeSet } = require('../util/set')
...
const { _owner } = require('../util/resource')
const {
  PROTOCOL_VERSION,
  _extractRunData,
  _encryptRunData
...
class Record {
  ...
  async _import (tx, kernel, preexistingJig, immutable, vout, cache) {
    ...
    _populatePreviousOutputs(tx, blockchain)
    ...
    // Get time of the transaction imported. If unbroadcast, then it'll be undefined.
    let txTime = Infinity
    try { txTime = await blockchain.time(tx.hash) } catch (e) { }

    // Make sure all referenced jigs are recent
    for (const [, location] of refs) {
      const txid = location.slice(0, 64)
      const vout = parseInt(location.slice(66))

      // If a ref is unspent, then it must be recent
      const spend = await blockchain.spends(txid, vout)
      if (!spend) continue

      const refTime = await blockchain.time(txid)
      const refSpendTime = await blockchain.time(spend)

      const moreRecentRefAvailableWhenTxWasPublished = refSpendTime <= txTime && refSpendTime >= refTime
      const newerRefWasAvailableForMoreThanTwoHours = txTime - refSpendTime > 2 * 60 * 60 * 1000
      const newerRefWasNotUpdatedTheImportedTransaction = tx.hash !== spend

      const stale = moreRecentRefAvailableWhenTxWasPublished &&
        newerRefWasAvailableForMoreThanTwoHours &&
        newerRefWasNotUpdatedTheImportedTransaction

      if (stale) throw new Error(`${location} is stale. Aborting.`)
    }
    ...
    */
    ...
    const dedupInnerRefs = jig => {
      ...
      JigControl._disableSafeguards(() => _deepReplace(jig, resourceReplacer))
    }
    ...
    this._outputs.forEach((o, n) => {
      ...
      const hex1 = Buffer.from(_owner(o.owner, bsvNetwork).script()).toString('hex')
    ...
    for (let vout = 0; vout < jigProxies.length; vout++) {
      ...
      await kernel._cache.set('jig://' + jigLocation, cachedState)
    }
    ...
    data.code.forEach((def, index) => {
      ...
      const deps = {}
      const props = kernel._code._extractProps(S).props
    ...
    for (let i = 0; i < this._code.length; i++) {
      try {
        const spend = await blockchain.spends(txid, i + 1)
    ...
    for (let i = 0; i < this._outputs.length; i++) {
      try {
        const spend = await blockchain.spends(txid, i + 1 + this._code.length)
    ...
    // Imported transactions are assumed to be signed. We should check this.
    this._signed = false
  }
  ...
  _storeAction (target, method, args, inputs, outputs, reads, before, after, proxies, kernel) {
    ...
    try {
      ...
      const resources = new SafeSet([target])
  ...
  async _pay (kernel) {
    ...
    const tx = this._cachedTx.tx

    this._addPlaceholderUnlockScripts(tx)

    const prevtxids = tx.inputs.map(input => input.prevTxId.toString('hex'))
    const prevraws = await Promise.all(prevtxids.map(txid => blockchain.fetch(txid)))
    const prevtxs = prevraws.map(raw => new Transaction(raw))
    const parents = tx.inputs.map((input, n) => prevtxs[n].outputs[input.outputIndex])

    const rawtx = tx.toString('hex')
    const paidhex = await kernel._purse.pay(rawtx, parents)
    const paidtx = new bsv.Transaction(paidhex)

    this._removePlaceholderUnlockScripts(paidtx)
    await _populatePreviousOutputs(paidtx, blockchain)

    this._cachedTx.tx = paidtx
    this._paid = true
  }

  _addPlaceholderUnlockScripts (tx) {
    // If already signed, then there's no need for a buffer
    ...
    const locks = _spentJigs
      ...
      .map(resource => _owner(resource.owner))
    const domains = locks.map((lock, n) => tx.inputs[n].script.toBuffer().length ? 0 : lock.domain())
    const Buffer = bsv.deps.Buffer
    domains.forEach((size, n) => tx.inputs[n].setScript(Buffer.alloc(size)))
  }

  _removePlaceholderUnlockScripts (tx) {
    // If already signed, then there's no need for a buffer
    if (this._signed) return
    const _spentJigs = this._inputs.filter(jig => jig.origin[0] !== '_')
    tx.inputs.slice(0, _spentJigs.length).forEach(input => input.setScript(''))
  }
  ...
  async _sign (kernel) {
    ...
    // Load the previous UTXOs
    const prevtxids = tx.inputs.map(input => input.prevTxId.toString('hex'))
    const prevraws = await Promise.all(prevtxids.map(txid => kernel._blockchain.fetch(txid)))
    const prevtxs = prevraws.map(raw => new Transaction(raw))
    const parents = tx.inputs.map((input, n) => prevtxs[n].outputs[input.outputIndex])

    // Sign with the owner
    const rawtx = tx.toString('hex')
    const signedhex = await kernel._owner.sign(rawtx, parents, locks)
    const signed = new bsv.Transaction(signedhex)
    await _populatePreviousOutputs(signed, kernel._blockchain)

    this._cachedTx.tx = signed
    this._signed = true
  }
  ...
  _buildBsvTransaction (kernel) {
    ...
    const resourceToRelativeLocation = resource => {
      switch (_resourceType(resource)) {
        ...
        default: throw new Error(`Unknown resource type: ${_text(resource)}`)
      }
    }
    ...
    _spentJigs.forEach((jig, index) => {
      ...
      const scriptBuffer = _owner(before.restore().owner, bsvNetwork).script()
    ...
    this._code.forEach(def => {
      const scriptBuffer = _owner(def.owner, bsvNetwork).script()
      const script = bsv.Script.fromBuffer(Buffer.from(scriptBuffer))
    ...
    this._outputs.forEach(jig => {
      ...
      const scriptBuffer = _owner(restored.owner, bsvNetwork).script()
      const script = bsv.Script.fromBuffer(Buffer.from(scriptBuffer))
