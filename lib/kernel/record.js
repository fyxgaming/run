...
class Record {
  ...
  _checkInteractivity () {
    ...
    const creations = this._outputs.concat(this._deletes).concat(this._refs)
    ...
    nonInteractiveCode.forEach(C => {
      ...
      if (badInteraction) throw new Error(`Interactivity violation: ${C.name} cannot interact with ${badInteraction.name}`)
    })
  }
  ...
  _isUnbound (creation, pending) {
    return _hasCreation(this._unbound, creation) ||
...
function _assignRecordLocations (record) {
  ...
  _sudo(() => {
    record._outputs.forEach((creation, n) => {
      creation.location = _compileLocation({ _record: record._id, _vout: n + 1 })
