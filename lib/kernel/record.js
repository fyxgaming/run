...
class Record {
  constructor () {
    ...
    // Full snapshots for incoming jigs
  ...
  _assignRecordLocation (jig) {
    ...
    _sudo(() => {
      ...
      parts[_hasJig(this._deletes, jig) ? 'vdel' : 'vout'] = this._jigs.length - 1
      jig.location = _compileLocation(parts)
      if (jig.origin === _UNDEPLOYED_LOCATION) jig.origin = jig.location
    })
  }
  ...
  _create (jig) {
    ...
    const native = _sudo(() => _location(jig.origin).native)
  ...
  _read (jig) {
    ...
    this._snapshot(jig, undefined, true)
  ...
  _update (jig, existingSnapshot = undefined) {
    ...
    const Jig = require('./jig')
    _assert(jig instanceof Code || jig instanceof Jig)
    const undeployed = _sudo(() => jig.origin === _UNDEPLOYED_LOCATION)
  ...
  _checkNotDisabled (jig, method) {
    ...
    if (_hasJig(this._disables, jig)) throw new StateError(`${method} disabled: ${reason}`)
  }
  ...
  _checkNotWithinBerry (jig, method) {
    ...
    if (withinBerry) throw new StateError(`Cannot ${method} ${_text(jig)} in berry`)
  }
   ...
   */
  _snapshot (jig, existingSnapshot, readOnly) {
    ...
    const prevss = this._snapshots.get(jig)
    if (prevss) {
    ...
    const snapshot = existingSnapshot || new Snapshot(jig, bindingsOnly, readOnly)
  ...
  _link (jig, readonly, method) {
    ...
    if (_defined(loc.record)) {
      if (loc.record !== this._id && !readonly) throw new Error(`Cannot ${method} ${_text(jig)}: open transaction`)
      return
    }
