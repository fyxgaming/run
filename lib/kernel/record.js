...
class Record {
  ...
  _link (jig, readonly) {
    ...
    if (recordid) {
      ...
      if (!thisRecord && !readonly) throw new Error(`Cannot link ${_text(jig)}: transaction open`)
      return
    }
    ...
    if (commitid) {
      ...
      if (!commit) throw new Error(`Cannot link ${_text(jig)}: commit not found`)
