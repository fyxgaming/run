...
const { _deepVisit, _deepClone, _deepReplace } = require('./deep')
...
class Record {
  constructor () {
    ...
    this._importLimit = new ImportLimit()
  ...
  async _createDependencyMap (jig) {
    ...
    const prev = await this._kernel._load(location, { _importLimit: this._importLimit })
    ...
    if (eString.indexOf('Missing inputs') !== -1 || eString.indexOf('txn-mempool-conflict') !== -1) {
      ...
      for (const input of this._tx.inputs) {
        try {
          ...
          try {
            const jig = await this._kernel._load(prevlocation, { _importLimit: this._importLimit })
            const Code = require('./code')
 ...
 */
Record._import = async (rawtx, published = false, jigToSync = null,
  importLimit = new ImportLimit()) => {
  ...
  try {
    ...
    for (let vin = 0; vin < payload.in; vin++) {
      ...
      const jig = jigToSyncLocation === location ? jigToSync
        : await kernel._load(location, { _importLimit: importLimit })
    ...
    for (let vref = 0; vref < payload.ref.length; vref++) {
      ...
      const jig = await kernel._load(location, { _importLimit: importLimit })
    ...
    if (jigToSync) {
      ...
      _deepReplace(jigToSync, replaceInnerRefs)
      _deepReplace(record._before.get(jigToSync), replaceInnerRefs)
    }
