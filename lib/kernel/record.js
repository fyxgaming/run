...
class Record {
  ...
  async _pay (kernel) {
    ...
    const paid = await kernel._purse.pay(txhex)
  ...
  _addPlaceholderUnlockScripts (tx) {
    ...
    const unlockSizes = locks.map(
      (lock, n) => tx.inputs[n].script.toBuffer().length ? 0
        : lock.unlockSize ? lock.unlockSize() : 1000)
    const Buffer = bsv.deps.Buffer
    unlockSizes.forEach((size, n) => tx.inputs[n].setScript(Buffer.alloc(size)))
  }
