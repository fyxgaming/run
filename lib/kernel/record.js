...
class Record {
  ...
  /**
   * Saves the original bindings
   */
  _captureBindings (jig) {
    if (this._bindings.has(jig)) return
    const bindings = _sudo(() => {
      return {
        location: jig.location,
        origin: jig.origin,
        nonce: jig.nonce,
        owner: _deepClone(jig.owner, SI),
        satoshis: jig.satoshis
      }
    })
