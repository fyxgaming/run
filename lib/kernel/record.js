...
const { _assert, _text, _hasCreation, _kernel, _defined } = require('../util/misc')
...
class Record {
  ...
  _assignRecordLocation (creation) {
    if (_hasCreation(this._creations, creation)) return
    this._creations.push(creation)
    _sudo(() => {
      const parts = { record: this._id }
      parts[_hasCreation(this._deletes, creation) ? 'vdel' : 'vout'] = this._creations.length - 1
