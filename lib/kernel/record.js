 ...
 */
Record._import = async (rawtx, published) => {
  ...
  try {
    ...
    for (let i = 0; i < payload.in; i++) {
      _assert(tx.inputs[i].prevTxId.toString('hex') === record._tx.inputs[i].prevTxId.toString('hex'))
      _assert(tx.inputs[i].outputIndex === record._tx.inputs[i].outputIndex)
    }

    // Make sure the outputs match
    for (let i = 1; i <= payload.out.length; i++) {
      _assert(tx.outputs[i].script.toString('hex') === record._tx.outputs[i].script.toString('hex'))
      _assert(tx.outputs[i].satoshis === record._tx.outputs[i].satoshis)
    }

    // TODO: If published, assign final locations, save state, etc.
    // Otherwise, what to do? Save tx? make state signed, ready to publish?
    if (published) {
...
function execute (record, cmd, data) {
  ...
  switch (cmd) {
    case 'deploy': {
      ...
      const srcList = data.filter((x, i) => i === 0)
      const encpropsList = data.filter((x, i) => i === 1)
