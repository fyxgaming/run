...
const { _populatePreviousOutputs, _txify } = require('../util/bsv')
...
class Record {
  ...
  async _pay (kernel) {
    ...
    const hex = this._cachedTx.tx.toBuffer().toString('hex')
    const paid = await kernel._purse.pay(hex)
  ...
  async _sign (kernel) {
    ...
    this._cachedTx.tx = _txify(await kernel._owner.sign(this._cachedTx.tx, locks))
