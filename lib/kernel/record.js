...
class Record {
  ...
  _delete (jig) {
    ...
    const native = Membrane._sudo(() => _location(jig.origin).native)
    _assert(!native)
    _checkState(!hasJig(this._disables, jig), `Jig temporarily disabled, please sync: ${_text(jig)}`)
    this._checkNotDisabled(jig)
  ...
  _checkNotDisabled (jig) {
    ...
    _checkState(!hasJig(this._unapprovable, jig), `New owners cannot sign for changes in this transaction\n\n${info}\n\n${hint}`)
  }
