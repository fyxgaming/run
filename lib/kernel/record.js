...
class Record {
  ...
  async _import (tx, kernel, preexistingJig, immutable, vout, cache) {
    ...
    */
    ...
    if (data.jigs !== this.outputs.length) throw new Error(`bad number of jigs: ${data.jigs} != ${this.outputs.length}`)
    ...
    for (let i = 0; i < this.code.length; i++) {
      if (tx.outputs[i + 1].spentTxId === null) {
        const S = this.code[i].S
        kernel._inventory._notify(S)
      }
    }
    for (let i = 0; i < this.outputs.length; i++) {
      if (tx.outputs[i + 1 + this.code.length].spentTxId === null) {
        const target = this.outputs[i]
  ...
  begin () { this._beginCount++ }
    ...
    if (--this._beginCount === 0 && this._onReadyForPublish && (this.code.length || this.actions.length)) {
  ...
  rollback (_lastPosted, kernel, error, unhandled) {
    ...
    this.code.forEach(def => kernel._inventory._notify(def.S))
  ...
  _storeCode (T, S, deps, props, success, error, owner, code, kernel) {
    ...
    try {
      ...
      this.code.push({ T, S, deps, props, success, error, owner })
