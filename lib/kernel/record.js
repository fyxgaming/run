...
class Record {
  ...
  async _calculateState (jig) {
    ...
    if (after._codeTarget) state.src = after._codeTarget.toString()
  ...
  async _createReferenceMap (jig) {
    ...
    const prev = await load(this._kernel, location, undefined, this._importLimit)
    ...
    if (eString.indexOf('Missing inputs') !== -1 || eString.indexOf('txn-mempool-conflict') !== -1) {
      ...
      for (const input of this._tx.inputs) {
        try {
          ...
          try {
            const jig = await load(this._kernel, prevlocation, undefined, this._importLimit)
            const Code = require('./code')
