...
class Record {
  ...
  _action (action) {
    ...
    // Early out if we aren't supposed to record, like Pluck and Owners
    if (!action._recordable()) return
  ...
  _sign (creation, callers = true) {
    ...
    if (_hasCreation(this._signs, creation)) {
      this._checkBound(creation, 'sign')
      if (callers) this._signCallers()
      return
    }
   ...
   */
  _checkBound (creation, method) {
    if (_hasCreation(this._unbound, creation)) {
      ...
      throw new StateError(`${method} unbound: ${reason}`)
    }
  }
