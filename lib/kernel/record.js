 ...
 */
Record._import = async (rawtx, published = false, jigToSync = null, limit = new ImportLimit()) => {
  ...
  try {
    ...
    for (let vin = 0; vin < payload.in; vin++) {
      ...
      const jig = jigToSyncLocation === location ? jigToSync
        : await kernel._load(location, { _importLimit: limit })
    ...
    for (let vref = 0; vref < payload.ref.length; vref++) {
      ...
      const jig = await kernel._load(location, { _importLimit: limit })
