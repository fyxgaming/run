...
class Record {
  ...
  async _pay (kernel) {
    ...
    const paidhex = await kernel._purse.pay(txhex, utxos)
  ...
  async _sign (kernel) {
    ...
    // Assign the locks to the UTXOs
    locks.forEach((lock, n) => { utxos[n].lock = lock })

    // Sign with the owner
    const txhex = tx.toString('hex')
    const signedhex = await kernel._owner.sign(txhex, utxos)
