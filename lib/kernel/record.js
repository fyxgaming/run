...
Record._import = async (rawtx, published) => {
  ...
  try {
    ...
    // TODO figure out
    if (published) record._state = STATE_PUBLISHED
  } catch (e) {
    record._rollback(e)
    throw e
  }
...
function execute (cmd, data) {
  ...
  switch (cmd) {
    case 'deploy':
      console.log('DEPLOY')
      // Eval all the code in a sandbox

      // Parents get installed first.

      // Inflate the properties, but when we get a jig we don't have, use one of the new deploys

      // Assign the properties

      // Deploy the code together

      // kernel._deploy([...Code])

      // Code.deploy() is special, not exposed. Run.deploy() is expected.
      // I could just call _deploy on it... simple
      // Are these assigned in the repository?
      // No. Repository is for local code only.
      // Instanceof code should not check repository then.
      // All code's prototype has Code in its inheritance
      // Sandbox, then change the prototype chain
      // Code has sync(), upgrade(), etc.
      // Special function to turn class into code
      // So here we deploy the class, but pass a param to not add it to the repository

      // What about the record
      // For all execute actions
      // Hijack record, but then do things normally
