...
Record._import = async (rawtx, published) => {
  ...
  // Make sure this is a Run transaction

  // _assert(isRunTransaction(tx), 'Not a run transaction')

  function _checkRunTransaction (tx) {
    _assert(tx.outputs.length, 'Not a run transaction: no outputs')
    _assert(tx.outputs[0].script.chunks.length === 6, 'Not a run transaction: bad payload size')
  }
  ...
  */
  ...
  // Where to check protocol

  // Don't add to RECORDS
  const record = new Record()
  // TODO
  record._outputs = [{}, {}, {}]
  record._deletes = [{}, {}, {}]
