...
class Record {
   ...
   */
  _create (jig) {
    this._checkNotPlucking(jig, 'create')
  ...
  _update (jig, existingSnapshot = undefined) {
    this._checkNotPlucking(jig, 'update')
   ...
   */
  _delete (jig) {
    this._checkNotPlucking(jig, 'delete')
  ...
  _auth (jig, callers = true) {
    this._checkNotPlucking(jig, 'auth')
  ...
  _checkNotDisabled (jig, method) {
    const reason = _hasJig(this._deletes, jig) ? `${_text(jig)} deleted` : `${_text(jig)} has new owner`
   ...
   */
  _checkNotPlucking (jig, method) {
    if (!this._stack.length) return
    const { _PluckAction } = require('./action')
    const error = `Cannot ${method} ${_text(jig)} during pluck`
    _checkState(!(this._stack[0] instanceof _PluckAction), error)
  }
  ...
  _link (jig, readonly, method) {
    ...
    // Commits should exist. If not, we have an internal bug.
    if (commitid) {
      const Commit = require('./commit')
      const commit = Commit._get(commitid)
      if (!commit) throw new Error(`Cannot ${method} ${_text(jig)}: upstream commit not found`)
