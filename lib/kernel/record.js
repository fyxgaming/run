...
class Record {
  ...
  async _import (tx, kernel, preexistingJig, immutable, vout, cache) {
    ...
    */
    ...
    for (const action of data.actions) {
      const resourceLoader = ref =>
        JigControl._disableSafeguards(() => {
          ...
          return this.proxies.get(this._outputs[vout - data.code.length])
        })
      ...
      if (action.method === 'init') {
        ...
        const subject = refs.get(action.target) ||
          this.proxies.get(this._outputs[parseInt(action.target.slice(2)) - 1 - data.code.length])
    ...
    const spentJigs = this._inputs.filter(i => i.origin[0] !== '_')
    ...
    const proxies = this._outputs.map(o => this.proxies.get(o))
  ...
  _storeCode (T, S, deps, props, success, error, owner, code, kernel) {
    ...
    try {
      ...
      if (!this._importing) kernel._inventory._notify(code._getSandboxed(T))
      return tempLocation
    } finally {
      this.end()
    }
    ...
    try {
      ...
      inputs.forEach(newTarget => {
        ...
        if (this._inputs.some(isDifferentInstance)) {
          ...
          const differentLocation = this._before.get(differentInstance).serialized.location
  ...
  async sign (kernel) {
    if (!this._cachedTx) this._buildBsvTransaction(kernel)
    const spentJigs = this._inputs.filter(jig => jig.origin[0] !== '_')
  ...
  _buildBsvTransaction (kernel) {
    ...
    this.reads.forEach(jig => {
      if (spentJigs.includes(jig) || this._outputs.includes(jig)) return
    ...
    const code = this._code.map(def => {
      ...
      return { text: _sourceCode(def.T), deps, props, creator }
    })
