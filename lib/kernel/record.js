...
const { _assert, _text, _checkState } = require('../util/misc')
...
class Record {
  ...
  _action (action) {
    ...
    Membrane._sudo(() => {
      for (const [jig, snapshot] of this._snapshots) {
        if (hasJig(this._disables, jig)) continue
    ...
    // All deleted jigs become unupdatable
    this._deletes
      .filter(jig => !hasJig(this._disables, jig))
  ...
  _create (jig) {
    ...
    _assert(!hasJig(this._disables, jig))
  ...
  _checkNotDisabled (jig) {
    ...
    const disabled = hasJig(this._disables, jig)
    const msg = disabled ? msg2 : msg1
    _checkState(!hasJig(this._unapprovable, jig), msg)
  }
