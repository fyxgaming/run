...
class Record {
  ...
  async _import (tx, kernel, preexistingJig, immutable, vout, cache) {
    ...
    */
    ...
    const spentLocations = spentJigs.map(jig => this._locations.get(jig.origin) || jig.location)
  ...
  _buildBsvTransaction (kernel) {
    ...
    this._reads.forEach(jig => {
      ...
      const location = publisher._lastPosted.get(jig.origin) ||
        this._locations.get(jig.origin) || jig.location
    ...
    // build inputs
    const spentLocations = spentJigs.map(jig => publisher._lastPosted.get(jig.origin) ||
      this._locations.get(jig.origin) || jig.location)
