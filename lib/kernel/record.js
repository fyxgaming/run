...
const { _kernel, _assert, _text, _sourceCode, _parent, _hasOwnProperty } = require('./misc')
...
class Record {
  ...
  _deploy (...Cs) {
    ...
    Cs.forEach(C => {
      ...
      // Unsealed classes don't need to be spent
      const parentUnsealed = _hasOwnProperty(Parent, 'sealed') ? !Parent.sealed : false
      if (parentUnsealed) return
