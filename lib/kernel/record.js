...
const PROTOCOL_COMMANDS = ['deploy']
...
class Record {
  ...
  _cmd (name, data) {
    ...
    _assert(PROTOCOL_COMMANDS.includes(name))
  ...
  _combine (record) {
    ...
    console.log('REDUCE NONCE', reduceNonce, outputs)
  ...
  _publish () {
    ...
    this._publishInternal()
      .catch(e => { this._state = STATE_ROLLBACK; this._rollback(e) })
      .then(() => { this._state = STATE_PUBLISHED })
  }
  ...
  _sortJigsForReplayability () {
    this._jigs.sort((a, b) => {
      ...
      const readsB = hasJig(this._reads, a)
  ...
  async _calculateState (jig) {
    ...
    console.log('SPENDS', this._spends)
    console.log('OUTPUTS', this._outputs)
    console.log('DEL', this._deletes)
    console.log('REF', this._reads)
  ...
  async _broadcastTx () {
    _assert(this._txid, 'Transaction incomplete')
  ...
  _deploy (...Cs) {
    ...
    Cs.forEach(C => {
      ...
      const parentUtility = Object.getOwnPropertyNames(Parent).includes('utility') && Parent.utility
      if (parentUtility) return
 ...
 */
Record._import = async (rawtx, published, jigToSync) => {
  const tx = new Transaction(rawtx)

  Log._info(TAG, 'Import', tx.hash)

  // Get the payload, which will also verify this is a run transaction
  ...
  try {
    ...
    for (let vin = 0; vin < payload.in; vin++) {
      ...
      const jig = jigToSyncLocation === location ? jigToSync : await kernel._load(location)
      record._spend(jig)
    }
    ...
    for (let vref = 0; vref < payload.ref.length; vref++) {
      ...
      const jig = await kernel._load(location)
      record._read(jig)
    }

    // Put the jig to sync into the ref and spend jigs
    if (jigToSync) {
      // TODO
      console.log('PLACING')
      console.log(jigToSync.deps)
    }

    // Lock in the initial state of the record. Each action will add to it.
    record._commit()
    console.log('^^^^^', jigToSync && jigToSync.deps)
    ...
    console.log(payload)
    console.log(record._payload)
    console.log(record._states)
    ...
    if (published) {
      record._state = STATE_PUBLISHED
      record._txid = tx.hash
      record._finalize()
    }
  } catch (e) {
    record._rollback(e)
...
function execute (record, cmd, data) {
  ...
  switch (cmd) {
    case 'deploy': {
      ...
      const codec = new Codec()._loadJigs(x => (x < record._jigs.length)
      ...
      for (let i = 0; i < srcList.length; i++) {
        ...
        if (parentMatch) {
          ...
          console.log('============')
          console.log(tempProps.deps, parentName)
          console.log('============')
        }
      ...
      console.log(newCode)
      console.log(newCode[0].location)

      // Turn the code into Code
      ...
      // Deploy of the code together
      const oldPublishIntercept = PUBLISH_INTERCEPT
      try {
        PUBLISH_INTERCEPT = newRecord => record._combine(newRecord)
        console.log(newCode)
