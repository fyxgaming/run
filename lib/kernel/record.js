...
class Record {
  constructor () {
    ...
    // Whether rollbacks (and before snapshots) are to be used
    this._rollbacks = false
    try { this._rollbacks = _kernel()._rollbacks } catch (e) { }
  ...
  _read (jig) {
    ...
    this._snapshot(jig, undefined, false)
   ...
   */
  _snapshot (jig, existingSnapshot, bindingsOnly = !this._rollbacks) {
    ...
    const snapshot = existingSnapshot || new Snapshot(jig, bindingsOnly)
