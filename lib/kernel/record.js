...
class Record {
  ...
  async _pay (kernel) {
    ...
    const unlockSize = this._estimateUnlockSize()
    const paid = await kernel._purse.pay(txhex, unlockSize)
  ...
  _estimateUnlockSize () {
    ...
    const inputs = this._cachedTx.tx.inputs

    const unlockSize = locks
      .map((lock, n) => inputs[n].script.toBuffer().length ? 0
