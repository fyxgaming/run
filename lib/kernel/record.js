...
class Record {
  ...
  async _import (tx, kernel, preexistingJig, immutable, vout, cache) {
    ...
    let txTime = Infinity
    try { txTime = await kernel._blockchain.time(tx.hash) } catch (e) { }
    ...
    for (const [, location] of refs) {
      ...
      const spend = await kernel._blockchain.spends(txid, vout)
      if (!spend) continue

      const refTime = await kernel._blockchain.time(txid)
      const refSpendTime = await kernel._blockchain.time(spend)
  ...
  async _pay (kernel) {
    ...
    const prevtxs = await Promise.all(prevtxids.map(txid => kernel._blockchain.fetch(txid)))
    ...
    await _populatePreviousOutputs(paidtx, kernel._blockchain)
