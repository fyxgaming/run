...
class Record {
  ...
  async _pay (kernel) {
    ...
    this._addPlaceholderUnlockScripts(this._cachedTx.tx)
    const txhex = this._cachedTx.tx.toString('hex')
    const spent = this._cachedTx.tx.inputs.reduce((prev, curr) => curr.output.satoshis + prev, 0)
    const inputs = (await Promise.all(this._cachedTx.tx.inputs
      .map(input => input.prevTxId.toString('hex'))
      .map(txid => kernel._blockchain.fetch(txid))))
      .map(tx => tx.toString('hex'))
    const paid = await kernel._purse.pay(txhex, inputs, spent)
    this._cachedTx.tx = new bsv.Transaction(paid)
    this._removePlaceholderUnlockScripts(this._cachedTx.tx)
    await _populatePreviousOutputs(this._cachedTx.tx, kernel._blockchain)
    this._paid = true
  }
  ...
  async _sign (kernel) {
    ...
    // Sign with the owner
    const txhex = this._cachedTx.tx.toString('hex')
    const signed = await kernel._owner.sign(txhex, inputs, locks)
    this._cachedTx.tx = new bsv.Transaction(signed)
    await _populatePreviousOutputs(this._cachedTx.tx, kernel._blockchain)
    this._signed = true
  }
