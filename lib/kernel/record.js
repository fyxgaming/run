...
class Record {
  ...
  _pop () {
    ...
    if (!this._stack.length) this._action(action)
  }
  ...
  _action (action) {
    ...
    _sudo(() => {
      for (const [jig, snapshot] of this._snapshots) {
        ...
        if (!jig.owner._value && !jig.satoshis._value) continue
        this._disable(jig)
      }
    })
    ...
    const EVENT_RECORD = require('./kernel')._EVENT_RECORD
    this._jigs.forEach(jig => kernel._emit(EVENT_RECORD, jig))
  ...
  _assignRecordLocation (jig) {
    if (this._jigs.includes(jig)) return
    this._jigs.push(jig)
    _sudo(() => {
      jig.location = `${this._id}_j${this._jigs.length - 1}`
  ...
  _update (jig, existingSnapshot = undefined) {
    if (_hasJig(this._updates, jig)) { this._authCallers(); return }
   ...
   */
  _delete (jig) {
    if (_hasJig(this._deletes, jig)) { this._authCallers(); return }
  ...
  _link (jig) {
    ...
    if (!commitid && !recordid) return
    if (recordid && recordid.startsWith(this._id)) return

    const Commit = require('./commit')
    const commit = Commit._get(commitid)
    if (!commit) throw new Error(`Cannot link ${_text(jig)}`)

    if (!this._deps.includes(commit)) this._deps.push(commit)
  }
  ...
  _rollback (e) {
    ...
    const kernel = _kernel()
    const EVENT_ROLLBACK = require('./kernel')._EVENT_ROLLBACK
    this._jigs.forEach(jig => kernel._emit(EVENT_ROLLBACK, jig))
  }
   ...
   */
  _caller () {
    // Get the call actions. New actions are skipped over.
