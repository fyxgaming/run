 ...
 * record.js
 *
 * Stores creation actions as they happen
 */

const bsv = require('bsv')
const { crypto } = bsv
const Unbound = require('../util/unbound')
const { _assert, _text, _hasJig, _kernel, _defined } = require('../util/misc')
...
class Record {
  constructor () {
    ...
    // CRUDAD ordered sets. Creations may belong to multiple.
    this._creates = []
    this._reads = []
    this._updates = []
    this._deletes = []
    ...
    // All creations updated in this record, in order of add. This is just used to assign locations.
    this._creations = []
  ...
  _action (action) {
    ...
    _sudo(() => {
      for (const [creation, snapshot] of this._snapshots) {
        if (_hasJig(this._disables, creation)) continue
    ...
    this._deletes
      .filter(creation => !_hasJig(this._disables, creation))
      .forEach(creation => this._disable(creation))

    // Assign locations to every creation updated
    this._creates.forEach(creation => this._assignRecordLocation(creation))
    this._auths.forEach(creation => this._assignRecordLocation(creation))
    this._updates.forEach(creation => this._assignRecordLocation(creation))
    this._deletes.forEach(creation => this._assignRecordLocation(creation))

    if (!this._replaying) {
      const kernel = _kernel()
      this._creations.forEach(creation => kernel._emit('update', creation))
    }
  ...
  _disable (creation) {
    _assert(!this._rolledBack)
    if (_hasJig(this._disables, creation)) return
  ...
  _assignRecordLocation (creation) {
    if (_hasJig(this._creations, creation)) return
    this._creations.push(creation)
    _sudo(() => {
      const parts = { record: this._id }
      parts[_hasJig(this._deletes, creation) ? 'vdel' : 'vout'] = this._creations.length - 1
      creation.location = _compileLocation(parts)
      if (creation.origin === _UNDEPLOYED_LOCATION) creation.origin = creation.location
    })
  }
  ...
  _create (creation) {
    ...
    _assert(!this._rolledBack)
    if (_hasJig(this._creates, creation)) { this._authCallers(); return }
    ...
    _assert(!_hasJig(this._updates, creation))
    _assert(!_hasJig(this._deletes, creation))
    _assert(!_hasJig(this._auths, creation))
    _assert(!_hasJig(this._disables, creation))
  ...
  _read (creation) {
    _assert(!this._rolledBack)
    if (_hasJig(this._reads, creation)) return
    ...
    // Note: Reads to not need to take snapshots because nothing changed
  }
  ...
  _update (creation, existingSnapshot = undefined) {
    ...
    _assert(!this._rolledBack)
    if (_hasJig(this._updates, creation)) {
    ...
    _assert(!undeployed || _hasJig(this._creates, creation))
    const native = _sudo(() => _location(creation.origin).native)
    _assert(!_defined(native))
    this._checkNotDisabled(creation, 'update')
  ...
  _delete (creation) {
    ...
    _assert(!this._rolledBack)
    if (_hasJig(this._deletes, creation)) {
  ...
  _auth (creation, callers = true) {
    ...
    _assert(!this._rolledBack)
    if (_hasJig(this._auths, creation)) {
    ...
    const Jig = require('./jig')
    _assert(creation instanceof Code || creation instanceof Jig)
    _assert(!_hasJig(this._creates, creation))
    this._checkNotDisabled(creation, 'auth')
  ...
  _authCallers () {
    ...
    this._stack
      ...
      .filter(creation => !_hasJig(this._creates, creation))
  ...
  _checkNotDisabled (creation, method) {
    const reason = _hasJig(this._deletes, creation) ? `${_text(creation)} deleted`
      : `${_text(creation)} has an unbound new owner or satoshis value`
    if (_hasJig(this._disables, creation)) throw new StateError(`${method} disabled: ${reason}`)
  }
  ...
  _snapshot (creation, existingSnapshot, readOnly) {
    ...
    const prevss = this._snapshots.get(creation)
    if (prevss) {
    ...
    this._snapshots.set(creation, snapshot)
  }
  ...
  _link (creation, readonly, method) {
    ...
    if (_defined(loc.record)) {
      if (loc.record !== this._id && !readonly) throw new Error(`Cannot ${method} ${_text(creation)}: open transaction`)
      return
    }

    // Commits should be being published. If not, we are in a transaction.
    if (_defined(loc.commit)) {
      const Commit = require('./commit')
      const commit = Commit._findPublishing(loc.commit)
  ...
  _rollback (e) {
    ...
    this._snapshots.forEach(snapshot => snapshot._rollback(e))
