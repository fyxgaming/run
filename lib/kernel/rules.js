...
class Rules {
  constructor () {
    // The owning parent creation if it exists (undefined = current, null = none)
    this._creation = undefined
    // Whether to allow admin mode to override everything
    this._admin = false
    // Whether to check for errors on the underlying creation
    this._errors = false
    // Whether this target has creation bindings that need protection
    this._creationBindings = false
    ...
    this._privacy = false
    // Whether this creation cannot be changed
    ...
    this._cow = false
    // List of method names that cannot be called on this creation
  ...
  static _cow () {
    const rules = new Rules()
    rules._creation = null
    rules._admin = true
    rules._errors = true
    rules._creationBindings = false
    rules._utxoBindings = false
    rules._reserved = false
    rules._codeMethods = false
    rules._jigMethods = false
  ...
  static _childProperty (creation, method) {
    ...
    const rules = new Rules()
    rules._creation = creationRules._cow ? null : creation
    rules._admin = creationRules._admin
    rules._errors = creationRules._errors
    rules._creationBindings = false
    rules._utxoBindings = false
    rules._reserved = false
    rules._codeMethods = false
