...
class Rules {
  constructor () {
    ...
    // Whether this jig should record reads and changes and methods can be recorded
    ...
    // Whether its properties are only updatable by its owner
    this._owned = false
  ...
  static _berryInstance () {
    ...
    rules._immutable = true
    rules._recordable = true
    rules._recordCalls = true // they are not recorded on-chain, but they do need to be added to the stack
    rules._owned = true
    rules._thisless = false
    rules._cow = false
    rules._cowProps = false
  ...
  static _childProperty (parentJig, method, owned) {
    ...
    rules._recordCalls = false
    rules._owned = parentRules._owned
