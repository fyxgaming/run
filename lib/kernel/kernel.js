...
class Kernel {
  ...
  async _fetch (txid) {
    ...
    if (this._client) {
      const cachedTx = await this._cache.get(`tx://${txid}`)
      if (typeof cachedTx !== 'undefined') return cachedTx
      throw new ClientModeError(txid, 'transaction')
    }
  ...
  async _spends (txid, vout) {
    // In client mode, we must use the cache.
    if (this._client) {
      const cachedSpend = await this._cache.get(`spend://${txid}_o${vout}`)
  ...
  async _time (txid) {
    // In client mode, we must use the cache.
    if (this._client) {
      const cachedTime = await this._cache.get(`time://${txid}`)
