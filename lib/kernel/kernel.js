...
class Kernel {
  constructor () {
    ...
    // State API implementation - optional
    this._state = null
  ...
  _blockchainWrapper () { return new BlockchainWrapper(this._blockchain, this._cacheAPI(), this._client) }

  // --------------------------------------------------------------------------

  _stateAPI () { return new StateWrapper(this._state, this._cacheAPI()) }
  ...
  async _trusted (txid, method) {
    return this._trustlist.has('*') ||
      ...
      await this._cacheAPI().get(`trust://${txid}`)
  }
}
