...
class Kernel {
  ...
  async _load (location, options = {}) {
    ...
    const hasVout = 'vout' in loc && loc.vout >= 1 && loc.vout <= payload.out.length
    const hasVdel = 'vdel' in loc && loc.vdel >= 0 && loc.vdel < payload.del.length
    ...
    // Get all referenced jigs out of the state
    // Load the referenced jigs (and prevent infinite loops)
    // Decode the state
