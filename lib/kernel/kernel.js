...
const { BlockchainWrapper } = require('./wrappers')
    ...
    return {
      async state (location) {
        ...
        try {
          ...
          const cacheValue = await kernel._cache.get(cacheKey)
          if (typeof cacheValue !== 'undefined') return cacheValue

          return await state.state(location)
        } finally {
      ...
      async trusted (txid) {
        ...
        try {
          ...
          return await state.trusted(txid)
        } finally {
      ...
      async banned (location) {
        ...
        try {
          ...
          return await state.banned(location)
        } finally {
