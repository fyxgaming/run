...
const { BlockchainWrapper, StateWrapper, CacheWrapper, OwnerWrapper, PurseWrapper } = require('./wrappers')
...
class Kernel {
  constructor () {
    ...
    // State API implementation - required
    this._state = null
  ...
  _blockchainAPI () { return new BlockchainWrapper(this._blockchain, this._cache, this._client) }

  // --------------------------------------------------------------------------

  _stateAPI () { return new StateWrapper(this._state, this._cache) }

  // --------------------------------------------------------------------------

  _cacheAPI () { return new CacheWrapper(this._cache) }

  // --------------------------------------------------------------------------

  _ownerAPI () { return new OwnerWrapper(this._owner) }

  // --------------------------------------------------------------------------

  _purseAPI () { return new PurseWrapper(this._purse) }
  ...
  async _trusted (txid, method) {
    return this._trustlist.has('*') ||
      ...
      await this._cache.get(`trust://${txid}`)
  }
}
