...
const { _assert, _text, _bsvNetwork, _addJigs, _Timeout, _kernel } = require('../util/misc')
  ...
  if (needsOwners) {
    ...
    _deepVisit(owners, x => {
      if (x instanceof Creation) {
        ...
        if (!txid && !nativeid) throw new StateError(`${x.name} must first be deployed`)
      }
    })
  }
}
