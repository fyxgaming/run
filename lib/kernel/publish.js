...
async function _publish (commit) {
  try {
    ...
    const [states, localRefmaps] = await captureStates(commit, timeout)
    timeout._check()
    ...
    // Add to cache the refmaps too
    await cacheRefmaps(commit, localRefmaps, txid)
    timeout._check()
...
async function captureStates (commit, timeout) {
  ...
  for (const jig of jigs) {
    const [state, localRefmap] = await _capture(jig, commit, timeout)
    states.set(jig, state)
    localRefmaps.set(jig, localRefmap)
  }

  return [states, localRefmaps]
}
...
async function cacheRefmaps (commit, localRefmaps, txid) {
  const promises = []
  const refmaps = new Map()

  // Convert each local refmap to a full refmap
  for (const [jig, localRefmap] of localRefmaps) {
    const refmap = {}
    for (const origin of Object.keys(localRefmap)) {
      const [location, nonce] = localRefmap[origin]
      const fullOrigin = origin.startsWith('_') ? txid + origin : origin
      const fullLocation = location.startsWith('_') ? txid + location : location
      refmap[fullOrigin] = [fullLocation, nonce]
    }
    refmaps.set(jig, refmap)
  }

  for (let i = 0; i < commit._outputs.length; i++) {
    const jig = commit._outputs[i]
    const refmap = refmaps.get(jig)
    _assert(refmaps)
    const vout = i + 1
    const key = `refmap://${txid}_o${vout}`
    promises.push(commit._kernel._cache.set(key, refmap))
  }
...
_publish._cacheRefmaps = cacheRefmaps
