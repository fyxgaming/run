...
async function _publish (commit) {
  try {
    ...
    // Cache refmaps for all our outputted jigs
    await captureAndCacheRefmaps(commit, txid, timeout)
    timeout._check()
...
async function captureAndCacheRefmaps (commit, txid, timeout) {
  const { _captureAndCacheRefmap } = require('./refmap')

  const jigs = commit._outputs.concat(commit._deletes)
  const promises = []

  for (const jig of jigs) {
    promises.push(_captureAndCacheRefmap(jig, commit, txid, timeout))
  }
...
_publish._captureAndCacheRefmaps = captureAndCacheRefmaps
