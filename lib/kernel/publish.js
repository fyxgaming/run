...
const { _assert, _addJigs, _checkState } = require('../util/misc')
...
// Serializes the broadcast portion of publish
const BROADCAST_QUEUE = new SerialTaskQueue()
...
async function _publish (commit) {
  try {
    ...
    const exec = createInstructions(commit, masterList)
    ...
    // Serialize pay-sign-broadcast because pay() may use outputs
    // that should not be used by another pay() running in parallel.
    const tx = await BROADCAST_QUEUE.enqueue(async () => {
...
function createInstructions (commit, masterList) {
  ...
  return commit._actions.map(action => instruction(action, codec))
}
...
_publish._createInstructions = createInstructions
_publish._createPayload = createPayload
_publish._createPartialTx = createPartialTx
_publish._applyJigBindings = applyJigBindings
