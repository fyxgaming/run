...
async function checkNoTimeTravel (commit, timeout) {
  ...
  for (const jig of commit._refs) {
    const moment = commit._moments.get(jig)

    if (!(moment.origin in refmap)) continue

    const refmapNonce = refmap[moment.origin][1]
    _checkState(moment.nonce >= refmapNonce, `Time travel for ${_text(jig)}`)
  }
}
...
function createPayload (commit, hashes, exec, masterList) {
  ...
  const ref = commit._refs.map(jig => commit._moments.get(jig).location)
