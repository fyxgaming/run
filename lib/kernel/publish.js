...
async function captureStates (commit, timeout) {
  ...
  const record = commit._record
  const jigs = record._outputs._arr().concat(record._deletes._arr())
...
async function getParents (tx, record, locks, feePerKb) {
  ...
  const rawTransactions = await Promise.all(inputs.map(input => _kernel()._blockchainAPI().fetch(input.prevTxId.toString('hex'))))
