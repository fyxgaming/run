...
async function _publish (commit, pay = true, sign = true) {
  try {
    ...
    const tx = await PURSE_SAFETY_QUEUE._enqueue(async () => {
      ...
      const paidtx = pay ? await payForTx(partialtx, commit) : partialtx
      timeout._check()

      // Sign the jig owners
      const signedtx = sign ? await signTx(paidtx, commit) : partialtx
      timeout._check()
