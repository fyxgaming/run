...
async function _publish (commit) {
  try {
    ...
    const importLimit = new _ImportLimit()
    const states = await calculateStates(commit, importLimit)
...
async function _addDetailsToBroadcastError (e) {
...
function _payload (tx) {
  ...
  try {
    ...
    _assert(Array.isArray(payload.cmds), badPayloadJSON)
    ...
    _assert(!payload.cmds.some(hash => typeof hash !== 'object'), badPayloadJSON)
...
async function calculateStates (commit, importLimit) {
  const states = new Map()
  const jigs = commit._outputs.concat(commit._deletes)

  for (const jig of jigs) {
    const state = await calculateState(commit, jig, importLimit)
