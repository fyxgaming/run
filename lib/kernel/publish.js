...
const { _assert, _text, _bsvNetwork, _Timeout, _kernel, _defined } = require('../util/misc')
...
const { _getMetadataVersion } = require('../util/version')
...
async function getParents (tx, record, locks, feePerKb) {
  ...
  const rawTransactions = await Promise.all(prevtxids.map(prevtxid => _kernel()._blockchainAPI().fetch(prevtxid)))
