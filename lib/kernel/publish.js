...
const { _assert, _text, _bsvNetwork, _Timeout, _kernel, _defined } = require('./misc')
...
async function assignInitialOwners (commit) {
  ...
  async function generateInitialOwners () {
    ...
    for (let i = 0; i < record._creates._size; i++) {
      ...
      const owner = needsOwner && await commit._kernel._ownerWrapper().nextOwner()
...
async function generateOutputScripts (commit) {
  const bsvNetwork = _bsvNetwork(commit._kernel._blockchainWrapper().network)
...
async function payForTx (tx, commit, feePerKb) {
  ...
  const paidhex = await commit._kernel._purseWrapper().pay(rawtx, parents)
...
async function getParents (tx, record, locks, feePerKb) {
  ...
  const rawTransactions = await Promise.all(prevtxids.map(prevtxid => _kernel()._blockchainWrapper().fetch(prevtxid)))
...
async function signTx (tx, commit, feePerKb) {
  ...
  const rawtx = tx.toString('hex')
  const signedhex = await commit._kernel._ownerWrapper().sign(rawtx, parents, locks)
...
async function broadcastTx (commit, tx, timeout) {
  ...
  await commit._kernel._purseWrapper().broadcast(tx.toString('hex'))

  // Broadcast to the blockchain
  try {
    txid = await commit._kernel._blockchainWrapper().broadcast(tx.toString('hex'))
  } catch (e) {
  ...
  if (eString.indexOf('Missing inputs') !== -1 || eString.indexOf('txn-mempool-conflict') !== -1) {
    ...
    for (const input of tx.inputs) {
      try {
        ...
        const prevspend = await commit._kernel._blockchainWrapper().spends(prevtxid, prevvout)
...
async function cacheStates (commit, states, txid) {
  ...
  record._outputs._forEach((jig, i) => {
    ...
    promises.push(commit._kernel._cacheWrapper().set(key, state))
  })
