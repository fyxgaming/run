...
async function _publish (commit) {
  try {
    ...
    await preverify(kernel, commit, states, payload, partialtx, timeout)
    timeout._check()
    ...
    const txid = await PURSE_SAFETY_QUEUE._enqueue(async () => {
      ...
      checkTx(signedtx, commit, partialtx)
...
async function assignInitialOwners (commit) {
  ...
  // Every new creation may potentially require an owner
  for (let i = 0; i < commit._creates.length; i++) {
    ...
    // Check if we need to assign a new owner
    if (!needsOwner) continue

    // Get a new owner
    const owner = await commit._kernel._ownerAPI().nextOwner()

    // Clone the owner for use in sandboxed code
    const makeCode = x => typeof x === 'function' ? Editor._lookupOrCreateCode(x) : undefined
    const sandboxedOwner = _deepClone(owner, SI, makeCode)

    // The lock, and any updated references, need to be added as refs.
    // When script() is called, if it reads anything, those need to be recorded.
    // This is not the right approach. These need to be assigned earlier. Damn.

    // Assign the owner initially as unbound so that any additional logic, such as
    // finalizeOwnersAndSatoshis(), can determine which owners are new.
    state._props.owner = new Unbound(sandboxedOwner)
  }

  // Make sure all owners are already deployed
  const owners = commit._creates.map(jig => commit._after.get(jig)._props.owner)
  _sudo(() => _deepVisit(owners, x => {
    if (x instanceof Creation) {
      const location = _sudo(() => x.location)
      const { txid, native } = _location(location)
      if (!txid && !native) throw new StateError(`${x.name} must first be deployed`)
    }
  }))
}
...
function createMasterList (commit) {
  let masterList = _addCreations(commit._inputs, commit._refs)
  masterList = _addCreations(masterList, commit._creates)
...
async function preverify (kernel, commit, states, payload, partialtx, timeout) {
  if (kernel._preverify) {
    ...
    try {
      ...
      const preverify = new _Preverify(commit, states)
...
async function payForTx (tx, commit) {
  ...
  const locks = getInputLocks(commit)
  const parents = await getParents(tx, commit, locks)
...
async function getParents (tx, commit, locks) {
...
function checkTx (tx, commit, partialtx) {
