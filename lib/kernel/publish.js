...
async function _publish (commit) {
  try {
    ...
    const partialtx = createPartialTx(commit, payload, outputScripts)
...
async function generateOutputScripts (commit) {
  const Unbound = require('../util/unbound')
  const { _OwnersAction } = require('./action')
  ...
  try {
    Record._CURRENT_RECORD = commit._record
    const action = new _OwnersAction()
    commit._record._push(action)
    scripts = commit._record._outputs
      .map(creation => commit._after.get(creation))
      .map(after => after._props.owner)
      .map(owner => owner instanceof Unbound ? owner._value : owner)
      .map(owner => _owner(owner, allowNullOwner, bsvNetwork))
...
function finalizeOwnersAndSatoshis (commit) {
  ...
  record._deletes.forEach(jig => {
    _sudo(() => {
      ...
      if (props.owner instanceof Unbound) props.owner = props.owner._value
      if (props.satoshis instanceof Unbound) props.satoshis = _defined(props.satoshis._value) ? props.satoshis._value : 0

      if (jig.owner instanceof Unbound) jig.owner = jig.owner._value
      if (jig.satoshis instanceof Unbound) jig.satoshis = _defined(jig.satoshis._value) ? jig.satoshis._value : 0

      _assert(jig.owner === null)
      _assert(jig.satoshis === null)
    })
  })
}
...
function createPartialTx (commit, payload, outputScripts) {
