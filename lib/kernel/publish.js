...
async function checkNoTimeTravel (commit, timeout) {
  ...
  for (const jig of commit._refs) {
    const bindings = commit._bindings.get(jig)

    if (!(bindings.origin in refmap)) continue

    const refmapNonce = refmap[bindings.origin][1]
    _checkState(bindings.nonce >= refmapNonce, `Time travel for ${_text(jig)}`)
  }
}
...
function createPayload (commit, hashes, exec, masterList) {
  ...
  const ref = commit._refs.map(jig => commit._bindings.get(jig).location)
...
function createPartialTx (commit, payloadJson) {
  ...
  for (const jig of commit._inputs) {
    ...
    const satoshis = bindings.satoshis
...
async function preverify (kernel, commit, states, payload, partialtx, timeout) {
  if (kernel._preverify) {
    ...
    try {
      ...
      const preverify = new _replay._Preverify(commit, states)
...
function getLocks (commit) {
  const locks = commit._inputs
    .map(jig => commit._bindings.get(jig))
...
async function getParents (tx, commit) {
  ...
  const jigSatoshis = commit._inputs
    .map(jig => commit._bindings.get(jig))
    .map(snapshot => Math.max(snapshot.satoshis, Transaction.DUST_AMOUNT))
...
function checkTx (tx, commit) {
  commit._inputs.forEach((jig, i) => {
    ...
    const bindings = commit._bindings.get(jig)
    const line1 = `origin: ${bindings.origin}`
    const line2 = `location: ${bindings.location}`
    const line3 = `owner: ${bindings.owner}`
    const details = `${line1}\n${line2}\n${line3}`
