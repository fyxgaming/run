...
async function _publish (commit) {
  try {
    ...
    const partialtx = createPartialTx(commit, payload, outputScripts, bsv.Transaction.DUST_AMOUNT)
...
function createPartialTx (commit, payload, outputScripts, dustAmount) {
  ...
  record._outputs.forEach((jig, i) => {
    const after = commit._after.get(jig)
    const satoshis = Math.max(after._props.satoshis, dustAmount)
