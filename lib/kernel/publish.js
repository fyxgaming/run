...
function createExec (record, masterList) {
  ...
  return record._actions.map(action => {
    const op = action.op()
    const data = _encode(action.data(), encodeOptions)
    return { op, data }
  })
}
...
function createMetadata (commit, hashes, exec, masterList) {
  ...
  const cre = owners.map(owner => _encode(owner, encodeOptions))
...
async function getParents (tx, record, locks, feePerKb) {
  ...
  const rawTransactions = await Promise.all(prevtxids.map(prevtxid => _kernel()._fetch(prevtxid)))
...
async function broadcastTx (commit, tx, timeout) {
  ...
  if (commit._kernel._purse.broadcast) {
  ...
  if (commit._kernel._state.broadcast) {
