...
class Purse {
  ...
  /**
   * Signs the transaction's purse inputs
   * @param {bsv.Transaction} tx Transaction to pay for
   * @returns {bsv.Transaction} Signed transaction
   */
  async sign (tx) { throw new NotImplementedError() }

  /**
   * Optionally broadcasts the transaction
   * @param {bsv.Transaction} tx Transaction to broadcast
   */
  async broadcast (tx) { throw new NotImplementedError() }
...
class Owner {
  /**
   * The address controlled by this owner API
   *
   * Either this must be specified, or locks must be specified.
   *
   * This will be used to create new resources.
   * @returns {string} Array of addresses, public keys, or custom locks
   */
  get address () { throw new NotImplementedError() }
  ...
  /**
   * The locks signable by this Owner class.
   *
   * Either this must be specified, or locks must be specified.
   *
   * The first entry will also be used to create new resources.
   * @returns {Array<string|Lock>} Array of addresses, public keys, or custom locks
   */
  get locks () { throw new NotImplementedError() }
  ...
  static [Symbol.hasInstance] (instance) {
    if (typeof instance !== 'object' && typeof instance !== 'function') return false
    if (!instance) return false
    const hasLocks = Array.isArray(instance.locks) && instance.locks.length
    if (!hasLocks && typeof instance.address !== 'string') return false
