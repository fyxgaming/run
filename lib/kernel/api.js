...
class Purse {
   ...
   * @param {Array<{script: string, satoshis: number}>} parents Array of spent UTXOs spent in this
   *    tx mapped 1-1 with transaction inputs
...
class Owner {
   ...
   * @param {string} rawtx Transaction to sign in serialized hex format
   * @param {Array<?{script: string, satoshis: number, lock: ?Lock}>} parents Array of UTXOs spent
   *    in this tx mapped 1-1 with transaction inputs. If a UTXO is undefined, then Run doesn't
   *    know about that input and it's not relevant to this method. Each jig UTXO will have a lock
   *    assigned to it. This lock is the owner of the jig and it is essentially a higher-level
   *    representation of the script.
   * @returns {string} Signed transaction in raw hex format
   */
  async sign (rawtx, parents) { throw new NotImplementedError() }
