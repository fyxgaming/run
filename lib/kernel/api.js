...
class Purse {
   ...
   * @param {string} txhex Partial transaction to pay for in hex
   * @param {number} remaining Estimated size of remaining unlocking scripts not included
   * @returns {string} Paid transaction in hex
   */
  async pay (txhex, remaining) { throw new NotImplementedError() }
...
class Lock {
   ...
   *
   * This method is optional. If not specified, then a default of 1000 is used.
   * @returns {number} Estimate of the script size in bytes
   */
  unlockSize () { throw new NotImplementedError() }
  ...
  static [Symbol.hasInstance] (instance) {
    ...
    // Make sure unlockSize is a function or undefined
    const unlockSize = instance.constructor.prototype.unlockSize
    if (typeof unlockSize !== 'function' && typeof unlockSize !== 'undefined') return false

    // Make sure unlockSize is not otherwise defined on the object
    if (Object.getOwnPropertyNames(instance).includes('unlockSize')) return false

    // Make sure unlockSize returns a number
    if (unlockSize && typeof instance.unlockSize() !== 'number') return false
