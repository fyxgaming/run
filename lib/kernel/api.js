...
class Owner {
  ...
  /**
   * An optional method to return the locations of all owned tokens.
   *
   * If not present, then the UTXOs from next() are used.
   * @returns {Array<string>} Token locations
   */
  async locations () { throw new NotImplementedError() }

  /**
   * An optional method to return whether a give lock is owned by this owner
   *
   * This determines whether it goes into the inventory. If not present, then next() is matched.
   * @param {Lock} lock Token lock
   * @returns {boolean} Whether this token is ours or not
   */
  async ours (lock) { throw new NotImplementedError() }
