...
class Blockchain {
  ...
  /**
   * Returns whether instance is a valid implementation of Blockchain
   */
...
class Purse {
  ...
  /**
   * Returns whether instance is a valid implementation of Purse
   */
  static [Symbol.hasInstance](instance) {
    if (typeof purse !== 'object' || !purse) return false
    if (typeof purse.pay !== 'function') return false
    return true
  }
}
...
class Logger {
  ...
  /**
   * Returns whether instance is a valid implementation of Logger
   */
  static [Symbol.hasInstance](instance) {
    return typeof logger === 'object' && logger
  }
}
...
class State {
  ...
  /**
   * Returns whether instance is a valid implementation of State
   */
  static [Symbol.hasInstance](instance) {
    if (typeof instance !== 'object' || !state) return false
 ...
 * 
 * Locks may be assigned as owners on tokens to give them non-standard ownership rules. They may
 * be created inside jigs, or passed as arguments to a method. For example:
 * 
 *    token.send(new GroupLock(pubkeys, 2))
 * 
...
class Lock {
  ...
  /**
   * Returns whether instance is a valid implementation of Lock
   */
  static [Symbol.hasInstance](instance) {
    if (!typeof instance !== 'object' || !instance) return false
...
class Owner {
   ...
   */
  async next () { throw new NotImplementedError() }

  /**
   * Already assumes it is authorized. Don't need tokens/actions here.
   * @param {bsv.Transaction} tx Transaction to sign
   * @param {Array<?Lock>} locks Locks for each input if they are tokens
   */
  async sign (tx, locks) { throw new NotImplementedError() }

  /**
   * Returns whether instance is a valid implementation of Key
   */
  static [Symbol.hasInstance](instance) {
    if (typeof instance !== 'object' || !key) return false
