...
/**
 * API that Run uses to fetch derived state
 */
class State {
  /**
   * Returns the state of a jig, code, or berry
   * @param {string} location Creation location. This should be a location without hash if a berry.
   * @returns {?object} JSON state or undefined if not exist
   */
  async state (location) { throw new NotImplementedError() }

  /**
   * Returns whether a particular txid should be trusted
   * @param {string} txid Txid to check
   * @returns {boolean} True if the txid is trusted, false if not or unknown
   */
  async trusted (txid) { throw new NotImplementedError() }

  /**
   * Returns whether a particular txid should never be loaded
   * @param {string} txid Txid to check
   * @returns {boolean} True if the txid is banned, false if not or unknown
   */
  async banned (txid) { throw new NotImplementedError() }
}
