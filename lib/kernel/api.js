...
class Purse {
   ...
   * @param {string} tx Transaction to sign in hex format
   * @param {Array<Utxo>} utxos Array of UTXOs used in this tx mapped 1-1 with transaction inputs
   * @returns {string} Paid transaction in hex format
   */
  async pay (tx, utxos) { throw new NotImplementedError() }
...
class Owner {
   ...
   * @param {string} tx Transaction to sign in hex format
   * @param {Array<?Utxo>} utxos Array of UTXOs used in this tx mapped 1-1 with transaction inputs.
   *    If a UTXO is undefined, then Run doesn't know about that input and it's not relevant to
   *    this method. Each jig UTXO will have a lock assigned to it. This lock is the owner of the
   *    jig and it is essentially a higher-level representation of the script.
   * @returns {string} Signed transaction in hex format
   */
  async sign (tx, utxos) { throw new NotImplementedError() }
...
/**
 * An unspent output that will be the input of another transaction
 */
class Utxo {
  /**
   * @returns {string} Transaction hash as a hex string
   */
  get txid () { throw new NotImplementedError() }

  /**
   * @returns {number} Output index
   */
  get vout () { throw new NotImplementedError() }

  /**
   * @returns {number} Amount of satoshis in the output
   */
  get satoshis () { throw new NotImplementedError() }

  /**
   * @returns {string} Locking script in hex
   */
  get script () { throw new NotImplementedError() }

  /**
   * @returns {?Lock} Optional jig lock associated with this output
   */
  get lock () { throw new NotImplementedError() }

  /**
   * @returns {boolean} Whether instance is a valid implementation of Utxo
   */
  static [Symbol.hasInstance] (instance) {
    if (typeof instance !== 'object') return false
    if (!instance) return false
    if (typeof instance.txid !== 'string') return false
    if (typeof instance.vout !== 'number') return false
    if (typeof instance.satoshis !== 'number') return false
    if (typeof instance.script !== 'string') return false
    if (!((typeof instance.lock === 'object' && instance.lock) ||
      typeof instance.lock === 'undefined')) return false
    return true
  }
}
...
module.exports = { Blockchain, Purse, Logger, State, Lock, Owner, Utxo }
