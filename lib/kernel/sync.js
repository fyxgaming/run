...
const { _assert, _kernel, _text, _Timeout, _defined } = require('../util/misc')
...
async function _sync (jig, options = {}) {
  ...
  if (Transaction._ATOMICALLY_UPDATING) throw new StateError('sync disabled in atomic update')
...
async function publishPending (jig, options) {
  ...
  while (true) {
    ...
    if (_defined(loc._error)) throw new StateError(`Cannot sync\n\n${loc._error}`)
...
async function forwardSync (jig, options) {
  ...
  const { _metadata } = require('./loader')
  ...
  while (true) {
    ...
    if (_defined(loc._record)) throw new StateError(`Cannot sync ${_text(jig)}: transaction in progress`)
    _assert(_defined(loc._txid) && _defined(loc._vout))

    const spendtxid = await kernel._blockchainAPI().spends(loc._txid, loc._vout)
    if (!spendtxid) break
    options._timeout._check()
