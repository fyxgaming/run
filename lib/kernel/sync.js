...
const { _assert, _kernel, _text, _Timeout, _defined } = require('./misc')
...
async function forwardSync (jig, options) {
  ...
  while (true) {
    ...
    const spendtxid = await kernel._blockchainWrapper().spends(loc._txid, loc._vout)
    if (!spendtxid) break
    options._timeout._check()

    if (Log._infoOn) Log._info(TAG, 'Forward syncing to', spendtxid)

    const rawSpendTx = await kernel._blockchainWrapper().fetch(spendtxid)
    options._timeout._check()
