...
const { _assert, _kernel, _text, _Timeout } = require('../util/misc')
...
async function publishPending (jig, options) {
  ...
  while (true) {
    ...
    if (error) throw new StateError(`Cannot sync\n\n${error}`)
...
async function forwardSync (jig, options) {
  ...
  while (true) {
    ...
    if (loc.recordid) throw new StateError(`Cannot sync ${_text(jig)}: transaction in progress`)
