...
async function _sync (jig, options = {}) {
  ...
  // Setup the options
  options.forward = typeof options.forward === 'undefined' ? true : options.forward
  options.inner = typeof options.inner === 'undefined' ? true : options.inner
  ...
  if (publishError) throw publishAndSyncForward
}
...
async function innerSync (jig, options) {
  ...
  if (options.forward) _sudo(() => _deepReplace(jig, x => dedups.get(x)))
}
