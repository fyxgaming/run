...
async function publishPending (jig, options) {
  ...
  while (true) {
    ...
    if (_defined(loc.error)) throw new StateError(`Cannot sync\n\n${loc.error}`)
    if (!_defined(loc.commit)) break
...
async function forwardSync (jig, options) {
  ...
  while (true) {
    ...
    if (_defined(loc.record)) throw new StateError(`Cannot sync ${_text(jig)}: transaction in progress`)
    _assert(_defined(loc.txid) && _defined(loc.vout))
