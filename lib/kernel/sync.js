...
async function _sync (jig, options = {}) {
  ...
  // Also dedup our inner forward syncs by origin
  options._forwardSyncs = options._forwardSyncs || new Map()
...
async function forwardSync (jig, options) {
  ...
  const origin = _sudo(() => jig.origin)

  const existing = options._forwardSyncs.get(jig) || options._forwardSyncs.get(origin)
  if (existing) {
    console.log('EXISTS')
    return
  }

  const promise = forwardSyncInternal(jig, options)

  options._forwardSyncs.set(jig, promise)
  options._forwardSyncs.set(origin, promise)
...
async function forwardSyncInternal (jig, options) {
  ...
  while (true) {
    ...
    await _import(spendtx, spendtxid, payload, kernel, published, jigToSync, options._timeout, preverify)
    options._timeout._check()
