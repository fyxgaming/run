...
function _unifyForImport (inputs, refs, jigToSync) {
  const Berry = require('./berry')
  const getOrigin = x => _sudo(() => x instanceof Berry ? x.location : x.origin)
  ...
  incoming.forEach(x => {
    ...
    if (y) {
      const line1 = `First location: ${x.location}`
      const line2 = `Second location: ${y.location}`
  ...
  // 3) Get the latest versions of every referenced jigs
