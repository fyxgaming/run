...
const { _text, _checkState, _assert, _hasOwnProperty } = require('../util/misc')
...
function _unifyForMethod (obj, fixed = []) {
  ...
  const getKey = x => _sudo(() => {
    const locationKey = x instanceof Berry ? 'location' : 'origin'
    if (!_hasOwnProperty(x, locationKey) || x[locationKey].startsWith('error://')) return x
