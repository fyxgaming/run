 ...
 * unify.js
 *
 * Unify: make jigs that interact together all use the same jigs in their latest common locations
 *
 * Unifification happens automatically. When a user calls a method:
 *
 *    a.f(b, c)
 *
 * then a, b, c, and also all their inner references, are unified. Similarly for deploy, etc.
 *
 ...
 * but whatever is in those other jigs are not part of the base jig state.
 *
 ...
 * updated, those indirect jigs should stay unified after the method is complete. They
 * must revert to their former state as it was referenced by the jigs before the method.
 *
 * This process, called de-unification, is used during imports. It's not crucial during user
 * method calls. Also, during imports, we only unify the inputs and refs once during load.
 * We don't have to unify with every action if we know they were all unified at the beginning.
 */
