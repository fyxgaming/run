...
class Inventory {
  ...
  async _syncLatest () {
    const locations = []
    const owners = Array.isArray(this._owner.owner) ? this._owner.owner : [this._owner.owner]
...
function sameLock (a, b) {
  if (a.script.length !== b.script.length) return false
  for (let i = 0; i < a.script.length; i++) {
    if (a.script[i] !== b.script[i]) return false
  }
  return true
}
