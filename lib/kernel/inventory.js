...
/**
 * Tracks the collection of up-to-date tokens for the current owner.
 *
...
class Inventory {
  ...
  get resources () {
    ...
    for (const token of this._tokens) {
      if (errored(token) || !ours(this._owner, token)) {
        remove.push(token)
      } else {
        tokens.push(token)
      }
  ...
  async _syncLatest () {
    ...
    // Remove tokens that are not in our new locations, but have a value
