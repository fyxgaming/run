...
const { _resourceType, _lockify } = require('../util/misc')
...
class Inventory {
  constructor (kernel) {
    ...
    this._resources = new TokenSet()
  }

  /**
   * @returns {Array<Jig>} Jigs owned by the current owner. Call sync() to update.
   */
  get jigs () { return this.resources.filter(token => _resourceType(token) === 'jig') }

  /**
   * @returns {Array<function>} Code owned by the current owner. Call sync() to update.
   */
  get code () { return this.resources.filter(token => _resourceType(token) === 'code') }
  ...
  get resources () {
    ...
    remove.forEach(resource => this._resources.delete(resource))
  ...
  async _syncLatest () {
    ...
    const toRemove = []
    const existingTokens = JigControl._disableSafeguards(() => {
      const arr = []
      for (const token of this._resources) {
        if (token.location.startsWith('_')) {
          arr.push(token)
        } else if (errored(token) || !ours(this._owner, token)) {
          toRemove.push(token)
        } else if (!newLocationsSet.has(token.location)) {
          toRemove.push(token)
        } else {
          arr.push(token)
        }
    ...
    toRemove.forEach(resource => this._resources.delete(resource))

    const existingLocationsSet = new Set(JigControl._disableSafeguards(
      () => existingTokens.map(token => token.location)))

    for (const location of newLocations) {
      if (existingLocationsSet.has(location)) continue
