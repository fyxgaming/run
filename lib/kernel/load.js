...
const { _assert, _extendsFrom, _text, _Timeout, _defined } = require('../util/misc')
const { _deterministicJSONStringify } = require('../sandbox/determinism')
...
async function load (location, BerryClass, kernel, session, timeout, complete = true) {
  ...
  timeout = timeout || new _Timeout('load', kernel._timeout)
...
async function _loadBerryFromLocation (location, kernel, session, timeout) {
  const loadFunction = async () => {
    ...
    const state = await kernel._cacheAPI().get(key)
...
async function _loadJigFromState (location, state, kernel, session, timeout) {
  ...
  if (!kernel._trustlist.has('cache')) {
    // Get the transaction and metadata if we are loading a non-berry
    const rawtx = await kernel._blockchainAPI().fetch(txid)
...
async function _pluckBerry (path, hash, version, B, kernel, session, timeout) {
  ...
  try {
    ...
    if (berryClassDeployed) {
      ...
      const key = `berry://${partialLocation}`
      await kernel._cacheAPI().set(key, state)
    }
...
async function _loadFromLocationWithBanCheck (location, loadFunction, kernel, session, timeout) {
  ...
  let bannedValue
  try {
    bannedValue = await kernel._cacheAPI().get(`ban://${location}`)
  } catch (e) {
  ...
  if (banned) {
    const dueToTrust = bannedValue.untrusted && await kernel._trusted(bannedValue.untrusted)
    ...
    if (unban) {
      try {
        await kernel._cacheAPI().set(`ban://${location}`, false)
      } catch (e) {
  ...
  try {
    ...
    if (e instanceof ExecutionError || e instanceof TrustError) {
      try {
        ...
        await kernel._cacheAPI().set(`ban://${location}`, value)
      } catch (e) {
