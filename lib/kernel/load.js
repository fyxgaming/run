...
const { _assert, _parentName, _text } = require('../util/misc')
...
class Bundle {
  ...
  async _initialize (location) {
    ...
    if (loc.native) {
      const NativeCode = Repository._native()._getByLocation(loc.native)
      ...
      this._jig = NativeCode
      return
    }

    // Get the on-chain tx
    const rawtx = await this._options._kernel._blockchain.fetch(loc.txid)
    ...
    // Get the state cache
    const state = await this._options._kernel._cache.get(`jig://${location}`)
...
async function _load (location, options = {}) {
  Log._info(TAG, 'Load', location)

  // TODO
  if (options._BerryClass) {
    const B = Repository._active()._install(options._BerryClass)
    return B.pluck(location)
  }

  const bundle = await _partialLoad(location, options)

  return await _completeLoad(bundle)
}
...
async function _partialLoad (location, options = {}) {
  ...
  if (!bundle._jig) {
    if (bundle._state) {
      await _partialLoadFromState(bundle)
    } else {
      await _loadViaReplay(bundle)
    }
  }
...
// ------------------------------------------------------------------------------------------------
// _partialLoadFromState
// ------------------------------------------------------------------------------------------------

async function _partialLoadFromState (bundle) {
  const { _hasVout, _hasVdel, _payload, _txid, _vout, _vdel, _state, _options } = bundle

  Log._debug(TAG, 'Recreating from cached state')
  ...
  const refs = new Set()
  // Dummy jig is a class so that setPrototypeOf works
  const dummyJig = x => { class A {}; A.location = x; return A }
  const codec = new Codec()._loadJigs(x => { refs.add(x); return dummyJig(x) })
  ...
  if (decodedState.kind === 'code') {
    ...
    if (parentName) {
      ...
      if (!_options._bundles.has(parentFullLocation)) {
        await _partialLoad(parentFullLocation, _options)
      }
...
async function _completeLoad (bundle) {
  ...
  if (bundle._partial) {
    ...
    for (const ref of _refs) {
      ...
      await _partialLoad(refFullLocation, _options)
    }
    ...
    const promises = Array.from(_options._bundles.values())
      .map(bundle => _completeLoad(bundle))
...
async function _loadViaReplay (bundle) {
  ...
  const record = await _import(_tx, _payload, true /* published */, null /* jigToSync */, _options._importLimit)
...
function load (_kernel, location, _BerryClass, _importLimit) {
  return _load(location, { _BerryClass, _importLimit, _kernel })
}
