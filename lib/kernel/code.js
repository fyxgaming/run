...
class Editor {
  _init (C) {
    this._T = undefined
    this._C = C
    this._network = _kernel()._blockchain._network
  ...
  _installNative (T, internal = false) {
    ...
    const S = this._sandbox(T, null, null)
...
function lookupByType (T) {
  ...
  const network = _kernel()._blockchain._network
...
Code._editor = C => CODE_EDITOR.get(C)
