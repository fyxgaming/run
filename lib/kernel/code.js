...
class CodeDeps {
  ...
  static get _Snapshot () { return require('../util/snapshot') }
...
class Code {
  ...
  upgrade (T, local = true, src = undefined) {
    ...
    const Editor = CodeDeps._Editor
    const Log = CodeDeps._Log
    const { _text, _hasOwnProperty } = CodeDeps._misc
    const Record = CodeDeps._Record
    const Snapshot = CodeDeps._Snapshot
    const _sudo = CodeDeps._sudo
    const TAG = CodeDeps._TAG
    ...
    const upgradable = _sudo(() => !_hasOwnProperty(this, 'upgradable') || this.upgradable)
    if (!upgradable) throw new Error(nonupgradableError)
    ...
    try {
      ...
      Record._CURRENT_RECORD._capture(() => {
        ...
        if (newCode.length) Editor._deployMultiple(...newCode)
  ...
  auth () {
    const Action = CodeDeps._Action
    const Editor = CodeDeps._Editor
    const { _text, _hasJig } = CodeDeps._misc
    const Record = CodeDeps._Record
    const Log = CodeDeps._Log
    const TAG = CodeDeps._TAG
  ...
  [Symbol.hasInstance] (x) {
    ...
    const C = Editor._lookupByType(this)
  ...
  static [Symbol.hasInstance] (x) {
    const Creation = CodeDeps._Creation
...
const NativeCode = CodeDeps._Editor._newCode()
