...
class Code {
  ...
  constructor (T, local = true, newCode = []) {
    ...
    membrane._init(Membrane._KIND_CODE, C)
...
function checkType (T) {
  ...
  _checkArgument(!_isAnonymous(T), `Anonymous functions and classes cannot be jigs: ${_text(T)}`)
  _checkArgument(!T.prototype || T.prototype.constructor === T, `Prototypal inheritance not supported: ${_text(T)}`)
  _checkArgument(T.toString().indexOf('[native code]') === -1, `Cannot install native code: ${_text(T)}`)
}
