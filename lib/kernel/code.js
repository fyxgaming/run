...
class Code {
  ...
  deploy (T, options = {}) {
    ...
    Log._info(TAG, 'Deploy', _display(T))
    ...
    if (run.blockchain.network !== this._network) {
      ...
      throw new Error(`Cannot deploy ${_display(T)} on the current network\n\n${hint}`)
    }
  ...
  async _installFromTx (def, location, tx, kernel, bsvNetwork, _partiallyInstalledCode = new Map()) {
    ...
    Log._info(TAG, 'Installing', _display(def.text), 'from', location)
