...
const {
  ...
  _deanonymizeSourceCode, _RESERVED_PROPS
} = require('../util/misc')
...
// Jig methods that cannot be overridden
const NONOVERRIDABLE_JIG_METHODS = ['auth', 'destroy', 'sync', 'toString', Symbol.hasInstance]
...
class Code {
  ...
  toString () {
    ...
    // Get the source code
    const D = editor._D
    const src = _deanonymizeSourceCode(D.toString(), editor._T)
...
class Editor {
  ...
  _install (T, local = true, newCode = []) {
    ...
    try {
      ...
      if (S.prototype.init) {
      // console.log(T.name, Object.getOwnPropertyNames(S.prototype))
        const Membrane = require('./membrane')
        const rules2 = Rules._childProperty(this._C, true, true)
        S.prototype.init = new Membrane(S.prototype.init, rules2)
      }
      // console.log(rules2)
...
function checkNoOverrideJigMethods (T) {
  ...
  NONOVERRIDABLE_JIG_METHODS.forEach(prop => {
