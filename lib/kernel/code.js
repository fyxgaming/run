...
class Code {
  toString () {
    const { _Repository } = Context
    return _Repository._getSourceCode(this)
  }

  async sync (options = {}) {
    const {
      _bsv, _TAG, _assert, _Membrane, _Bindings, _Log, _Repository, _Record, _text, _kernel,
      _deepVisit, _Jig, _Importer
    } = Context
    ...
    await _Repository._deploy(this)

    // Create an import limit
    options._importLimit = new _Importer._ImportLimit()
    ...
    while (true) {
      ...
      const record = await _import(spendTx, payload, true /* published */, this /* jigToSync */, options._importLimit)
