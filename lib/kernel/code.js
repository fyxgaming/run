...
const { _networkSuffix, _deployable, _tokenType, _ownerLock, _activeRun } = require('../util/misc')
...
class Code {
  ...
  async installFromTx (def, location, tx, kernel, bsvNetwork, partiallyInstalledCode = new Map()) {
    ...
    const hex2 = Buffer.from(_ownerLock(def.owner, bsvNetwork).script).toString('hex')
