...
class Code {
  ...
  sync (options = {}) {
    ...
    if (require('./transaction')._ATOMICALLY_UPDATING) throw new Error('sync disabled during atomic update')
  ...
  upgrade (T, local = true, src = undefined) {
    ...
    const nonupgradableError = `${_text(this)} is non-upgradable`
    _checkState(_sudo(() => !_hasOwnProperty(this, 'upgradable') || this.upgradable), nonupgradableError)
...
class Editor {
      ...
      if (_hasOwnProperty(T, 'presets')) {
        ...
        _deepVisit(this._C, x => {
          if (x instanceof Code && x !== this._C) {
            const error = `${_text(x)} must have presets`
            _checkArgument(!_location(x.location).undeployed, error)
          }
        })
  ...
  _setupBehavior (T, local = false, newCode = []) {
    ...
    const upgradeFromBerryError = `Cannot upgrade from berry class: ${_text(this._T)}`
    const upgradeToBerryError = `Cannot upgrade to berry class: ${_text(T)}`
    _checkState(!(this._installed && _extendsFrom(this._T, Berry)), upgradeFromBerryError)
    _checkState(!(this._installed && _extendsFrom(T, Berry)), upgradeToBerryError)
...
function checkSealedOption (value) {
  const badSealed = `Invalid sealed option: ${value}`
  _checkArgument(value === true || value === false || value === 'owner', badSealed)
}

// ------------------------------------------------------------------------------------------------

function checkUpgradableOption (value) {
  const badUpgradable = `Invalid upgradable option: ${value}`
  _checkArgument(value === true || value === false, badUpgradable)
}
...
function checkValidJigClass (T) {
  ...
  _checkArgument(!badWord, `Must not have any reserved jig words: ${badWord}`)
...
function checkValidBerryClass (T) {
  ...
  _checkArgument(!badWord, `Must not have any reserved berry words: ${badWord}`)
...
function checkUpgradable (T, editor) {
  ...
  _checkState(origin !== Bindings._UNDEPLOYED_LOCATION, 'Cannot upgrade undeployed code')
}
...
Code._makeDeps = makeDeps
Code._preinstall = preinstall
Code._editor = C => EDITOR.get(lookupByType(C))
Code._checkSealedOption = checkSealedOption
Code._checkUpgradableOption = checkUpgradableOption
