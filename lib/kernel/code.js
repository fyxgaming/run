...
class Bundle {
  ...
  _addParent (T, compartment) {
    const Parent = parent(T, this._code)
    if (!Parent) return

    const ParentSandbox = this._add(Parent)

    const deps = prop(T, 'deps') || {}
  ...
  _add (T) {
    ...
    const compartment = this._code._realm.makeCompartment()

    this._addParent(T, compartment)

    const Sandbox = compartment.evaluate(code(T))
...
class Code {
  constructor () {
    ...
    this._realmIntrinsics = realmIntrinsics(this._realm)

    // Make for a good test in sandbox?
    const keys = new Set()
    Object.keys(this._intrinsics).forEach(name => keys.add(name))
    Object.keys(this._realmIntrinsics).forEach(name => keys.add(name))
    for (const name of keys) {
      if (!!this._intrinsics[name] !== !!this._realmIntrinsics[name]) {
        console.log(name, !!this._intrinsics[name], !!this._realmIntrinsics[name])
      }
    }
  }
...
const getIntrinsicCode = name => `typeof ${name} !== 'undefined' ? ${name} : null`

// Returns an object with the built-in intrinsics in this environment
function intrinsics () {
  let code = 'const x = {};'
  intrinsicNames.forEach(name => { code += `x.${name} = ${getIntrinsicCode(name)};` })
  code += 'return x'
  return new Function(code)() // eslint-disable-line
}

function realmIntrinsics (realm) {
  const compartment = realm.makeCompartment()
  const ret = {}
  for (const name of intrinsicNames) {
    ret[name] = compartment.evaluate(getIntrinsicCode(name))
  }
  return ret
}
