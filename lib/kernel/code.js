...
class Code {
  constructor (network) {
    ...
    this._installs = new Map() // T | S | location -> S
  ...
  isSandboxed (T) {
    const installed = this._installs.get(T)
    return installed && T === installed
  }

  _getSandboxed (query) {
    if (this.isSandboxed(query)) return query
    return this._installs.get(query)
  }
   ...
   */
  deploy (T, options = {}) {
    // If we are already deploying this code, return the sandbox
    if (this._pending.has(T)) return this._installs.get(T)

    // Short-circut deployment at Jig and Berry because they are built-in
    if (T === this.Jig || T === Jig || T.name === 'Jig') return this.Jig
    if (T === this.Berry || T === Berry || T.name === 'Berry') return this.Berry

    // Check that this code can be deployed
    if (!_deployable(T)) throw new Error(`${T} is not deployable`)

    // Check if this type was already deployed
    const SPrev = this._installs.get(T)
    ...
    // We'll definitely be deploying this class
    ...
    if (classProps.includes(`location${net}`)) {
      const S = this._installs.get(T[`location${net}`])
      if (S) return S
    }
