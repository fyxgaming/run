...
class Editor {
  ...
  _install (T, local = true, newCode = []) {
    ...
    try {
      ...
      const Membrane = require('./membrane')
      this._membrane._init(Membrane._KIND_CODE, this._C, this._D)
  ...
  _installNative (T, internal = false) {
    ...
    // Configure the membrane for the native code
    const Membrane = require('./membrane')
    this._membrane._init(Membrane._KIND_NATIVE, this._C, this._D)
