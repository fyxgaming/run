...
const RESERVED = [
  // Code methods
  'toString', 'upgrade', 'sync', 'auth', 'destroy',
  // Future properties
...
class Code {
  ...
  constructor (T, local = true, newCode = []) {
    ...
    const Membrane = require('../membrane/membrane')
    const BaseHandler = require('../membrane/base')
    const membrane = new Membrane(new BaseHandler())
   ...
   */
  toString () {
    // Can't get source of a non-code child class
    const editor = Code._editor(this)
    _checkState(editor, 'toString unavailable')
...
class Editor {
  ...
  _install (T, local = true, newCode = []) {
    ...
    try {
      ...
      if (_extendsFrom(T, Jig)) {
        this._membrane._inner = require('../membrane')._JIG_CLASS_MEMBRANE
      } else {
        this._membrane._inner = require('../membrane')._SIDEKICK_CODE_MEMBRANE
      }
  ...
  _installNative (T, internal = false) {
    ...
    this._membrane._inner = require('../membrane')._NATIVE_CODE_MEMBRANE
