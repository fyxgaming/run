...
const { info } = util
const { _fromTokenJson, _tokenizeRefs, _findAllTokenRefsInTokenJson } = require('../util/json')
  ...
  async installFromTx (def, location, tx, kernel, bsvNetwork, partiallyInstalledCode = new Map()) {
    ...
    try {
      ...
      if (def.props) {
        ...
        const refs = _findAllTokenRefsInTokenJson(def.props)
        ...
        const classProps = _fromTokenJson(def.props, {
          _sandboxIntrinsics: this._sandboxIntrinsics,
          _revivers: [_tokenizeRefs(ref => tokens.get(ref))]
        })
