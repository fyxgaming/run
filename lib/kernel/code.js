...
class Repository {
  constructor () {
    // Why does each network get their own sandbox? Because the properties are frozen on it.

    this._sandboxedFuncs = new Map() // network -> Map<Func, SandboxedFunc>

    // Funcs are frozen when they are deployed
  }

  _getSandboxedFunc (func, network) {
  }
}
...
class Code {
  ...
  _deploy (kernel, func, options) {
    ...
    const network = kernel.blockchain.network

    // If this type was already deployed on this network, don't deploy again
    const sandboxedFunc = this._repository._getSandboxedFunc(func, network)
    if (sandboxedFunc) return sandboxedFunc
