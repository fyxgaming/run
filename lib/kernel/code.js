...
class Code {
  ...
  toString () {
    ...
    // Get the source code
    const D = editor._D
    const src = _unsandboxSourceCode(D.toString(), editor._T)
...
class Editor {
  ...
  _install (T, local = true, newCode = []) {
    ...
    try {
      ...
      const S = makeSandbox(this._C, T, local, newCode, ParentCode)
      ...
      // Configure the membrane for the static
      const Jig = require('./jig')
      if (!_extendsFrom(T, Jig)) {
        const Membrane = require('./membrane')
        Membrane._configure(this._C)
      }
...
function makeSandbox (C, T, local, newCode, ParentCode) {
