...
class Code {
  ...
  auth () {
    ...
    const Record = require('./record')
    _checkState(!_hasJig(Record._CURRENT_RECORD._creates, this), 'auth unavailable')
...
class Editor {
  ...
  /**
   * Gets the usable source code for this code
   */
  _source () {
    return _deanonymizeSourceCode(this._D.toString(), this._name)
  }
...
function makeSandbox (C, T, local = false, newCode = undefined) {
  ...
  Object.keys(Sprops.deps || {}).forEach(name => _setOwnProperty(SGlobal, name, Sprops.deps[name]))
