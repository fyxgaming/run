...
class Code {
  ...
  sign () {
    ...
    const Editor = CodeDeps._Editor
    const { _text, _hasCreation } = CodeDeps._misc
    const Record = CodeDeps._Record
    const Log = CodeDeps._Log
    const TAG = CodeDeps._TAG

    if (Log._debugOn) Log._debug(TAG, 'Sign', _text(this))

    // Non-jig child classes and native code cannot be signed. Errors.
    const editor = Editor._get(this)
    if (!editor) throw new Error('sign unavailable on non-jigs')
    if (editor._native) throw new Error('sign unavailable on native jigs')

    // We cannot sign code just created because there is no input
    if (_hasCreation(Record._CURRENT_RECORD._creates, this)) throw new Error('sign unavailable on new jigs')

    // Record a sign action
    Action._sign(this)
