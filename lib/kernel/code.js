...
class Code {
  constructor (kernel) {
    ...
    this.evaluator = new Evaluator(kernel)
    this._sandboxIntrinsics = this.evaluator._sandboxIntrinsics
  ...
  extractProps (type) {
    ...
    try {
      TokenJSON._serialize(props, {
        _sandboxIntrinsics: this._sandboxIntrinsics,
  ...
  deploy (type, options = {}) {
    ...
    info(this._kernel._logger, 'Deploying', type.name)
    ...
    if (!util._deployable(type)) throw new Error(`${type} is not deployable`)
    ...
    try {
      ...
      const SandboxObject = this._sandboxIntrinsics.Object
      ...
      const tokenLoader = ref => {
        ...
        return _tokenType(token) === 'code' ? this.getInstalled(token) : token
      }
      ...
      tokens
        .filter(token => _tokenType(token) === 'code')
  ...
  async installFromTx (def, location, tx, kernel, bsvNetwork, partiallyInstalledCode = new Map()) {
    ...
    const hex2 = Buffer.from(util.ownerScript(def.owner, bsvNetwork).toBytes()).toString('hex')
