...
class Editor {
  ...
  _install (T, local = true, newCode = []) {
    ...
    try {
      ...
      if (_extendsFrom(T, Jig)) {
        this._membrane._inner = Membrane._JIG_CODE
      } else {
        this._membrane._inner = Membrane._SIDEKICK_CODE
      }
  ...
  _installNative (T, internal = false) {
    ...
    const Membrane = require('./membrane')
    this._membrane._inner = Membrane._NATIVE_CODE
