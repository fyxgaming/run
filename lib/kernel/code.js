...
const { _networkSuffix, _deployable, _tokenType, _ownerScript, _activeRun } = require('../util/misc')
...
class Code {
  ...
  extractProps (T) {
    ...
    // Check that these properties are serializable and extract tokens
    const refs = []
    try {
      TokenJSON._serialize(props, {
        _replacer: TokenJSON._replace._multiple(
          TokenJSON._replace._tokens(ref => { this.refs.push(ref); return {} }),
    ...
    try {
      ...
      if (Parent !== Object.getPrototypeOf(Object) &&
        ...
        if (realdeps[Parent.name]) {
          const currentSandbox = this.getSandboxed(realdeps[Parent.name])
      ...
      const [S, Sglobal] = this.sandboxType(T, env)
      ...
      const tokens = []
      const tokenSaver = token => { tokens.push(token); return tokens.length - 1 }
      const tokenLoader = ref => this.getSandboxed(tokens[ref]) || tokens[ref]
      ...
      let serialized = null
      try {
        serialized = TokenJSON._serialize(staticProps, opts)
      } catch (e) {
        throw new Error(`A static property of ${T.name} is not supported\n\n${e}`)
      }
      ...
      if (hasPresets) {
        ...
        try {
          this.pending.add(T)
          tempLocation = run.transaction.storeCode(T, S, realdeps, actionProps, success, error)
        } finally { this.pending.delete(T) }

        T[`origin${net}`] = T[`location${net}`] = tempLocation
        S[`origin${net}`] = S[`location${net}`] = tempLocation
        T[`owner${net}`] = S[`owner${net}`] = T.owner
      }

      // Deploy each deployable
      tokens.filter(token => _tokenType(token) === 'code')
        .forEach(T => this.deploy(T, options))

      // Create a safe clone of the static properties for the sandbox
      try {
        const safeStaticProps = TokenJSON._deserialize(serialized, opts)
      ...
      if (Sglobal) {
        Object.entries(realdeps).forEach(([name, dep]) => {
          ...
          Sglobal[name] = this.deploy(dep, options)
        })
      }
  ...
  async installFromTx (def, location, tx, kernel, bsvNetwork, partiallyInstalledCode = new Map()) {
    ...
    const hex2 = Buffer.from(_ownerScript(def.owner, bsvNetwork).toBytes()).toString('hex')
    ...
    try {
      ...
      if (ParentRegex.test(def.text)) {
        ...
        if (parentName === 'Jig' && typeof parentLocation === 'undefined') {
          ...
          env[parentName] = await kernel._transaction.load(parentLocation, { partiallyInstalledCode })
        }
      }

      const [S, Sglobal] = Sandbox._instance._evaluate(def.text, env)
      ...
      if (Sglobal) {
        const promises = Object.entries(def.deps || {}).map(([name, dep]) => {
          ...
          return kernel._transaction.load(location, { partiallyInstalledCode }).then(T => {
            Sglobal[name] = T
          })
        })
      ...
      if (def.deps) {
        ...
        Object.keys(def.deps).forEach(name => {
          S.deps[name] = Sglobal[name] || env[name]
        })
      }

      // Hydrate class prop tokens and apply them to the sandbox
      if (def.props) {
        // First pre-load the tokens in the class props
        const tokens = new Map()
        const refs = TokenJSON._findAllTokenRefsInTokenJson(def.props)
        const loadToken = async ref => {
          const location = (ref[1] === 'i' || ref[1] === 'o') ? txid + ref : ref
          const token = await kernel._transaction.load(location, { partiallyInstalledCode })
          tokens.set(ref, token)
        }
        const promises = refs.map(ref => loadToken(ref))
        await Promise.all(promises)

        // Deserialize the prop JSON
        const classProps = TokenJSON._deserialize(def.props, {
          _reviver: TokenJSON._revive._multiple(
            TokenJSON._revive._tokens(ref => tokens.get(ref)),
            TokenJSON._revive._arbitraryObjects())
        })
  ...
  installJig () {
    // Jigs don't use TokenSet and TokenMap, because it makes
  ...
  sandboxType (T, env) {
    ...
    return Sandbox._instance._sandboxType(T)
  }
