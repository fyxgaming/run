...
class Bundle {
  _deployed () { return false }
  _failed () { return false }
}
...
class Code {
  constructor () {
    this._sandboxes = {} // { <network>: Map<Func|SandboxFunc, SandboxFunc> }
  }

  _deploy (kernel, T, options = {}) {
    // Check if already has
    // if (this.pending.has(type)) return

    const network = kernel._blockchain.network
    const installs = this._sandboxes[network] = this._sandboxes[network] || new Map()

    // Check if we've already deployed this type
    if (installs.has(T)) return installs.get(T)
    if (!deployable(T)) throw new Error(`${_name(T)} not deployable`)
    const locationPreset = preset(T, 'location', network)
    if (locationPreset && installs.has(locationPreset)) return installs.get(locationPreset)

    // Nope. Start deploying.
    kernel._logger.info(`Deploying ${_name(T)}`)

    // Create a code bundle. This is a container of code that needs to be deployed together.
    const bundle = options.bundle || new Bundle()

    // Make the bundle

    console.log(locationPreset)

    // We are the outer deploy. Therefore, we can deploy for real.
    if (!options.bundle) {
      console.log(bundle ? '' : '')
    }
  }
