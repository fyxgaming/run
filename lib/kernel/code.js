...
class Editor {
  ...
  _install (T, local = true, newCode = []) {
    ...
    try {
      ...
      if (_extendsFrom(T, Jig)) {
        this._membrane._inner = require('../membrane')._JIG_CODE
      } else {
        this._membrane._inner = require('../membrane')._SIDEKICK_CODE
      }
  ...
  _installNative (T, internal = false) {
    ...
    this._membrane._inner = require('../membrane')._NATIVE_CODE
