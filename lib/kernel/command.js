...
const { _assert, _parent, _hasOwnProperty, _sourceCode, _text } = require('../util/misc')
...
const _COMMANDS = ['deploy', 'upgrade', 'destroy', 'auth', 'call']
...
function _deploy (record, ...Cs) {
  ...
  Cs.forEach(C => {
    ...
    // Spend parent non-native classes if they are owner-sealed
    const native = !!Repository._native()._get(Parent)
    if (native) return
...
module.exports = { _COMMANDS, _deploy, _upgrade, _destroy, _auth, _call }
