...
class _Action {
  ...
  static _publish (action) {
    ...
    Log._info(action.constructor._TAG, 'Publishing', action._id)
  }
...
class _DeployAction extends _Action {
  ...
  _add (C) {
    if (this._outputs.has(C)) return

    Log._info(_DeployAction._TAG, 'Adding', C.name)

    this._outputs.add(C)

    const src = _sourceCode(C)
    Log._debug(_DeployAction._TAG, 'Source:', src)

    const Membrane = require('./membrane')
    const props = Membrane._sudo(() => _deepClone(Object.assign({}, C), Sandbox._hostIntrinsics))
    Log._debug(_DeployAction._TAG, 'Props:', props)

    this._code.set(C, { src, props })
  }
}
