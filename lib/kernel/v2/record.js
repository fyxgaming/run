...
const { _location, _owner, _BINDINGS } = require('../../util/bindings')
...
class Record {
  ...
  async _payForTx () {
    ...
    */
    ...
    // const rawtx = this._tx.toHex()
    // await this._kernel._purse.pay(rawtx, parents)
  }
  ...
  async _broadcastTx () {
    ...
    */
    ...
    if (typeof purse.broadcast === 'function') {
      try {
        await purse.broadcast(tx.toString('hex'))
      } catch (e) {
    ...
    try {
      await blockchain.broadcast(tx.toString('hex'))
    } catch (e) {
      ...
      if (eString.indexOf('Missing inputs') !== -1 || eString.indexOf('txn-mempool-conflict') !== -1) {
        ...
        for (const input of tx.inputs) {
          try {
            ...
            const prevspend = await blockchain.spends(prevtxid, prevvout)

            if (prevspend) {
              const prevraw = await blockchain.fetch(prevtxid)
              const prevtx = new Transaction(prevraw)
              const type = _outputType(prevtx, prevvout)
