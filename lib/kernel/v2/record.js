  ...
  _addAction (id, args, inputs, outputs, refs) {
    ...
    for (const [jig, snapshot] of outputs) {
      ...
      Membrane._sudo(() => {
        ...
        snapshot.props.origin = jig.origin
        snapshot.props.location = jig.location
        snapshot.props.nonce = jig.nonce
      })
    }
  ...
  _dependencyPublished (record) {
    ...
    for (const [jig, snapshot] of record._outputs) {
      if (jigInMapKeys(this._inputs, jig)) {
        ...
        _BINDINGS.forEach(key => { inputSnapshot.props[key] = snapshot.props[key] })
      }

      if (jigInMapKeys(this._refs, jig)) {
        const key = jigMapKey(this._refs, jig)
        const refSnapshot = this._ref.get(key)
        _BINDINGS.forEach(key => { refSnapshot.props[key] = snapshot.props[key] })
      }
    }
...
const jigInMapKeys = (map, jig) => jigInArray(Array.from(map.keys()))
const indexOfJigInArray = (arr, jig) => arr.findIndex(x => sameJig(x, jig))
const indexOfJigInMapKeys = (map, jig) => indexOfJigInArray(Array.from(map.keys()))
