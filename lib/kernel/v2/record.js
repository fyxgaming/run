...
class Record {
  ...
  async _calculateState (x) {
    const missing = []

    const getLocalLocation = x => {
    ...
    const stateCodec = new Codec()._saveJigs(x => {
      const location = getLocalLocation(x)
    ...
    const after = this._after.get(x)

    const state = { }
    state.kind = after._kind
    state.props = after._props
    ...
    const encodedState = stateCodec._encode(state)

    console.log('AFTER', JSON.stringify(encodedState, 0, 3))
