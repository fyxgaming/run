...
class Record {
  ...
  _add (action) {
    ...
    // Add inputs for each changed that is not in creates
    for (const jig of changed) {
      if (_jigInArray(action._creates, jig)) continue
      if (_jigInMapKeys(this._inputs, jig)) continue

      const ss = action._snapshots.get(jig)
