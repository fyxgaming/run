...
class Record {
  ...
  constructor () {
    ...
    this._inputs = new SafeSet() // Set<Jig>
    this._outputs = new SafeSet() // Set<Jig>
    this._refs = new SafeMap() // Jig -> Location

    this._bindings = new SafeMap() // Jig -> Object
    this._spends = new SafeSet() // Set<Jig>
  ...
  _add (action) {
    ...
    Array.from(action._inputs)
      .filter(jig => !(this._outputs.has(jig) && !this._inputs.has(jig)))
      .forEach(jig => {
        this._inputs.add(jig)
        this._saveBindings(jig)
        this._linkRecords(jig, true)
      })

    // Add outputs from the action to the record
    Array.from(action._outputs)
      .filter(jig => !this._outputs.has(jig))
      .forEach(jig => this._outputs.add(jig))

    // Add references from the action to the record and also detect if these rely on other records.
    Array.from(action._refs)
      .filter(jig => !this._inputs.has(jig))
      .filter(jig => !this._outputs.has(jig))
      .forEach(jig => {
  ...
  async _publish () {
    ...
    console.log('bdn', this._bindings.size)
    console.log('spn', this._spends.size)
    ...
    for (const jig of this._outputs) {
      const spent = this._spends.has(jig)
  ...
  _linkRecords (jig, isInput) {
    ...
    if (isInput) parent._spends.add(jig)
    this._parents.add(parent)
  }

  _saveBindings (jig) {
    if (this._bindings.has(jig)) return
    const bindings = {}
    Membrane._sudo(() => Bindings._BINDINGS.forEach(x => { bindings[x] = jig[x] }))
    this._bindings.set(jig, bindings)
  }
}
  ...
  constructor (Cs) {
    ...
    Cs.forEach(C => {
      ...
      if (!C.options || !C.options.utility) {
        ...
        if (Parent) {
          if (!Cs.includes(Parent)) this._inputs.add(Parent)
          this._outputs.add(Parent)
        }
      }
