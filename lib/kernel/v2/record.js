...
class Record {
  ...
  _publish () {
    ...
    const publishFailure = e => {
      const unhandled = this._syncListeners.length === 0

      try {
        this._rollback(e, unhandled)
      } catch (e2) {
        e.message += `\n\nError during rollback: ${e2}`
      }

      this._syncListeners.forEach(s => s.reject(e))
    ...
    this._publishInternal().catch(publishFailure).then(publishSuccess)
  }
  ...
  async _publishInternal () {
    ...
    // Update the inventory with jigs created, deleted or updated
    this._notifyInventory()
  ...
  _notifyInventory () {
    this._jigs.forEach(jig => this._kernel._inventory._notify(jig))
  }
