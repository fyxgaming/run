...
class Record {
  ...
  _deploy (Cs) {
    ...
    // Store the action
    // this._addAction('deploy', actionArgs, inputs, outputs, refs)
  ...
  _spend (jig) {
    if (hasJig(this._inputs, jig)) return
    this._inputs.push(jig)
    Membrane._sudo(() => {
   ...
   */
  _addAction (id, args, inputs, outputs, refs, owner) {
    /*
