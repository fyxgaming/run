...
class Record {
  ...
  // Protections:
  // inputs that are already inputs must have outputs
  // Need some way of marking owners as unbound without affecting clone?
  ...
  _add (action) {
    ...
    // Inputs that are previous outputs must have bound or undefined owners
    ...
    outputs.forEach(jig => {
      ...
      Membrane._sudo(() => {
        ...
        ss._props.origin = jig.origin
        ss._props.location = jig.location
        ss._props.nonce = jig.nonce
      })
    })
