...
class Record {
  ...
  _upstreamPublished (record) {
    ...
    for (const [jig, snapshot] of record._after) {
      const ours = this._jigs.find(j => sameJig(j, jig))
      if (!ours) continue

      const ss = this._before.get(ours)
      if (!ss) continue

      _BINDINGS.forEach(binding => { ss._props[binding] = snapshot._props[binding] })

      const Unbound = require('./unbound')

      if (!this._spentDownstream(ours)) {
        Membrane._sudo(() => {
          if (ours.owner instanceof Unbound && !ours.owner._value) ours.owner = snapshot._props.owner
          if (ours.satoshis instanceof Unbound && !ours.satoshis._value) ours.satoshis = snapshot._props.satoshis
        })
      }
    }
  ...
  async _publishInternal () {
    ...
    // TODO: This state needs to be ready when the jig is committed...

    // Calculate state
    const stateCodec = new Codec()._saveJigs(x => {
      const vout = this._outputs.findIndex(y => sameJig(x, y))
      if (vout >= 0) return `_o${vout}`

      const vdel = this._deletes.findIndex(y => sameJig(x, y))
      if (vdel >= 0) return `_d${vout}`

      const vref = this._reads.findIndex(y => sameJig(x, y))
      if (vref >= 0) return ref[vref]

      const location = Membrane._sudo(() => x.location)
      const { native } = _location(location)
      if (native) return location

      // But what if this jig was updated, but not referenced?
      // This needs to be serializing a snapshot

      // Snapshot -> state?

      // return location
      throw new InternalError(`Cannot locate: ${_text(x)}`)
    })

    // const localLocation = jig => {
    // if ()
    // }

    // if (!location.startsWith(this._id)) return location

    const calculateState = jig => Membrane._sudo(() => {
      // If origin or record starts with this, remove this
      // And assign it its actual location

      // TODO: Origin and location
      const props = Object.assign({}, jig)

      const encodedProps = stateCodec._encode(props)

      const kind = 'jig'

      // Parent class
      return { kind, props: encodedProps }
    })

    const ostates = this._outputs.map(calculateState)
    const dstates = this._deletes.map(calculateState)

    console.log('ostates', JSON.stringify(ostates, 0, 2))
    console.log('dstates', JSON.stringify(dstates, 0, 2))
    ...
    const lock = jsonCodec._encode(this._owners)
    const ref = this._refs.map(jig => this._before.get(jig)._props.location)

    // Create the OP_RETURN json
    const json = { in: this._inputs.length, ref, out, lock, exec }
  ...
  async _getOwners () {
    ...
    let numLoops = 0
    while (this._owners.some(owner => owner instanceof Unbound)) {
      if (++numLoops === 10) {
  ...
  _assignOwnersAndSatoshis () {
    ...
    // Assign owners for new creates
    for (let i = 0; i < this._owners.length; i++) {
      const jig = this._creates[i]
      const owner = this._owners[i]
      ...
      this._after.get(jig)._props.owner = owner
    }
    ...
    this._outputs.forEach(jig => {
      ...
      if (ssprops.satoshis instanceof Unbound) ssprops.satoshis = ssprops.satoshis._value
    })
  }
