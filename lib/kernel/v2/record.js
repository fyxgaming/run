...
class Record {
  ...
  _spentDownstream (jig) {
    for (const record of this._downstream) {
      for (const jig2 of record._spent) {
  ...
  async _publishInternal () {
    ...
    // Look at each owner of each action
    // Can we make this less hacky?

    console.log('THIS OWNER', this._owner)

    if (this._owner instanceof Unbound) {
      this._owner = await this._kernel().owner()
