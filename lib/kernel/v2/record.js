...
class Record {
  ...
  async _calculateState (jig) {
    ...
    // Setup the codec where we will detect if we don't have all refs
    const missing = []
    ...
    // If we serialized everything, move forward
    if (!missing) return encodedState

    // Otherwise, get locations from input
  }
