...
class Record {
  ...
  async _calculateState (jig) {
    ...
    const dependencyMap = this._createDependencyMap(jig)
  ...
  async _createDependencyMap (jig) {
    ...
    const state = await this._getState(location)

    console.log(state)

    // Codec decode ... for each jig, decode that jig too. Just that.
    // Map loaded origins -> locations

    return new Map()
  }

  async _getState (location) {
    const state = await this._kernel._cache.get(location)
    if (state) return state

    // TODO: If it doesn't exist, override state cache, load the jig, set back.
  }
