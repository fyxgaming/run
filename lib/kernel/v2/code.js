...
const REPOSITORY = new Map() // T | Code -> Code
...
class Code {
  constructor (T) {
    ...
    const PrevCode = REPOSITORY.get(T) || REPOSITORY.get(presetLocation)
    ...
    S.deploy = () => deploy(NewCode)
    S.upgrade = T => upgrade(NewCode, T)
    S.sync = () => sync(NewCode)
    S.release = () => release(NewCode)

    try {
      // Temporarily add the new code for dependencies, even though its not fully installed
      REPOSITORY.set(T, NewCode)
      REPOSITORY.set(NewCode, NewCode)
      if (presetLocation) REPOSITORY.set(presetLocation, NewCode)
...
function checkReserved (T) {
  ...
  for (const reserved of RESERVED) {
    if (props.includes(reserved)) {
      throw new InstallFailedError(T, `Cannot specify reserved function: ${reserved}`)
    }
  }
}
