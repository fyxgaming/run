...
const ACTIVE_REPOSITORY = () => {
  ...
  REPOSITORIES.set(network, new Map())
  return REPOSITORIES.get(network)
}

// TODO: Install _Unbound as native
// Native are the same across all repositories
// They should be created where the code is defined
// Should Code be in there?

// Q: Do you use anonymous functions?
// Q: Static functions don't follow the normal rules of passing args
//  - functions
...
class Code {
  constructor (T, options = {}) {
    const network = options._native ? 'all' : _kernel()._blockchain.network
    ...
    const repository = options._native ? NATIVE_REPOSITORY : ACTIVE_REPOSITORY()
    ...
    const [S, SGlobal] = Sandbox._sandboxType(T, env, options._native)
    ...
    try {
      ...
      // TODO: _deepClone creates code ... need special handling
      // Native code props should not be cloned.
      ...
      if (options._native) {
        ...
        Membrane._sudo(() => {
          C.origin = 'native://' + T.name
          C.location = 'native://' + T.name
          C.nonce = 0
          C.owner = null
          C.satoshis = null
        })
        ...
        const Sprops = _deepClone(props, Sandbox._intrinsics)
        ...
        if (!presetLocation) {
          ...
          Membrane._sudo(() => {
            ...
            C.nonce = -1
            C.owner = new Unbound(undefined)
            C.satoshis = new Unbound(0)
          })
  ...
  _deploy () {
    ...
    _transaction(record => _deploy(record, Cs))
  }
  ...
  static [Symbol.hasInstance] (x) {
    ...
    const repository = ACTIVE_REPOSITORY()
    if (repository.has(x) && repository.get(x) === x) return true
    if (NATIVE_REPOSITORY.has(x) && NATIVE_REPOSITORY.get(x) === x) return true
    return false
  }
}
