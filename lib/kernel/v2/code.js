...
const { _location, _owner, _satoshis, _JIG, _USER, _Unbound } = require('../../util/bindings')
...
class Code {
  constructor (T) {
    ...
    try {
      ...
      if (!presetLocation) {
        S.origin = 'error://Not deployed'
        S.location = 'error://Not deployed'
        S.owner = new _Unbound(undefined)
        S.satoshis = new _Unbound(0)
      }

      // If owner is a new owner, and calling from the outside, can't call anymore.

      // Do we change the original?
...
function checkPresets (T) {
  ...
  for (const network of Object.keys(presets)) {
    ...
    const anyJigPresets = !_isUndefined(npresets.location) || !_isUndefined(npresets.origin) ||
      !_isUndefined(npresets.owner) || !_isUndefined(npresets.satoshis)
    const allJigPresets = !_isUndefined(npresets.location) && !_isUndefined(npresets.origin) &&
      !_isUndefined(npresets.owner) && !_isUndefined(npresets.satoshis)
    ...
    if (anyJigPresets) {
      try {
        _location(npresets.location, _JIG | _USER)
        _location(npresets.origin, _JIG | _USER)
