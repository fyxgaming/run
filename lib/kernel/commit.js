...
class Commit {
  ...
  constructor (record) {
    ...
    record._upstream.forEach(commit => commit._downstream.push(this))
  ...
  async _onReady () {
    if (!this._record._upstream.length) return
  ...
  _onUpstreamPublished (commit) {
    ...
    for (const [creation, prevss] of commit._after) {
      const ours = record._inputs.find(j => _sameCreation(j, creation)) ||
