...
class Commit {
  ...
  constructor (record) {
    ...
    // Save the original binding information
    this._bindings = new Map(record._bindings)
  ...
  _onUpstreamPublished (commit) {
    ...
    for (const [jig, prevss] of commit._after) {
      ...
      // Update the original bindings with our new bindings
      const bindings = this._bindings.get(ours)
      if (bindings) _BINDINGS.forEach(name => { bindings[name] = prevss._props[name] })
  ...
  async _buildRefmap (timeout) {
    ...
    const beforeLocations = incoming.map(jig => this._bindings.get(jig).location)
  ...
  static async _buildRefmapForIncoming (incoming, timeout) {
    ...
    _deepVisit(incoming, x => {
      ...
      if (x instanceof Berry) {
        ...
        // Don't traverse deep into berries for the same reason as jigs and code.
        return incoming.includes(x)
      }
    })
