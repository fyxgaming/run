...
class Commit {
  ...
  _saveRecordBindings () {
    ...
    _sudo(() => {
      this._outputs.forEach((jig, index) => {
        const bindings = {}
        _BINDINGS.forEach(name => { bindings[name] = jig[name] })
  ...
  _onUpstreamPublished (commit) {
    ...
    for (const [jig, prevss] of commit._after) {
      ...
      if (rb) {
        rb.origin = prevss._props.origin
        if (rb.owner instanceof Unbound && !rb.owner._value) rb.owner = prevss._props.owner
        if (rb.satoshis instanceof Unbound && !rb.satoshis._value) rb.satoshis = prevss._props.satoshis
      }
