...
class Commit {
  ...
  constructor (record) {
    ...
    // Save the moment information
    this._moments = new Map(record._moments)
  ...
  _onUpstreamPublished (commit) {
    ...
    for (const [jig, prevss] of commit._after) {
      ...
      // Update the moments with our new bindings
      const moment = this._moments.get(ours)
      if (moment) _BINDINGS.forEach(binding => { moment[binding] = prevss._props[binding] })
  ...
  async _buildRefmap (timeout) {
    ...
    const beforeLocations = incoming.map(jig => this._moments.get(jig).location)
