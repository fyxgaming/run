...
const { crypto } = bsv
const { _assert, _kernel, _sameCreation, _addCreations, _subtractCreations } = require('../util/misc')
...
class Commit {
  ...
  constructor (record) {
    ...
    // Copy the commit refmap if it exists
    ...
    if (!this._record._replaying) {
      this._outputs.forEach(creation => this._kernel._emit('update', creation))
      this._deletes.forEach(creation => this._kernel._emit('update', creation))
    }
  }
  ...
  _saveRecordBindings () {
    ...
    _sudo(() => {
      this._outputs.forEach((creation, index) => {
        const bindings = {}
      ...
      this._deletes.forEach((creation, index) => {
        const bindings = {}
        bindings.location = creation.location
        bindings.origin = creation.origin
        bindings.nonce = creation.nonce
  ...
  _onUpstreamPublished (commit) {
    ...
    for (const [creation, prevss] of commit._after) {
      const ours = this._inputs.find(j => _sameCreation(j, creation)) ||
        this._refs.find(j => _sameCreation(j, creation))
      if (!ours) continue
  ...
  _spentDownstream (creation) {
    for (const commit of this._downstream) {
      for (const creation2 of commit._inputs) {
  ...
  async _buildRefmap (timeout) {
    ...
    const beforeLocations = incoming.map(creation => this._before.get(creation)._props.location)
  ...
  _onPublishFail (e) {
    ...
    if (unhandled) {
      ...
      _sudo(() => {
        this._outputs.forEach(creation => { creation.location = errorLocation })
        this._deletes.forEach(creation => { creation.location = errorLocation })
      })
    }
...
Commit._calculateInputsOutputsRefs = function (record) {
  const inputs = _subtractCreations(_addCreations(_addCreations(record._updates, record._auths), record._deletes), record._creates)
  const outputs = _subtractCreations(_addCreations(inputs, record._creates), record._deletes)
  const refs = _subtractCreations(_subtractCreations(_subtractCreations(record._reads, inputs), outputs), record._deletes)
