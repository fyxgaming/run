...
class Commit {
  ...
  constructor (record) {
    ...
    this._upstream = record._deps // commits we depend on
    ...
    const EVENT_COMMIT = require('./kernel')._EVENT_COMMIT
    this._outputs.forEach(jig => this._kernel._emit(EVENT_COMMIT, jig))
    this._deletes.forEach(jig => this._kernel._emit(EVENT_COMMIT, jig))
  }
  ...
  async _sync () {
    ...
    if (!this._publishing && !this._record._autopublish) return
  ...
  /**
   * Deactivates this commit from being published and synced
   */
  _deactivate () {
  ...
  /**
   * Reactivates the commit so that it can be published
   */
  _activate () {
    ACTIVE_COMMITS.set(this._id, this)
  ...
  _onPublishFail (e) {
    ...
    const _EVENT_ROLLBACK = require('./kernel')._EVENT_ROLLBACK
    this._outputs.forEach(jig => this._kernel._emit(_EVENT_ROLLBACK, jig))
    this._deletes.forEach(jig => this._kernel._emit(_EVENT_ROLLBACK, jig))
