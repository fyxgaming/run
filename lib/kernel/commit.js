...
const { _assert, _sameJig, _addJigs, _subtractJigs } = require('../util/misc')
...
class Commit {
  ...
  constructor (record) {
    ...
    // Create the publisher and publish if necessary
    this._publisher = new Publisher(this)
    if (!this._upstream.length) this._publisher._publish()
  }
  ...
  _onUpstreamPublished (commit) {
    ...
    for (const [jig, prevss] of commit._after) {
      const ours = this._masterList.find(j => _sameJig(j, jig))
      if (!ours) continue
