...
class Commit {
  ...
  async _getBeforeRefmap (timeout) {
    if (this._beforeRefmap) return this._beforeRefmap

    const incoming = this._inputs.concat(this._refs)
    const incomingLocations = incoming.map(jig => this._before.get(jig)._props.location)
    const incomingRefmaps = await Promise.all(incomingLocations.map(location => refmap(location, this._kernel)))

    const combinedRefmap = {}

    // Always take the latest references for every jig
    for (const refmap of incomingRefmaps) {
    ...
    this._beforeRefmap = combinedRefmap
