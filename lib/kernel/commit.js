...
class Commit {
  ...
  _combine (record) {
    ...
    Membrane._sudo(() => {
      this._jigs.forEach((jig, index) => {
        ...
        if (_hasJig(this._creates, jig)) jig.origin = location
        jig.location = location

        // if (_hasJig(reduceNonce, jig)) jig.nonce -= 1
      })
    })
  ...
  _determineTransactionJigs () {
    ...
    this._outputs = this._spends.filter(jig => !_hasJig(this._deletes, jig))
    this._creates.filter(jig => jig => !_hasJig(this._deletes, jig)).forEach(jig => this._outputs.push(jig))
  }
  ...
  _sortJigsForReplayability () {
    this._jigs.sort((a, b) => {
      const spendsA = _hasJig(this._spends, a)
      const spendsB = _hasJig(this._spends, b)

      if (spendsA && spendsB) return 0
      if (spendsA) return -1
      if (spendsB) return 1

      const readsA = _hasJig(this._reads, a)
      const readsB = _hasJig(this._reads, b)
