...
class Code {
  ...
  _checkValid(T) {
    ...
    if (!_deployable(T)) throw new Error(`${_display(T)} not deployable`)
...
class CodeDescriptor {
  ...
  _addLocalCopy(T) {
    ...
    if (T.toString() !== this._T.toString()) {
      throw new Error(`Detected different versions of ${_display(T)}`)
    }
  ...
  _checkProps() {
    ...
    if (classProps.includes('deps') && !isBasicObject(this._T.deps)) {
      throw new Error(`deps must be an object on ${_display(this._T)}`)
    }

    // Check the presets is an object
    if (classProps.includes('presets')) {
      if (typeof T.presets !== 'object' || !T.presets) {
        throw new Error(`presets must be an object on ${_display(T)}`)
      }

      const networkPresets = T.presets[network]
      if (networkPresets) {
        if (typeof networkPresets !== 'object' || !networkPresets) {
          throw new Error(`presets for ${network} must be an object on ${_display(T)}`)
        }
        ...
        if (hasJigPresets) {
          ...
          if (origin || location || owner) {
            throw new Error(`jig presets for ${network} are not fully defined ${_display(T)}`)
          }
        }
      }
    }
