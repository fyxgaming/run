...
class Transaction {
  ...
  _onReadyForPublish () {
    ...
    this._kernel._syncer._publish(this._record)
    this._record = new Record(this._onReadyForPublish.bind(this))
  }
  ...
  import (tx) { return this._activeRecord._import(tx, this._kernel, false) }

  rollback () { this._activeRecord.rollback(this._kernel._syncer._lastPosted, this._kernel, false, 'intentional rollback') }
  ...
  // return this._record.inputs
  // .filter(input => input.origin !== '_')
  // .map(input => this._record.proxies.get(input))
  // }

  // get outputs () {
  // return this._record.outputs.map(output => this._record.proxies.get(output))
  // }

  get actions () {
    return this._activeRecord.actions.map(action => {
      return {
        target: this._activeRecord.proxies.get(action.target),
  ...
  async _loadResource (location, options = {}) {
    ...
    if (vout > 0 && vout < record.code.length + 1) {
      return this._kernel._code._getSandboxed(location) || options._partiallyInstalledCode.get(location)
    }

    // otherwise, a jig. get the jig.
    const proxies = record.outputs.map(o => record.proxies.get(o))
    const jigProxies = new Array(1 + record.code.length).concat(proxies)
