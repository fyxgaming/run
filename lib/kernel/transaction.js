...
class Transaction {
  constructor (kernel) {
    this._kernel = kernel
    try {
      this._owner = kernel._owner.locks[0]
...
class ProtoTransaction {
  ...
  async import (tx, kernel, preexistingJig, immutable, vout, cache) {
    ...
    */
    ...
    this.outputs.forEach((o, n) => {
      ...
      const hex1 = Buffer.from(_ownerLock(o.owner, bsvNetwork).script).toString('hex')
  ...
  buildBsvTransaction (kernel) {
    ...
    spentJigs.forEach((jig, index) => {
      ...
      const scriptBuffer = _ownerLock(before.restore().owner, bsvNetwork).script
    ...
    this.code.forEach(def => {
      const scriptBuffer = _ownerLock(def.owner, bsvNetwork).script
      const script = bsv.Script.fromBuffer(Buffer.from(scriptBuffer))
    ...
    this.outputs.forEach(jig => {
      ...
      const scriptBuffer = _ownerLock(restored.owner, bsvNetwork).script
      const script = bsv.Script.fromBuffer(Buffer.from(scriptBuffer))
