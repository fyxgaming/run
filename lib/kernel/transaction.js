...
const { _assert, _kernel, _Timeout } = require('../util/misc')
...
class Transaction {
  ...
  async publish () {
    ...
    if (commit._upstream.length) {
      ...
      // The publish promise becomes a sync once autopublish is enabled
  ...
  async export (options = {}) {
    ...
    const timeout = new _Timeout('export', _kernel()._timeout)
