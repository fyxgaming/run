...
class _ProtoTransaction {
  ...
  async _import (tx, kernel, preexistingJig, immutable, vout, cache) {
    ...
    */
    ...
    data.code.forEach((def, index) => {
      ...
      const success = () => {}
      const error = () => {}
      console.log('CREATOR', def.creator)
      this._storeCode(S, S, deps, props, success, error, def.creator, kernel._code, kernel)
      console.log('==')
    })
  ...
  _storeCode (T, S, deps, props, success, error, owner, code, kernel) {
    ...
    try {
      ...
      this.code.push({ type: T, sandbox: S, deps, props, success, error, owner })
