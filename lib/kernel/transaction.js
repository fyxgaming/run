...
class Transaction {
  ...
  publish (options = { }) {
    ...
    _checkArgument(typeof options.sign === 'undefined' || typeof options.sign === 'boolean', `Invalid sign: ${_text(options.pay)}`)
  ...
  async _finishAndPublish (pay, sign) {
    ...
    const tx = await _PURSE_SAFETY_QUEUE._enqueue(async () => {
      const partialTx = new bsv.Transaction(this._rawtx)
    ...
    return _kernel()._blockchain.broadcast(rawtx)
  }
