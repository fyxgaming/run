...
class Transaction {
  ...
  cache () {
    ...
    if (Transaction._ATOMICALLY_UPDATING) throw new StateError('cache disabled during atomic update')
    ...
    const timeout = new _Timeout('cache')

    const cacheAsync = async () => {
      const { _cacheStates } = require('./publish')

      // Add to cache, both outputs and deleted states
      await _cacheStates(this._commit, this._commit._states, this._txid || this._tx.hash)
      timeout._check()
    }
    ...
    this._cachePromise
      .then(() => { logEnd(); this._cachePromise = null })
  ...
  async _buildAsync (commit, timeout) {
    try {
      ...
      const hashes = _hashStates(commit, states)
      ...
      await _preverify(commit._kernel, record, states, payload, partialtx, timeout)
      timeout._check()
  ...
  async _finishAndPublish (pay, sign, timeout) {
    ...
    try {
      ...
      const txid = await _PURSE_SAFETY_QUEUE._enqueue(async () => {
        ...
        // Sign the jig owners
        const signedtx = sign ? await _signTx(paidtx, this._commit, feePerKb) : paidtx
        timeout._check()
      ...
      if (anythingToTrust) {
        this._commit._kernel._trustlist.add(txid)
      }
