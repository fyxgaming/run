...
const { FriendlySet } = require('../util/friendly')
...
class Transaction {
  ...
  begin () { this.protoTx.begin(); return util.activeRunInstance() }
  end () { this.protoTx.end(); return util.activeRunInstance() }
...
class ProtoTransaction {
  ...
  async import (tx, run, preexistingJig, immutable, vout, cache) {
    ...
    const bsvNetwork = util._bsvNetwork(run.blockchain.network)
