...
class Transaction {
  ...
  async load (location, options = {}) {
    ...
    if (!loc.innerLocation) {
      return this._loadRunToken(location, options)
    } else {
  ...
  async _loadRunToken (location, options = {}) {
        ...
        JigControl._disableSafeguards(() => {
          const deserialized = TokenJSON._deserialize(cachedState.state, {
            ...
            _reviver: TokenJSON._revive._multiple(
              TokenJSON._revive._resources(resourceLoader),
...
class _ProtoTransaction {
  ...
  async _import (tx, kernel, preexistingJig, immutable, vout, cache) {
    ...
    */
    ...
    // TokenJSON._serialize(refs, { _replacer: x => 'REPLACED' })
    ...
    if (data.jigs !== this.outputs.length) {
      const line = JigControl._disableSafeguards(() => {
        return this.outputs
      })
        ...
        return TokenJSON._serialize(state, {
          ...
          _replacer: TokenJSON._replace._cache(
            TokenJSON._replace._multiple(
              TokenJSON._replace._resources(resourceToRelativeLocation),
  ...
  _storeAction (target, method, args, inputs, outputs, reads, before, after, proxies, kernel) {
    ...
    try {
      ...
      const resources = new TokenSet([target])
