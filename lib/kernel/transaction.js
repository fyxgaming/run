...
class ProtoTransaction {
  ...
  async import (tx, kernel, preexistingJig, immutable, vout, cache) {
    ...
    */
    ...
    this.outputs.forEach((o, n) => {
      ...
      const hex1 = Buffer.from(_ownerScript(o.owner, bsvNetwork).script).toString('hex')
  ...
  buildBsvTransaction (kernel) {
    ...
    spentJigs.forEach((jig, index) => {
      ...
      const scriptBuffer = _ownerScript(before.restore().owner, bsvNetwork).script
    ...
    this.code.forEach(def => {
      const scriptBuffer = _ownerScript(def.owner, bsvNetwork).script
      const script = bsv.Script.fromBuffer(Buffer.from(scriptBuffer))
    ...
    this.outputs.forEach(jig => {
      ...
      const scriptBuffer = _ownerScript(restored.owner, bsvNetwork).script
      const script = bsv.Script.fromBuffer(Buffer.from(scriptBuffer))
