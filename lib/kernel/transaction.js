...
class Transaction {
  ...
  async load (location, options = {}) {
    ...
    if (!loc.innerLocation) {
      return this._loadRunResource(location, options)
    } else {
  ...
  async _loadRunResource (location, options = {}) {
    ...
    */
    ...
    for (const action of data.actions) {
      ...
      const args = JigControl._disableSafeguards(() => {
        ...
        return ResourceJSON._deserialize(action.args, {
  ...
  _buildBsvTransaction (kernel) {
    ...
    const actions = this.actions.map(action => {
      ...
      if (method === 'init') {
        ...
        return { target, method, args, creator: ResourceJSON._serialize(action.creator, opts) }
      }
