...
const { PROTOCOL_VERSION, _extractRunData, _encryptRunData } = require('../util/opreturn')
...
class Transaction {
  ...
  begin () { this._protoTx.begin(); return _activeRun() }
  end () { this._protoTx.end(); return _activeRun() }
...
class ProtoTransaction {
  ...
  async import (tx, kernel, preexistingJig, immutable, vout, cache) {
    ...
    */
    ...
    this.outputs.forEach((o, n) => {
      ...
      const hex1 = Buffer.from(_ownerScript(o.owner, bsvNetwork).toBytes()).toString('hex')
  ...
  storeCode (type, sandbox, deps, props, success, error, owner, code, kernel) {
    ...
    try {
      ...
      if (kernel._owner instanceof Owner) kernel._owner.update(code.getSandboxed(type))
      return tempLocation
    } finally {
      this.end()
    }
  ...
  buildBsvTransaction (kernel) {
    ...
    spentJigs.forEach((jig, index) => {
      ...
      const scriptBuffer = _ownerScript(before.restore().owner, bsvNetwork).toBytes()
    ...
    this.code.forEach(def => {
      const scriptBuffer = _ownerScript(def.owner, bsvNetwork).toBytes()
      const script = bsv.Script.fromBuffer(Buffer.from(scriptBuffer))
    ...
    this.outputs.forEach(jig => {
      ...
      const scriptBuffer = _ownerScript(restored.owner, bsvNetwork).toBytes()
      const script = bsv.Script.fromBuffer(Buffer.from(scriptBuffer))
