...
class ProtoTransaction {
    ...
    for (let vout = 0; vout < jigProxies.length; vout++) {
      ...
      const serialized = JigControl.disableProxy(() => {
        ...
        const opts = {
          ...
          _replacers: [_refifyTokens(x => x.location.startsWith(tx.hash) ? x.location.slice(64) : x.location)]
        }

        return _toTokenJson(state, opts)
      })
  ...
  buildBsvTransaction (kernel) {
    ...
    const { _sandboxIntrinsics } = kernel._code
    const _replacers = [_refifyTokens(tokenSaver)]
    const opts = { _sandboxIntrinsics, _replacers }

    // build each action
    const actions = this.actions.map(action => {
      const { method } = action

      const args = _toTokenJson(action.args, opts)
      ...
      if (method === 'init') {
        ...
        return { target, method, args, creator: _toTokenJson(action.creator, opts) }
      }
    ...
    const code = this.code.map(def => {
      ...
      const props = Object.keys(def.props).length ? _toTokenJson(def.props, opts) : undefined
