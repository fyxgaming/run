...
class ProtoTransaction {
  ...
  rollback (lastPosted, kernel, error, unhandled) {
    ...
    this.code.forEach(def => kernel._inventory._update(def.sandbox))
    ...
    this.outputs.forEach(jig => kernel._inventory._update(jig))
  ...
  storeCode (type, sandbox, deps, props, success, error, owner, code, kernel) {
    ...
    try {
      ...
      kernel._inventory._update(code.getSandboxed(type))
      return tempLocation
    } finally {
      this.end()
    }
  ...
  storeAction (target, method, args, inputs, outputs, reads, before, after, proxies, kernel) {
    ...
    outputs.forEach(target => kernel._inventory._update(proxies.get(target)))
  }
