...
class Transaction {
  constructor () {
    ...
    this._commit = null // Imported or built commit
    this._tx = null // Imported or built tx
  ...
  static async _import (tx, txid) {
    ...
    const commit = await _import(tx, txid, payload, kernel, published, jigToSync, timeout, preverify)
