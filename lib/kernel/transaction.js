...
class Transaction {
  ...
  _nextTokenOwner () { return this._tokenOwnerOverride || this._kernel._owner.next() }
  ...
  storeCode (type, sandbox, deps, props, success, error) {
    return this._activeProtoTx.storeCode(type, sandbox, deps, props, success, error,
      this._nextTokenOwner(), this._kernel._code, this._kernel)
  }
  ...
  async load (location, options = {}) {
    ...
    if (options.BerryClass) {
      return _pluckBerry(location, this._kernel._blockchain, this._kernel._code, options.BerryClass)
    }
  ...
  async loadRunToken (location, options = {}) {
    ...
    // Check that we are loading a token
...
class ProtoTransaction {
  ...
  async import (tx, kernel, preexistingJig, immutable, vout, cache) {
    ...
    this.beginCount = 1
    this.paid = true // Correct? Assume all inputs are paid
    ...
    */
    ...
    const dedupInnerRefs = jig => {
      ...
      JigControl._disableSafeguards(() => _deepReplaceObjects(jig, tokenReplacer))
    }
  ...
  storeCode (type, sandbox, deps, props, success, error, owner, code, kernel) {
    ...
    try {
      ...
      if (!this.importing) kernel._inventory._notify(code.getSandboxed(type))
      return tempLocation
    } finally {
      this.end()
    }
  ...
  async pay (kernel) {
    ...
    this.cachedTx.tx = await kernel._purse.pay(this.cachedTx.tx) || this.cachedTx.tx
    this.paid = true
  }
  ...
  async sign (kernel) {
    ...
    const numPayments = this.cachedTx.tx.inputs.length - spentJigs.length
    const locks = spentJigs
      .map(target => this.before.get(target))
      .map(checkpoint => checkpoint.restore())
      .map(token => _lockify(token.owner))
      .concat(new Array(numPayments))
    const tx = await kernel._owner.sign(this.cachedTx.tx, locks)
    this.cachedTx.tx = tx || this.cachedTx.tx
  }
