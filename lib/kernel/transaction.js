...
class ProtoTransaction {
  ...
  rollback (lastPosted, kernel, error, unhandled) {
    ...
    // Update the locker's code
    this.code.forEach(def => kernel._locker._update(def.sandbox))
    ...
    // Update the locker's jigs
    this.outputs.forEach(jig => kernel._locker._update(jig))
  ...
  storeCode (type, sandbox, deps, props, success, error, owner, code, kernel) {
    ...
    try {
      ...
      kernel._locker._update(code.getSandboxed(type))
      return tempLocation
    } finally {
      this.end()
    }
  ...
  storeAction (target, method, args, inputs, outputs, reads, before, after, proxies, kernel) {
    ...
    outputs.forEach(target => kernel._locker._update(proxies.get(target)))
  }
  ...
  async sign (kernel) {
    if (!this.cachedTx) this.buildBsvTransaction(kernel)
    return kernel._owner.unlock(this.cachedTx.tx)
  }
