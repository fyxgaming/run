...
const { _assert, _setOwnProperty, _text, _extendsFrom } = require('../util/misc')
...
function _executeDeploy (encdata, masterList) {
  ...
  for (let i = 0; i < ncode; i++) {
    ...
    _assert(typeof props === 'object' && !Array.isArray(props), 'DEPLOY props must be an object')
...
function _executeUpgrade (encdata, masterList) {
  ...
  _assert(typeof data[2] === 'object' && !Array.isArray(data[2]), 'UPGRADE props must be an object')
...
function _executeCall (encdata, masterList) {
  ...
  _assert((data[0] instanceof Code && _extendsFrom(data[0], Jig)) || data[0] instanceof Jig, 'Must only execute CALL on jigs')
