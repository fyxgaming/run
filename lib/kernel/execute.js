...
function _executeDeploy (encdata, masterList) {
  ...
  const code = []
  for (let i = 0; i < ncode; i++) code.push(Editor._newCode())
...
function _executeAuth (data, masterList) {
  ...
  const Code = require('./code')
  const Jig = require('./jig')
  if (!(x instanceof Code || x instanceof Jig)) throw new StateError('Invalid auth target')
