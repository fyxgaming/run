...
const { _text, _defineGetter, _limit } = require('./kernel/misc')
...
const { _extractMetadata } = require('./kernel/metadata')
...
/**
 * A generic plugin that most of the APIs may implement
 */
class Plugin {
  /**
   * Called when the plugin is assigned to a Run instance. This method is optional.
   * @param {Run} run
   */
  attach (run) { /* no-op */ }

  /**
   * Called when the plugin is removed from a Run instance. This method is optional.
   * @param {Run} run
   */
  detach (run) { /* no-op */ }
}
...
class Run {
  ...
  constructor (options = {}) {
    ...
    this._api = parseApi(options.api, keys.includes('api'))
    ...
    this._logger = parseLogger(options.logger)
    ...
    this._kernel._blockchain = parseBlockchain(options.blockchain, keys.includes('blockchain'), this._api, this._apiKey, options.network, this._kernel._cacheWrapper())
    this._kernel._state = parseState(options.state, options.network, this._api, this._apiKey, keys.includes('state'))
    this._kernel._purse = parsePurse(options.purse, this._kernel._blockchainWrapper(), wallet, keys.includes('purse'))
    this._kernel._app = parseApp(options.app, keys.includes('app'))
    this._kernel._owner = parseOwner(options.owner, this._kernel._blockchainWrapper(), wallet, keys.includes('owner'))
  ...
  set api (api) {
    ...
    this._kernel._blockchain = parseBlockchain(undefined, false, api,
      this._apiKey, this.blockchain.network, this._kernel._cacheWrapper())
    this._kernel._state = parseState(undefined, this.blockchain.network, api, this._apiKey, false)
    this._api = api
  }

  set apiKey (apiKey) {
    apiKey = parseApiKey(apiKey, true)
    this._kernel._blockchain = parseBlockchain(undefined, false, this._api,
      apiKey, this.blockchain.network, this._kernel._cacheWrapper())
    this._kernel._state = parseState(undefined, this.blockchain.network, this._api, apiKey, false)
    this._apiKey = apiKey
  }
  ...
  set debug (debug) {
    ...
    if (isActive(this)) Log._enableDebug = this._debug
  }
  ...
  set network (network) {
    this._kernel._blockchain = parseBlockchain(undefined, false, this._api,
      this._apiKey, network, this._kernel._cacheWrapper())
    this._kernel._state = parseState(undefined, network, this._api, this._apiKey, false)
  }
  ...
  set owner (owner) {
    const newOwner = parseOwner(owner, this._kernel._blockchainWrapper(), null, true)
  ...
  set purse (purse) {
    this._kernel._purse = parsePurse(purse, this.blockchain, null, true)
  }

  set state (state) {
    this._kernel._state = parseState(state, this.network, this.api, this.apiKey, true)
  }
...
function assignDefaults (options) {
  ...
  // Default to run api on mainnet and testnet
  if (typeof options.blockchain === 'undefined' && typeof options.api === 'undefined') {
    if (options.network === 'main' || options.network === 'test') {
      options.api = 'run'
    }

    if (options.network === 'stn') {
...
function parseApi (api, specified) {
  if (api === 'run' || api === 'mattercloud' || api === 'whatsonchain') {
    return api
  }

  if (typeof api === 'undefined' && !specified) {
    return Run.defaults.api
  }
...
function parseApiKey (apiKey, specified) {
  ...
  if (typeof apiKey === 'undefined' && !specified) {
    return Run.defaults.apiKey
  }
...
function parseApp (app, specified) {
  ...
  if (typeof app === 'undefined' && !specified) {
    return parseApp(Run.defaults.app, true)
  }
...
function parseAutofund (autofund, specified) {
  if (typeof autofund === 'boolean') {
    return autofund
  }

  if (typeof autofund === 'undefined' && !specified) {
    return parseAutofund(Run.defaults.autofund, true)
  }
  ...
  if (typeof blockchain === 'undefined' && !specified) {
    switch (network) {
      ...
      default:
        ...
        throw new Error(`Unsupported network: ${_text(network)}`)
    }
  }
...
function parseClient (client, specified) {
  ...
  if (typeof client === 'undefined' && !specified) {
    return parseClient(Run.defaults.client, true)
  }
...
function parseDebug (debug, specified) {
  ...
  if (typeof debug === 'undefined' && !specified) {
    return parseDebug(Run.defaults.debug, true)
  }
...
function parseNetworkRetries (networkRetries, specified) {
  ...
  if (typeof networkRetries === 'undefined' && !specified) {
    return Run.defaults.networkRetries
  }
...
function parseNetworkTimeout (networkTimeout, specified) {
  if (typeof networkTimeout === 'number' && networkTimeout >= 0 &&
    !Number.isNaN(networkTimeout) && networkTimeout !== Infinity) return networkTimeout

  if (typeof networkTimeout === 'undefined' && !specified) {
    return Run.defaults.networkTimeout
  }
...
function parseOwner (owner, blockchain, wallet, specified) {
  if (wallet) {
...
function parsePurse (purse, blockchain, wallet, specified) {
  if (wallet) {
...
function parseState (state, network, api, apiKey, specified) {
  ...
  if (typeof state === 'undefined' && !specified) {
    ...
    // If we are on mainnet and using run api, then use Run's State Server
    if (network === 'main' && api === 'run') {
      return new RunConnect({ apiKey })
    }

    // Otherwise, no state server will be used
    return undefined
  }
...
function parseTimeout (timeout, specified) {
  if (typeof timeout === 'number' && timeout >= 0 &&
    !Number.isNaN(timeout) && timeout !== Infinity) return timeout
...
function parseTrust (trust, specified) {
  ...
  if (typeof trust === 'undefined' && !specified) return lastTrusts || []

  // We allow a simple array to be used to specify '*' easily
  if (typeof trust === 'string') trust = [trust]

  // Make sure the trust parameter is an array or single string
  if (!Array.isArray(trust)) throw new Error(`Not trustable: ${_text(trust)}`)

  // Make sure the trust parameter has valid values
...
Run.defaults.logger = Log._defaultLogger
...
Run._Codec = require('./kernel/codec')
...
Run._RecentBroadcasts = require('./kernel/recent-broadcasts')
...
Run._StateFilter = require('./kernel/state-filter')
