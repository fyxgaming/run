...
const { _unifyByOrigin } = require('./kernel/unify')
...
class Run {
  ...
  constructor (options = {}) {
    ...
    this._kernel._autounify = parseAutounify(options.autounify)
  ...
  get autounify () { return this._kernel._autounify }
  ...
  set autounify (autounify) { this._kernel._autounify = parseAutounify(autounify) }
  ...
  set networkRetries (networkRetries) { this._networkRetries = REST._retries = parseNetworkRetries(networkRetries) }
  set networkTimeout (networkTimeout) { this._networkTimeout = REST._timeout = parseNetworkTimeout(networkTimeout) }
  set owner (owner) {
  ...
  activate () {
    ...
    // TODO: LOGGER does not set global right away. Has to be active....
...
function parseAutounify (autounify) {
  switch (typeof autounify) {
    case 'boolean': return autounify
    case 'undefined': return Run.defaults.autounify
    default: throw new Error(`Invalid autounify: ${autounify}`)
  }
}
