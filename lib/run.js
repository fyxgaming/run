...
const { TokenSet, TokenMap } = require('./util/token-set')
...
function parseBlockchain (options, logger) {
  ...
  if (typeof blockchain === 'undefined') {
    switch (options.network) {
      ...
      case 'main':
      case 'test':
        // If loggers are different, we still re-use the same blockchain. Yes, a bug.
        if (lastBlockchain instanceof BlockchainApi &&
          ...
          lastBlockchain.logger === options.logger &&
...
function parseLogger (logger) {
  ...
  switch (typeof logger) {
    ...
    case 'undefined': return defaultLogger
...
Run.TokenSet = TokenSet
Run.TokenMap = TokenMap
