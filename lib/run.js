...
class Run {
  constructor (options = {}) {
    ...
    this._kernel._manual = parseManual(options.manual)
    ...
    if (this.blockchain instanceof Mockchain && this.purse instanceof LocalPurse && !options.manual) {
  ...
  get manual () { return this._kernel._manual }
  ...
  set manual (manual) { this._kernel._manual = parseManual(manual) }
  set owner (owner) {
...
function parseManual (manual) {
  switch (typeof manual) {
    case 'boolean': return manual
    case 'undefined': return Run.defaults.manual
    default: throw new Error(`Invalid manual: ${manual}`)
  }
}
...
_defineGetter(Run, 'GroupLock', () => require('./extra/group-lock'))
