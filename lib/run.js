...
class Run {
  ...
  constructor (options = {}) {
    ...
    this._kernel._state = parseState(options.network, this._api, this._apiKey)
  ...
  set api (api) {
    ...
    this._kernel._state = parseState(this.blockchain.network, api, this._apiKey)
    this._api = api
  }
  ...
  set apiKey (apiKey) {
    ...
    this._kernel._state = parseState(this.blockchain.network, this._api, apiKey)
    this._apiKey = apiKey
  }
  ...
  set network (network) {
    ...
    this._kernel._state = parseState(network, this._api, this._apiKey)
  }
...
function parseCache (cache, network, specified) {
  ...
  if (typeof cache === 'undefined' && !specified) {
    ...
    // If we were already using a cache that applies, re-use it
    if (lastCache && (typeof lastCache.network === 'undefined' || lastCache.network === network)) {
      return lastCache
    }
...
function parseState (network, api, apiKey) {
...
function parseNetworkRetries (networkRetries) {
  switch (typeof networkRetries) {
    case 'number':
      if (Number.isNaN(networkRetries) || networkRetries < 0) throw new Error(`Invalid network retries: ${_text(networkRetries)}`)
      return networkRetries
    case 'undefined': return 2
    default: throw new Error(`Invalid timeout: ${_text(networkRetries)}`)
  }
}
...
_defineGetter(Run, 'DiskCache', () => { warnPlugins('DiskCache'); return DiskCache })
...
_defineGetter(Run, 'NodeCache', () => { warnPlugins('NodeCache'); return NodeCache })
...
_defineGetter(Run.module, 'DiskCache', () => { warnPlugins('DiskCache', rm); return DiskCache })
...
_defineGetter(Run.module, 'NodeCache', () => { warnPlugins('NodeCache', rm); return NodeCache })
