...
class Run {
  ...
  constructor (options = {}) {
    ...
    this._networkRetries = parseNetworkRetries(options.networkRetries, true)
    this._networkTimeout = parseNetworkTimeout(options.networkTimeout)
    ...
    this._kernel._owner = parseOwner(options.owner, this._kernel._blockchainWrapper(), options.wallet)
  ...
  set owner (owner) {
    this._kernel._owner = parseOwner(owner, this._kernel._blockchainWrapper())
    if (this._inventory) this._inventory.detach(this)
...
function parseNetworkTimeout (networkTimeout) {
  switch (typeof networkTimeout) {
    case 'number':
      if (Number.isNaN(networkTimeout) || networkTimeout < 0) throw new Error(`Invalid network timeout: ${_text(networkTimeout)}`)
      return networkTimeout
    case 'undefined': return 10000
    default: throw new Error(`Invalid network timeout: ${_text(networkTimeout)}`)
  }
}
