...
const { Blockchain, Key } = require('./kernel/api')
...
const StandardKey = require('./owner/standard-key')
const AddressLock = require('./owner/address-lock')
...
function parseOwner (owner, network, logger, run) {
  switch (typeof owner) {
    ...
    case 'string':
    case 'object':
      if (Key._isKey(owner)) return owner
      return new StandardKey(owner, network)
...
Run._util.StandardKey = StandardKey
