...
const { _text, _defineGetter, _checkArgument } = require('./util/misc')
...
class Run {
  constructor (options = {}) {
    ...
    this._kernel._trustlist = parseTrustlist(options.trust)
  ...
  trust (x) {
    Log._info(TAG, 'Trust', x)
    _checkArgument(typeof x === 'string', `Invalid trust string: ${_text(x)}`)
    if (this._kernel._trustlist.includes(x)) return
    this._kernel._trustlist.push(x)
  }
  ...
  off (event, listener) {
    const matches = x => x.event === event && x.listener === listener
    this._kernel._listeners = this._kernel._listeners.filter(x => !matches(x))
  }

  activate () {
    Log._info(TAG, 'Activate')
    Run.instance = this
...
function parseTrustlist (trust) {
  // If not defined and there is a previous instance, use its list. Trusting code is intentional.
  const lastTrustlist = Kernel._instance && Kernel._instance._trustlist
  if (typeof trust === 'undefined') return lastTrustlist || []
  ...
  if (!Array.isArray(trust) || trust.some(x => typeof x !== 'string')) {
    throw new Error(`Invalid trust: ${_text(trust)}`)
  }
