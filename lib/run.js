...
class Run {
  ...
  constructor (options = {}) {
    ...
    this._api = parseApi(options.api)
    this._apiKey = parseApiKey(options.apiKey)
    ...
    this._kernel._purse = parsePurse(options.purse, this._kernel._blockchainWrapper(), options.wallet, keys.includes('purse'))
    this._kernel._app = parseApp(options.app)
    this._kernel._owner = parseOwner(options.owner, this._kernel._blockchainWrapper(), options.wallet, keys.includes('owner'))
...
function parseApi (api) {
  if (typeof api !== 'undefined' && api !== 'run' && api !== 'mattercloud' && api !== 'whatsonchain') throw new Error(`Invalid api: ${_text(api)}`)
  return api
}
...
function parseApiKey (apiKey) {
  if (typeof apiKey !== 'undefined' && typeof apiKey !== 'string') throw new Error(`Invalid apiKey: ${_text(apiKey)}`)
  return apiKey
}
...
function parseAutofund (autofund) {
  if (typeof autofund !== 'boolean') throw new Error(`Invalid autofund: ${_text(autofund)}`)
  return autofund
}
...
function parseClient (client) {
  if (typeof client !== 'boolean') throw new Error(`Invalid client: ${_text(client)}`)
  return client
}
...
function parseDebug (debug) {
  if (typeof debug !== 'boolean') throw new Error(`Invalid debug: ${_text(debug)}`)
  return debug
}
