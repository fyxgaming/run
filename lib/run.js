...
// Extra
const compileAsm = require('./extra/compile-asm')
const expect = require('./extra/expect')
const GroupLock = require('./extra/group-lock')
const parseHex = require('./extra/parse-hex')
...
function parseOwner (owner, blockchain) {
  ...
  if (typeof owner === 'undefined' || typeof owner === 'string' || typeof owner === 'object') {
    return new LocalOwner({ owner, network: blockchain.network, blockchain })
  }
...
// Extra
Run.compileAsm = compileAsm
Run.expect = expect
Run.GroupLock = GroupLock
Run.parseHex = parseHex
Run.Token = Token
