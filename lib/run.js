...
class Run {
  ...
  constructor (options = {}) {
    ...
    this._kernel._cache = parseCache(options.cache, this._kernel._blockchain)
  ...
  set cache (cache) { this._kernel._cache = null; this._kernel._cache = parseCache(cache, this._kernel._blockchain) }
...
function parseCache (cache, blockchain) {
  if (cache instanceof Cache) return cache

  if (typeof cache === 'undefined') {
    // If we already have a cache, then re-use it
    if (Run.instance && Run.instance.cache && Run.instance.network === blockchain.network) {
      return Run.instance.cache
    }

    // If we are on mainnet, prefer RunConnect
    if (blockchain.network === 'main') {
      if (blockchain instanceof RunConnect) {
        return blockchain
      } else {
