...
const RunSDKState = require('./plugins/run-sdk-state')
...
function hookPlugins (run) {
  if (run._kernel._blockchain instanceof RunSDKBlockchain) run._kernel._blockchain.hook(run._kernel._cache)
  ...
  if (run._kernel._purse instanceof RunSDKPurse) run._kernel._purse.hook()
  if (run._kernel._state instanceof RunSDKState) run._kernel._state.hook(run._kernel._cache)
}
...
function parseCache (cache, specified, network) {
  ...
  if (cache instanceof RunDB) {
    ...
    throw new Error(`The RunDB plugin is now a State API implementation, not a Cache\n\n${hint}`)
  }
...
Run.plugins.RunSDKState = RunSDKState
