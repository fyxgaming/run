...
const RunSDKOwner = require('./plugins/run-sdk-owner')
...
function hookPlugins (run) {
  if (run._kernel._blockchain instanceof RunSDKBlockchain) run._kernel._blockchain.hook(run._kernel._cache, run._kernel._client)
  if (run._kernel._cache instanceof RunSDKCache) run._kernel._cache.hook()
  if (run._kernel._owner instanceof RunSDKOwner) run._kernel._owner.hook()
}
...
function parseBlockchain (blockchain, specified, api, apiKey, network) {
...
Run.plugins.RunSDKOwner = RunSDKOwner
