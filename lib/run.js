...
const { ArgumentError } = require('./kernel/errors')
...
class Run {
  ...
  constructor (options = {}) {
    ...
    this._kernel._cache = parseCache(options.cache, options.network, this._api, this._apiKey, keys.includes('cache'))
  ...
  set api (api) {
    ...
    this._kernel._cache = parseCache(undefined, this.blockchain.network, api, this._apiKey, false)
    this._api = api
  }
  ...
  set cache (cache) {
    this._kernel._cache = parseCache(cache, this.network, this._api, this._apiKey, true)
  }
...
function parseCache (cache, network, api, apiKey, specified) {
  ...
  if (typeof cache === 'undefined' && !specified) {
    ...
    if (lastCache && lastCache.api === api && lastCache.apiKey === apiKey &&
      (typeof lastCache.network === 'undefined' || lastCache.network === network)) {
      return lastCache
    }
...
// Errors
Run.errors = require('./kernel/errors')
