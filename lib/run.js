...
const { Blockchain, Logger, Purse, State, Key } = require('./kernel/api')
...
const LocalKey = require('./owner/private-key')
const AddressLock = require('./owner/standard-lock')
const PubKeyLock = require('./owner/pubkey-lock')
...
class Run {
  ...
  get internal () { return { code: this._kernel._code } }

  get jigs () { return this._kernel._locker._all('jig') }
  get code () { return this._kernel._locker._all('code') }
...
function parseOwner (owner, network, logger, run) {
  if (owner instanceof Key) return owner
  if (typeof owner === 'undefined' || typeof owner === 'string' || typeof owner === 'object') {
    return new LocalKey(owner, network)
  }
...
Run._util = {}
Run._util.AddressLock = AddressLock
Run._util.PubKeyLock = PubKeyLock
Run._util.LocalKey = LocalKey
