...
const { _wrap, _unwrap } = require('./kernel/wrap')
...
class Run {
  ...
  constructor (options = {}) {
    ...
    this._kernel._cache = parseCache(options.cache, network, keys.includes('cache'))
    this._kernel._blockchain = parseBlockchain(options.blockchain, keys.includes('blockchain'), this._kernel._cache, this._kernel._client, this._api, this._apiKey, network)
    this._kernel._state = parseState(options.state, keys.includes('state'), this._kernel._cache, this._kernel._client, network, this._api, this._apiKey)
    this._kernel._purse = parsePurse(options.purse, this._kernel._blockchain, wallet, keys.includes('purse'))
    this._kernel._app = parseApp(options.app, keys.includes('app'))
    this._kernel._owner = parseOwner(options.owner, this._kernel._blockchain, wallet, keys.includes('owner'))
  ...
  set api (api) {
    ...
    this._kernel._blockchain = parseBlockchain(undefined, false, this._kernel._cacheWrapper(), this._kernel._client, api, this._apiKey, this.network)
    this._kernel._state = parseState(undefined, false, this._kernel._cache, this._kernel._client, this.blockchain.network, api, this._apiKey)
    this._api = api
  }

  set apiKey (apiKey) {
    apiKey = parseApiKey(apiKey, true)
    this._kernel._blockchain = parseBlockchain(undefined, false, this._kernel._cacheWrapper(), this._kernel._client, this._api, apiKey, this.network)
    this._kernel._state = parseState(undefined, false, this._kernel._cache, this.blockchain.network, this._api, apiKey)
    this._apiKey = apiKey
  }
  ...
  set blockchain (blockchain) {
    this._kernel._blockchain = parseBlockchain(blockchain, true, this._kernel._cache, this._kernel._client, undefined, undefined, undefined)
    this._api = undefined
  ...
  set cache (cache) {
    ...
    this._kernel._blockchain = parseBlockchain(this._kernel._blockchain, true, this._kernel._cache, this._kernel._client, undefined, undefined, undefined)
    this._kernel._state = parseState(this._kernel._state, true, this._kernel._cache, this._kernel._client, undefined, undefined, undefined)
  }
  ...
  set network (network) {
    ...
    this._kernel._blockchain = parseBlockchain(undefined, false, this._kernel._cacheWrapper(), this._kernel._client, this._api, this._apiKey, network)
    this._kernel._state = parseState(undefined, false, this._kernel._cache, this._kernel._client, network, this._api, this._apiKey)
  }
  ...
  set owner (owner) {
    const newOwner = parseOwner(owner, this._kernel._blockchain, null, true)
  ...
  set state (state) {
    this._kernel._state = parseState(state, true, this._kernel._cache, this._kernel._client, this.network, this.api, this.apiKey)
  }
...
function parseApi (api, specified, blockchain, network) {
  if (api === 'run' || api === 'mattercloud' || api === 'whatsonchain') return api
...
function parseBlockchain (blockchain, specified, cache, client, api, apiKey, network) {
  return wrap(get())

  function wrap (b) {
    _wrap(b)
    b.$wrapper.cache = cache
    b.$wrapper.client = client
    return b
  }
  ...
  function get () {
    ...
    if (typeof blockchain === 'undefined' && !specified) {
      switch (network) {
        ...
        default:
          return parseBlockchain(Run.defaults.blockchain, true, cache, client, undefined, undefined, undefined)
      }
    }
...
function parseState (state, specified, cache, client, network, api, apiKey) {
  return wrap(get())

  function wrap (s) {
    if (!s) return s
    _wrap(s)
    s.$wrapper.cache = cache
    s.$wrapper.client = client
    return s
  }
  ...
  function get () {
    ...
    if (typeof state === 'undefined' && specified) return state
