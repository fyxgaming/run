...
class Run {
  ...
  constructor (options = {}) {
    ...
    this._kernel._blockchain = parseBlockchain(options.blockchain, keys.includes('blockchain'), this._api, this._apiKey, network, this._kernel._cacheWrapper())
...
function parseTrust (trust, specified) {
  ...
  if (typeof trust === 'undefined' && !specified) trust = Array.from(lastTrusts || [])
  ...
  // Add the default trustlist if there is one
  if (Array.isArray(Run.defaults.trust)) trust = trust.concat(Run.defaults.trust)
  if (typeof Run.defaults.trust === 'string') trust.push(Run.defaults.trust)
