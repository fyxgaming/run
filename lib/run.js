...
const { _text, _defineGetter, _limit, _hasOwnProperty } = require('./util/misc')
...
const RemoteBlockchain = require('./plugins/remote-blockchain')
...
function parseCache (cache) {
  ...
  if (typeof cache === 'undefined') {
    ...
    if (Run.instance && Run.instance.cache) return Run.instance.cache

    // If we are running outside the browser, use a simple local cache
    if (typeof window === 'undefined') return new LocalCache()
...
function parseTrust (trust) {
  ...
  // Trust all built-in extras by default
  const extras = [
    require('./extra/asm'),
  ...
  extras.filter(T => _hasOwnProperty(T, 'presets')).forEach(T => {
    if (T.presets.main) trust.push(T.presets.main.location.slice(0, 64))
    if (T.presets.test) trust.push(T.presets.test.location.slice(0, 64))
  })
...
const warnPlugins = name => console.warn(`Run.${name} deprecated. Use Run.plugins.${name}.`)
const warnUtil = name => console.warn(`Run.${name} deprecated. Use Run.util.${name}.`)
...
Run.plugins.RemoteBlockchain = RemoteBlockchain
...
_defineGetter(Run, 'RemoteBlockchain', () => { warnPlugins('RemoteBlockchain'); return RemoteBlockchain })
...
Run.module = {}
_defineGetter(Run.module, 'BrowserCache', () => { warnPlugins('BrowserCache'); return BrowserCache })
_defineGetter(Run.module, 'IndexedDbCache', () => { warnPlugins('IndexedDbCache'); return IndexedDbCache })
_defineGetter(Run.module, 'Inventory', () => { warnPlugins('Inventory'); return Inventory })
_defineGetter(Run.module, 'LocalCache', () => { warnPlugins('LocalCache'); return LocalCache })
_defineGetter(Run.module, 'LocalOwner', () => { warnPlugins('LocalOwner'); return LocalOwner })
_defineGetter(Run.module, 'LocalPurse', () => { warnPlugins('LocalPurse'); return LocalPurse })
_defineGetter(Run.module, 'MatterCloud', () => { warnPlugins('MatterCloud'); return MatterCloud })
_defineGetter(Run.module, 'Mockchain', () => { warnPlugins('Mockchain'); return Mockchain })
_defineGetter(Run.module, 'PayServer', () => { warnPlugins('PayServer'); return PayServer })
_defineGetter(Run.module, 'RemoteBlockchain', () => { warnPlugins('RemoteBlockchain'); return RemoteBlockchain })
_defineGetter(Run.module, 'RunConnect', () => { warnPlugins('RunConnect'); return RunConnect })
_defineGetter(Run.module, 'Viewer', () => { warnPlugins('Viewer'); return Viewer })
_defineGetter(Run.module, 'WhatsOnChain', () => { warnPlugins('WhatsOnChain'); return WhatsOnChain })
