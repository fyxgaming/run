...
const { _wrap } = require('./kernel/wrap')
...
const RunSDKCache = require('./plugins/run-sdk-cache')
const RunSDKBlockchain = require('./plugins/run-sdk-blockchain')
...
function parseBlockchain (blockchain, specified, cache, client, api, apiKey, network) {
  ...
  function hook (b) {
    if (b instanceof RunSDKBlockchain) b.hook(cache, client)
    return b
  }
...
function parseCache (cache, network, specified) {
  ...
  function hook (c) {
    if (c instanceof RunSDKCache) c.hook()
    return c
  }
...
Run.plugins.RunSDKBlockchain = RunSDKBlockchain
Run.plugins.RunSDKCache = RunSDKCache
