...
const { _metadata } = require('./kernel/loader')
...
const RunConnect2 = require('./plugins/run-connect2')
const RunDB = require('./plugins/run-db')
const RunState = require('./plugins/run-state')
...
class Run {
  ...
  constructor (options = {}) {
    ...
    this._kernel._cache = parseCache(options.cache, options.network)
    this._kernel._client = parseClient(options.client)
    this._kernel._blockchain = parseBlockchain(options.blockchain, options.api, options.apiKey, options.network, this._kernel._cacheAPI())
  ...
  set api (api) {
    this._kernel._blockchain = parseBlockchain(undefined, api,
      this.blockchain.apiKey, this.blockchain.network, this._kernel._cacheAPI())
  }

  set apiKey (apiKey) {
    this._kernel._blockchain = parseBlockchain(undefined, this.blockchain.api,
  ...
  set cache (cache) { this._kernel._cache = null; this._kernel._cache = parseCache(cache, this.network) }
...
function parseBlockchain (blockchain, api, apiKey, network, cache) {
...
function parseCache (cache, network) {
  ...
  if (typeof cache === 'undefined') {
    ...
    // If we are on mainnet, prefer RunState
    if (network === 'main') return new RunState()
...
Run.plugins.RunConnect2 = RunConnect2
Run.plugins.RunDB = RunDB
Run.plugins.RunState = RunState
...
_defineGetter(Run, 'RunState', () => { warnPlugins('RunState'); return RunState })
...
_defineGetter(Run.module, 'RemoteBlockchain', () => { warnPlugins('RemoteBlockchain', rm); return RemoteBlockchain })
...
_defineGetter(Run.module, 'RunState', () => { warnPlugins('RunState', rm); return RunState })
...
Run._network = require('./util/network')
...
Run.util.metadata = rawtx => {
  ...
  return _metadata(new bsv.Transaction(rawtx))
}
