...
const StandardLock = require('./kernel/standard-lock')
const Sandbox = require('./kernel/sandbox')
const Log = require('./kernel/log')
const { _text } = require('./kernel/type')
...
class Run {
  constructor (options = {}) {
    ...
    this._kernel._importLimit = typeof options.importLimit === 'undefined' ? 5 : options.importLimit
  ...
  async load (location, _BerryClass) {
    this._checkActive()
    return this._kernel._loader._load(location, { _BerryClass })
  }

  install (T) {
    this._checkActive()
    return Repository._install(T)
  }

  async deploy (T) {
    this._checkActive()
    const C = Repository._install(T)
    Repository._deploy(C)
    await C.sync()
    return C.location
  }
...
Run.configure = (env, network) => {
  ...
  Run._Log._logger = Run.defaults.logger
...
Run._REST = require('./kernel/rest')
Run._Snapshot = require('./kernel/snapshot')
Run._type = require('./kernel/type')
...
Run.protocol = require('./kernel/record')._PROTOCOL_VERSION
