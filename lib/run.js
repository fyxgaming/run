...
class Run {
  ...
  set networkRetries (networkRetries) {
    this._networkRetries = REST.defaults.retries = parseNetworkRetries(networkRetries)
    if (isActive(this)) REST.defaults.retries = parseNetworkRetries(networkRetries)
  }

  set networkTimeout (networkTimeout) {
    this._networkTimeout = parseNetworkTimeout(networkTimeout)
    if (isActive(this)) REST.defaults.timeout = parseNetworkTimeout(networkTimeout)
  }
...
function parseBlockchain (blockchain, api, apiKey, network, cache) {
  ...
  // Check the API
  if (!(typeof api === 'undefined' || api === 'run' || api === 'bitindex' || api === 'mattercloud' || api === 'whatsonchain')) {
