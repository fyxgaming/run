...
class Run {
  ...
  load (location, options = {}) {
    checkActive(this)
    _checkState(!Transaction._ATOMICALLY_UPDATING, 'load disabled during atomic update')
  ...
  sync () {
    _checkState(!Transaction._ATOMICALLY_UPDATING, 'sync all disabled during atomic update')
  ...
  import (rawtx, options = {}) {
    _checkState(!Transaction._ATOMICALLY_UPDATING, 'import disabled during atomic update')
  ...
  activate () {
    ...
    _checkState(!Transaction._ATOMICALLY_UPDATING, 'activate disabled during atomic update')
    Run.instance = this
  ...
  deactivate () {
    ...
    _checkState(!Transaction._ATOMICALLY_UPDATING, 'deactivate disabled during atomic update')
    Run.instance = null
    if (this._inventory) this._inventory.deactivate()
  ...
  unify (...jigs) {
    ...
    _checkArgument(!jigs.some(jig => !(jig instanceof Creation)), 'Must only unify jigs')
