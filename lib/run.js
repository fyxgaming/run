...
class Run {
  ...
  constructor (options = {}) {
    ...
    this._kernel._blockchain = parseBlockchain(options.blockchain, keys.includes('blockchain'), this._kernel._cacheWrapper(), this._kernel._client, this._api, this._apiKey, network)
    this._kernel._state = parseState(options.state, network, this._api, this._apiKey, keys.includes('state'))
