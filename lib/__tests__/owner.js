...
describe('Owner', () => {
  ...
  describe('jigs', () => {
    ...
    test('sync', async () => {
      ...
      const c = new A()
      await run2.sync()
      expect(run2.owner.jigs).toEqual([a, b, c])
    })

    test('fail to post with sync', async () => {
      const run = createRun()
      hookPay(run, false)
      class A extends Jig {}
      const a = new A()
    ...
    test('fail to post without sync', async () => {
      const run = createRun()
      hookPay(run, false)
      class A extends Jig {}
      const a = new A()
      ...
      while (true) {
        try { a instanceof Jig } catch (e) { break } // eslint-disable-line
        await sleep(10)
      }
      expect(run.owner.jigs.length).toBe(0)
      expect(run.owner.code.length).toBe(0)
