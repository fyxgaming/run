...
class WrappedState {
  ...
  setWrappingEnabled (enabled) {
    if (enabled) {
      this.pull = WrappedState.prototype.wrappedPull
      this.broadcast = this.unwrappedBroadcast && WrappedState.prototype.wrappedBroadcast
      this.locations = this.unwrappedLocations && WrappedState.prototype.wrappedLocations
    } else {
