...
const SPEND_DEDUP_CACHE = {}
const SPEND_RESULT_CACHE = {}
const SPEND_CACHE_DURATION = 1000
...
async function _fetchAndCacheSpend (txid, vout, network, cache, apiKey) {
  const key = `${network}-${txid}_o${vout}`

  return _dedupRequest(SPEND_DEDUP_CACHE, key, async () => {
    // Cache spends for 1 second to speed up loads
    return _cacheResponse(SPEND_RESULT_CACHE, key, SPEND_CACHE_DURATION, async () => {
