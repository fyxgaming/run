...
class RunConnect {
  ...
  constructor (options = {}) {
    ...
    this.url = 'https://api.run.network'
  }
  ...
  async broadcast (rawtx) {
    ...
    const url = `${this.url}/v1/${this.network}/tx`
...
async function _fetchAndCacheTransactionData (txid, network, request, cache) {
  ...
  const url = `${this.url}/v1/${network}/tx/${txid}`
...
async function _fetchAndCacheUtxos (scripthash, network, request, cache) {
  ...
  const url = `${this.url}/v1/${network}/utxos/${scripthash}`
...
async function _fetchAndCacheStates (location, network, request, cache) {
  ...
  try {
    const url = `${this.url}/v1/${network}/state/${encodeURIComponent(location)}?all=1&filter=${base64}`
    const options = { cache: 1000, response }
    return await request(url, options)
  } catch (e) {
