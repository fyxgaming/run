...
class WhatsOnChain {
  ...
  async broadcast (rawtx) {
    ...
    const txid = await REST._request(url, options)
    ...
    return REST._cache(REQUEST_RESULT_CACHE, key, REQUEST_CACHE_DURATION, async () => {
      try {
        ...
        const [jsonResult, hexResult] = await Promise.all([
          REST._request(jsonUrl, undefined, _headers(apiKey)),
          REST._request(hexUrl, undefined, _headers(apiKey))
        ])
        const data = {
          rawtx: hexResult,
...
async function _fetchAndCacheUtxos (scripthash, script, network, cache, apiKey) {
  ...
  return REST._dedup(REQUEST_DEDUP_CACHE, key, async () => {
    ...
    return REST._cache(REQUEST_RESULT_CACHE, key, REQUEST_CACHE_DURATION, async () => {
      ...
      const data = await REST._request(url, { headers: _headers(apiKey) })
