...
async function _fetchAndCacheTransactionData (txid, network, request, cache, apiKey) {
  const jsonResponse = async json => {
    // Verify
    if (typeof json.time !== 'number' || json.time < 0) throw new Error(`Invalid time fetched for ${txid}: ${json.time}`)
  ...
  try {
    ...
    const [time, rawtx] = await Promise.all([
      request(jsonUrl, undefined, { headers: _headers(apiKey), cache: 1000, response: jsonResponse }),
      request(hexUrl, undefined, { headers: _headers(apiKey), cache: 1000, response: hexResponse })
    ])
    return { rawtx, time }
  } catch (e) {
