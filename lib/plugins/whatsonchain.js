...
class WhatsOnChain {
  ...
  async broadcast (rawtx) {
    ...
    const txid = await request(url, options)
  ...
  async utxos (script) {
    ...
    const utxos = await _fetchAndCacheUtxos(scripthash, script, this.network, this.cache, this.apiKey)
...
async function _fetchAndCacheTransactionData (txid, network, cache, apiKey) {
  ...
  return _dedup(REQUEST_DEDUP_CACHE, key, async () => {
    // Cache 404s for 1 second to speed up loads
    return _cache(REQUEST_RESULT_CACHE, key, REQUEST_CACHE_DURATION, async () => {
      try {
        // Fetch
        ...
        const [jsonResult, hexResult] = await Promise.all([
          request(jsonUrl, undefined, _headers(apiKey)),
          request(hexUrl, undefined, _headers(apiKey))
        ])
        const data = {
          rawtx: hexResult,
...
async function _fetchAndCacheUtxos (scripthash, script, network, cache, apiKey) {
  ...
  return _dedup(REQUEST_DEDUP_CACHE, key, async () => {
    ...
    return _cache(REQUEST_RESULT_CACHE, key, REQUEST_CACHE_DURATION, async () => {
      ...
      const data = await request(url, { headers: _headers(apiKey) })
