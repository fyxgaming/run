...
async function _fetchAndCacheTransactionData (txid, network, cache, apiKey) {
  ...
  try {
    ...
    const [jsonResult, hexResult] = await Promise.all([
      request(jsonUrl, undefined, { headers: _headers(apiKey), cache: 1000 }),
      request(hexUrl, undefined, { headers: _headers(apiKey), cache: 1000 })
    ])
    const data = {
      rawtx: hexResult,
...
async function _fetchAndCacheUtxos (scripthash, script, network, cache, apiKey) {
  ...
  const data = await this.request(url, { headers: _headers(apiKey), cache: 1000 })
