...
class StateServer extends RunSDKState {
  // --------------------------------------------------------------------------
  // constructor
  // --------------------------------------------------------------------------
  ...
  async pull (key, options) {
    ...
    try {
      ...
      states = await request(url, requestOptions)
    } catch (e) {
    ...
    // Cache the states, except for the one we requested, because RunSDKState will cache that
    if (this.cache) {
      await Promise.all(
