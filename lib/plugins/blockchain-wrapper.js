...
class BlockchainWrapper {
  ...
  async wrappedBroadcast (rawtx) {
    ...
    try {
      ...
      throw new Error(`Invalid transaction: ${e.toString()}`)
    }
    ...
    if (this.cache) {
      ...
      if (recentBroadcasts && recentBroadcasts.some(x => x.rawtx === rawtx)) {
        if (Log._infoOn) Log._info(this.tag, 'Already broadcasted', tx.hash)
        return
      }
    }
