...
const WrappedPurse = require('./wrapped-purse')
 ...
 */
class LocalPurse extends WrappedPurse {
  /**
  ...
  async pay (rawtx, parents) {
    ...
    for (const utxo of utxos) {
      ...
      if (isJig(prevTx, utxo.vout)) continue
    ...
    for (let i = numInputsBefore; i < tx.inputs.length; i++) {
      ...
      const sig = await _signature(tx, i, prevout.script, prevout.satoshis, this.bsvPrivateKey)
  ...
  async utxos () {
    ...
    return utxos.filter((o, i) => !this.jigFilter || !isJig(txns[i], o.vout))
  }
}
