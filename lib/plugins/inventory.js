...
class Inventory {
  ...
  async _syncLatest () {
    ...
    for (const location of locations) {
      ...
      let value = null
      try {
        value = await this._run.cache.get(`ban://${location}`)
      } catch (e) {
        // Swallow cache get failures, because it's a cache.
        if (Log._warnOn) Log._warn(TAG, `Failure to get from cache ban://${location}\n\n${e.toString()}`)
      }

      // Banned jigs are skipped over in the inventory - except if it was untrusted, and now its txid is trusted
      if (typeof value === 'object' && (!value.untrusted || !(await this._run._kernel._trusted(value.untrusted)))) {
        continue
      }

      try {
        await this._run.cache.set(`ban://${location}`, false)
      } catch (e) {
