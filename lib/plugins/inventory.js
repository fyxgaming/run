...
class Inventory {
  ...
  activate (run) {
    ...
    const initOwner = async () => {
      try {
        ...
        this._pending.forEach(creation => this._detect(creation))
        this._pending = null
      } catch (e) {
        if (this._initOwner !== initOwner) return
  ...
  _ours (creation) {
    try {
      ...
      // Creations with undefined unbound owners will become ours
      let creationOwner = _sudo(() => creation.owner)
      if (creationOwner instanceof Unbound) creationOwner = creationOwner._value
      if (creationOwner === undefined) return true
