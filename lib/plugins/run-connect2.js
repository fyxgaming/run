...
const { _browser, _dedup, _cache, _text } = require('../util/misc')
const { _scripthash } = require('../util/bsv')
const { _dedupUtxos } = require('../util/network')
...
class RunConnect {
  ...
  async utxos (script) {
    ...
    return await _fetchAndCacheUtxos(scripthash, this.network, this.cache)
  }
...
async function _fetchAndCacheTransactionData (txid, network, cache) {
  ...
  return _dedup(FETCH_DEDUP_CACHE, key, async () => {
    // Cache 404s and spends for 1 second to speed up loads
    return _cache(FETCH_RESULT_CACHE, key, FETCH_CACHE_DURATION, async () => {
      // Fetch
...
async function _fetchAndCacheUtxos (scripthash, network, cache) {
  ...
  return _dedup(UTXOS_DEDUP_CACHE, key, async () => {
    // Cache UTXOs for 1 second to prevent too frequent updates
    return _cache(UTXOS_RESULT_CACHE, key, UTXOS_CACHE_DURATION, async () => {
      // Fetch
