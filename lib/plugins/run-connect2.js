...
async function _fetchAndCacheTransactionData (txid, network, cache) {
  // Dedup
  if (fetchRequests[txid]) return new Promise((resolve, reject) => fetchRequests[txid].push({ resolve, reject }))
  fetchRequests[txid] = []
  ...
  try {
    ...
    // Return
    fetchRequests[txid].forEach(promise => promise.resolve(data))
    return data
  } catch (e) {
    fetchRequests[txid].forEach(promise => promise.reject(e))
    throw e
  } finally {
    delete fetchRequests[txid]
  }
}
