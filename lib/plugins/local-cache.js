...
class LocalCache {
  ...
  async set (key, value) {
    ...
    if (key.startsWith('jig://') && value.kind === 'code') {
      const filter = this._map.get('filter://code') || StateFilter.create()
  ...
  _reduceToFit () {
    ...
    // If there is a code filter, move it to the end to preserve it
    ...
    while (this._sizeBytes > _limit(this._maxSizeBytes)) {
      ...
      // The code filter is not counted in our size, and will be at the end
      if (oldestKey === 'filter://code') return
