...
class LocalCache {
  ...
  _reduceToFit () {
    ...
    this._moveToFront(CONFIG_KEY_CODE_FILTER)
    this._moveToFront(CONFIG_KEY_RECENT_BROADCASTS)
    ...
    while (this._sizeBytes > _limit(this._maxSizeBytes)) {
      ...
      if (oldestKey.startsWith('jig://') && oldestValue.kind === 'code') {
        const filter = this._map.get(CONFIG_KEY_CODE_FILTER)
        if (filter) {
          // Size does not change when we just remove an item
          StateFilter.remove(filter, oldestKey)
          this._map.set(CONFIG_KEY_CODE_FILTER, filter)
        }
      }
