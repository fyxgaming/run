...
class PurseWrapper {
  ...
  async send (script, satoshis) {
    ...
    // Validates the satoshis
    const badSatoshis = typeof satoshis !== 'number' || !Number.isInteger(satoshis) || satoshis < 0
    if (badSatoshis) throw new Error(`Invalid satoshis: ${_text(satoshis)}`)
  ...
  async wrappedPay (rawtx, parents) {
    ...
    if (badParents) throw new Error(`Invalid parents: ${_text(badParents)}`)
