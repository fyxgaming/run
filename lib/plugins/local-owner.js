...
const { _bsvNetwork, _text, _activeKernel } = require('../kernel/misc')
const { _signature, _sighash } = require('../kernel/bsv')
const RunSDKOwner = require('./run-sdk-owner')
 ...
 */
class LocalOwner extends RunSDKOwner {
  // --------------------------------------------------------------------------
  // constructor
  // --------------------------------------------------------------------------
  ...
  constructor (privkey, network = undefined) {
    ...
    try { network = network || _activeKernel()._blockchain.network } catch (e) { }
