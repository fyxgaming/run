...
const { _signature, _sighash } = require('../util/bsv')
  ...
  async sign (rawtx, parents, locks) {
    ...
    for (let i = 0; i < tx.inputs.length; i++) {
      ...
      if (isGroup) {
        ...
        const signedPubkeys = getSignedPubkeys(tx, i, prevout, sigs, locks[i].pubkeys)
...
function getSignedPubkeys (tx, vin, prevout, sigs, pubkeys) {
  const sighashType = bsv.crypto.Signature.SIGHASH_ALL | bsv.crypto.Signature.SIGHASH_FORKID
  const satoshisBN = new bsv.crypto.BN(prevout.satoshis)
  const hashbuf = _sighash(tx, sighashType, vin, prevout.script, satoshisBN)
