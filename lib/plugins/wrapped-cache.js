 ...
 *
 *    - Logging of calls and performance
 ...
 *
 *    new WrappedCache(myCache)
 *
 * or extend
 *
 *    class MyCache extends WrappedCache { ... }
 */
...
class WrappedCache {
  constructor (cache = this) {
    if (!(cache instanceof Cache)) {
      throw new Error(`${this.constructor.name} does not implement Cache`)
    }
  ...
  setWrappingEnabled (enabled) {
    if (enabled) {
      this.get = WrappedCache.prototype.wrappedGet
      this.set = WrappedCache.prototype.wrappedSet
    } else {
