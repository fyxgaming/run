 ...
 *
 *    new WrappedOwner(myOwner)
 *
 * or extend your class from it:
 *
 *    class MyOwner extends WrappedOwner { ... }
 */
...
class WrappedOwner {
  ...
  setWrappingEnabled (enabled) {
    if (enabled) {
      this.nextOwner = WrappedOwner.prototype.wrappedNextOwner
      this.sign = WrappedOwner.prototype.wrappedSign
    } else {
