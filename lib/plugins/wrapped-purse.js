 ...
 *
 *    new WrappedPurse(myPurse)
 *
 * or extend your class from it:
 *
 *    class MyPurse extends WrappedPurse { ... }
 */
...
class WrappedPurse {
  ...
  setWrappingEnabled (enabled) {
    if (enabled) {
      this.pay = WrappedPurse.prototype.wrappedPay
      this.broadcast = WrappedPurse.prototype.wrappedBroadcast
    } else {
  ...
  async wrappedBroadcast (rawtx) {
    if (!this.unwrappedBroadcast) return
