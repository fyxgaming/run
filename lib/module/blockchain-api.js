...
class BlockchainApi {
  ...
  async utxos (scriptOrAddress) {
    ...
    const prior = this.requests.get(script.hash)
    if (prior) return new Promise((resolve, reject) => prior.push({ resolve, reject }))

    // Create a new promise list for other code to piggy-back on
    this.requests.set(script.hash, [])

    try {
      // Query the utxos
      const utxos = await this.api.utxos(script)
      ...
      const correctedUtxos = this.cache.correctForServerUtxoIndexingDelay(dedupedUtxos, script)

      // Notify all other code that was also waiting for this request
      this.requests.get(script.hash).forEach(o => o.resolve(correctedUtxos))

      return correctedUtxos
    } catch (e) {
      // Notify all other code that this request failed
      this.requests.get(script.hash).forEach(o => o.reject(e))

      throw e
    } finally {
      // Whether we succeeded or failed, remove the promises for this request
      this.requests.delete(script.hash)
    }
  }
...
class BlockchainApiCache {
  ...
  correctForServerUtxoIndexingDelay (utxos, script) {
...
class RestApi {
  ...
  async fetch (txid, force = false) {
    const data = await this.get(this.fetchUrl(this.network, txid))
    return this.fetchResp(data)
  }

  async utxos (script) {
    const data = await this.get(this.utxosUrl(this.network, script))
    return this.utxosResp(data, script)
  }
  ...
  async fetchWithTimeout (url, options = {}) {
    ...
    let timedOut = false
    const timerId = setTimeout(() => { timedOut = true; controller.abort() }, this.timeout)

    try {
      return await fetch(url, options).then(res => handleFetchResponse(res, options.method, url))
    } catch (e) {
  ...
  utxosUrl (network, script) { throw new Error('Not implemented') }
  utxosResp (data, script) { throw new Error('Not implemented') }
}
...
class RunConnect extends RestApi {
  ...
  utxosUrl (network, script) { return `https://api.run.network/v1/${network}/utxos/${script.hash}` }
  utxosResp (data, script) { return typeof data === 'string' ? JSON.parse(data) : data }
}
...
class MatterCloud extends RestApi {
  ...
  broadcastUrl (network) { return `https://api.mattercloud.net/api/v3/${network}/tx/send${this.suffix}` }
  broadcastData (tx) { return { rawtx: tx.toBuffer().toString('hex') } }
  fetchUrl (network, txid) { return `https://api.mattercloud.net/api/v3/${network}/tx/${txid}${this.suffix}` }
  fetchResp (data) { const ret = jsonToTx(data); ret.confirmations = ret.confirmations || 0; return ret }
  utxosUrl (network, script) { return `https://api.mattercloud.net/api/v3/${network}/scripthash/${script.hash}/utxo${this.suffix}` }
  utxosResp (data, script) { return data.map(o => { return Object.assign({}, o, { script }) }) }
  get suffix () { return this.apiKey ? `?api_key=${this.apiKey}` : '' }
}
...
class WhatsOnChain extends RestApi {
  ...
  utxosUrl (network, script) { return `https://api.whatsonchain.com/v1/bsv/${network}/script/${script.hash}/unspent` }
  utxosResp (data, script) {
    return data.map(o => {
...
async function handleFetchResponse (res, method, url) {
...
function parseApi (options) {
  switch (typeof options.api) {
    case 'string': {
      const Api = apis.find(x => x.shortName === options.api)
