...
class BlockchainApi {
  constructor (options = {}) {
    ...
    this._remoteBlockchain = parseRemoteBlockchain(options)
  ...
  get api () {
    if (typeof this._remoteBlockchain.api === 'string') {
      return this._remoteBlockchain.api
    } else {
      return 'custom'
    }
  }

  get timeout () { return this._remoteBlockchain.timeout }
  set timeout (value) { this._remoteBlockchain.timeout = value }

  async broadcast (tx) {
    // Populate previous outputs. We need this for script verification.
    // TODO: Should we do this? If we're going to populate, we should just broadcast.
    ...
    await this._remoteBlockchain.broadcast(tx)
...
class RestApi {
  ...
  async broadcast (tx) {
    // Broadcast the transaction
    await REST._post(this._broadcastUrl(this.network), this._broadcastData(tx), this.timeout)
  }

  async fetch (txid, force = false) {
    const data = await REST._get(this._fetchUrl(this.network, txid), this.timeout)
    return this._fetchResp(data)
  }

  async utxos (script) {
    const data = await REST._get(this._utxosUrl(this.network, script), this.timeout)
    return this._utxosResp(data, script)
  }

  get api () { return 'custom' }
