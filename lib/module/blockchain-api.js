...
class BlockchainApiCache {
  ...
  _get (txid) {
    ...
    const expired = Date.now() - tx._fetchedTime > this._expiration
    if (expired) {
  ...
  _fetched (tx) {
    ...
    if (this._transactions.size > this._size) {
...
class RestApi {
  ...
  static get _name () { return 'Unknown' }
  _broadcastUrl (network) { throw new Error('Not implemented') }
  _broadcastData (tx) { throw new Error('Not implemented') }
  _fetchUrl (network, txid) { throw new Error('Not implemented') }
  _fetchResp (data) { throw new Error('Not implemented') }
  _utxosUrl (network, script) { throw new Error('Not implemented') }
  _utxosResp (data, script) { throw new Error('Not implemented') }
}
...
class RunConnect extends RestApi {
  static get _shortName () { return 'run' }
...
class MatterCloud extends RestApi {
  ...
  static get _shortName () { return 'mattercloud' }
...
class WhatsOnChain extends RestApi {
  ...
  static get _shortName () { return 'whatsonchain' }
...
function parseApi (options) {
  switch (typeof options.api) {
    case 'string': {
      const Api = apis.find(x => x._shortName === options.api)
