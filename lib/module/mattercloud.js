...
class MatterCloud extends RemoteBlockchain {
  ...
  async _getTransactionData (txid) {
    ...
    try {
      const json = await REST._get(fetchurl, this.timeout)
    ...
    for (let i = 0; i < tx.outputs.length; i++) {
      query += `${txid}_o${i}`
      if (i !== tx.outputs.length - 1) query += ','
    }
    const spendsurl = `https://txdb.mattercloud.io/api/v1/spends/${query}`
    const json = await REST._get(spendsurl, this.timeout)
