...
class Inventory {
  ...
  async _syncLatest () {
    ...
    for (const location of locations) {
      ...
      // Check if we've previously tried to load this jig and it failed. Don't try again.
      ...
      try {
        ...
        // Assume all other errors are due to non-jig utxos
  ...
  _detect (jig) {
    ...
    if (this._ours(jig)) {
      ...
      if (!existing) console.log('ADD', _text(jig))
      this._jigs.push(jig)
    } else {
      if (existing && Log._infoOn) Log._info(TAG, 'Remove', _text(jig))
      if (existing) console.log('REMOVE2', _text(jig))
    }
  }
