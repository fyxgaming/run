...
const { _text, _Timeout, _backgroundTask } = require('../util/misc')
...
class Inventory {
  ...
  activate (run) {
    ...
    const getOwner = async () => run.owner.owner ? await run.owner.owner() : await run.owner.nextOwner()

    const success = (owner, task) => {
      if (task !== this._getOwnerTask) return
    ...
    this._lock = null
    this._pending = []
    this._getOwnerTask = _backgroundTask(getOwner, success, failure)
  }
  ...
  deactivate () {
    ...
    this._run.off('record', this._listener)
  ...
  async sync () {
    ...
    if (this._getOwnerTask) await this._getOwnerTask
