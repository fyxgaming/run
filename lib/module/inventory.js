...
class Inventory {
  ...
  activate (run) {
    ...
    const getOwnerTask = async () => {
      try {
        const owner = run.owner.owner ? await run.owner.owner() : await run.owner.nextOwner()
        if (this._getOwnerTask !== getOwnerTask) return
    ...
    this._getOwnerTask = getOwnerTask
    this._getOwnerTaskPromise = getOwnerTask()
  }
  ...
  deactivate () {
    ...
    this._run = null
    this._lock = null
    this._getOwnerTaskPromise = null
    this._pending = null
  }
  ...
  async sync () {
    ...
    if (this._getOwnerTaskPromise) await this._getOwnerTaskPromise
