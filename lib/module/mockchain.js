...
class Mockchain {
  constructor () {
    ...
    this._allowFundingBroadcasts = false // Whether funding transactions with not inputs are allowed
  }
  ...
  async broadcast (rawtx) {
    ...
    if (Log._infoOn) Log._info(TAG, 'Broadcast', txid)
    ...
    if (!this._allowFundingBroadcasts && tx.inputs.length === 0) throw new Error('tx has no inputs')
    if (tx.outputs.length === 0) throw new Error('tx has no outputs')
    if (!this._allowFundingBroadcasts && tx.getFee() < tx.toBuffer().length * Transaction.FEE_PER_KB / 1000) throw new Error('insufficient priority')
    if (!this._allowFundingBroadcasts && tx.verify() !== true) throw new Error(tx.verify())
    ...
    if (chainlen > _limit(this.mempoolChainLimit)) {
  ...
  async fetch (txid) {
    if (Log._infoOn) Log._info(TAG, 'Fetch', txid)
  ...
  async utxos (script) {
    if (Log._infoOn) Log._info(TAG, 'Utxos', script.toString())
  ...
  async time (txid) {
    if (Log._debugOn) Log._debug(TAG, 'Time', txid)
  ...
  async spends (txid, vout) {
    if (Log._infoOn) Log._info(TAG, `Spends ${txid}_o${vout}`)

    const location = `${txid}_o${vout}`
    if (!this._spends.has(location)) throw new Error(`location not found: ${location}`)
    return this._spends.get(location)
  }
  ...
  fund (address, satoshis) {
    ...
    if (Log._debugOn) Log._debug(TAG, 'Broadcast', txid)
