...
class LocalOwner {
  ...
  constructor (options = {}) {
    ...
    const bsvNetwork = options.blockchain && _bsvNetwork(options.blockchain.network)
  ...
  async sign (rawtx, parents, locks) {
    ...
    if (Log._infoOn) Log._info(TAG, 'Sign')
    ...
    for (let i = 0; i < tx.inputs.length; i++) {
      ...
      if (isStandardLock || isPayToPublicKeyHashOut) {
        const sig = _signature(tx, i, new Script(parents[i].script), parents[i].satoshis, this.bsvPrivateKey)
      ...
      const isGroup = locks[i] &&
        locks[i].constructor.name === 'Group' &&
        locks[i].pubkeys.includes(this.pubkey) &&
        tx.inputs[i].script.chunks.length <= locks[i].m

      if (isGroup) {
        const sig = _signature(tx, i, parents[i].script, parents[i].satoshis, this.bsvPrivateKey)
