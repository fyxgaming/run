...
class LocalOwner {
  ...
  constructor (options = {}) {
    ...
    // Note: We check both pk.network and pk.network.name because the bsv library
    // sets up keys in both forms. 

    //    new PrivateKey('testnet')                 -> pk.network.name
    //    new PrivateKey(undefined, 'testnet')      -> pk.network
    ...
    if (bsvNetwork && options.privkey && options.privkey instanceof PrivateKey && 
      options.privkey.network !== bsvNetwork && options.privkey.network.name !== bsvNetwork) {
