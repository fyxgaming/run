...
class LocalOwner {
  ...
  async sign (rawtx, utxos) {
    ...
    for (let i = 0; i < tx.inputs.length; i++) {
      ...
      const lockName = utxos[i] && utxos[i].lock && utxos[i].lock.constructor.name

      // Sign P2PKH inputs
      const isStandardLock = lockName === 'StandardLock' && utxos[i].lock.address === this.address
      const isPayToPublicKeyHashOut = tx.inputs[i].output && tx.inputs[i].output.script.isPublicKeyHashOut() &&
      ...
      if (lockName === 'GroupLock' && utxos[i].lock.pubkeys.includes(this.pubkey) &&
          tx.inputs[i].script.chunks.length <= utxos[i].lock.m) {
        const sig = tx.signature(i, this.bsvPrivateKey).toString('hex')
