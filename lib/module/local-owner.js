...
class LocalOwner {
  ...
  async sign (rawtx, parents, locks) {
    ...
    for (let i = 0; i < tx.inputs.length; i++) {
      ...
      const isStandardLock = locks[i] &&
        locks[i].constructor.name === 'StandardLock' &&
        locks[i].address === this.address
      ...
      const isGroupLock = locks[i] &&
        locks[i].constructor.name === 'GroupLock' &&
