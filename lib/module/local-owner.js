...
class LocalOwner {
  ...
  async sign (tx, locks) {
    ...
    for (let i = 0; i < tx.inputs.length; i++) {
      // Sign P2PKH inputs
      if (locks[i] instanceof StandardLock) {
        const sig = tx.signature(i, this.bsvPrivateKey)
        const script = Script.buildPublicKeyHashIn(this.bsvPublicKey, sig.toDER())
        tx.inputs[i].setScript(script)
      }

      // Sign multi-sig inputs
      if (locks[i] instanceof Group) {
        // TODO
      }
    }
