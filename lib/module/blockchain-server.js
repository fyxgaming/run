 ...
 *
 * Blockchain API implementation that connects to a REST server
 */
...
class BlockchainServer {
  constructor (options = {}) {
    ...
    this.api = parseApi(options.api, options.timeout, this.logger, this.network)
...
function parseApi (api, timeout, logger, network) {
  switch (typeof api) {
    case 'string': {
      const apiData = Object.values(BlockchainServer.api).find(api => api.name === api)
    ...
    case 'object':
      ...
      return api
    case 'undefined': return new RunConnect({ timeout, logger, network })
...
class Api {
  ...
  async broadcast (tx) {
    // Broadcast the transaction
    await this._post(this.broadcastUrl(this.network), this.broadcastData(tx))
  }

  async fetch (txid, force = false) {
    const data = (await this._get(this.fetchUrl(this.network, txid))).data
    return this.fetchResp(data)
  }

  async utxos (address) {
    const data = (await this._get(this.utxosUrl(this.network, address))).data
...
class RunConnect extends Api {
  static get name () { return 'run' }
...
class MatterCloud extends Api {
  static get name () { return 'mattercloud' }
  broadcastUrl (network) { return `https://api.bitindex.network/api/v3/${network}/tx/send` }
  broadcastData (tx) { return { rawtx: tx.toBuffer().toString('hex') } }
  fetchUrl (network, txid) { return `https://api.bitindex.network/api/v3/${network}/tx/${txid}` }
  fetchResp (data) { const ret = jsonToTx(data); ret.confirmations = ret.confirmations || 0; return ret }
  utxosUrl (network, address) { return `https://api.bitindex.network/api/v3/${network}/addr/${address.toString()}/utxo` }
  utxosResp (data, address) { return data.map(o => { return Object.assign({}, o, { script: new Script(o.scriptPubKey) }) }) }
}

class WhatsOnChain extends Api {
  static get name () { return 'whatsonchain' }
  ...
  fetchUrl (network, txid) { return `https://api.whatsonchain.com/v1/bsv/${network}/tx/hash/${txid}` }
  fetchResp (data) { const ret = jsonToTx(data); ret.confirmations = ret.confirmations || 0; return ret }
...
BlockchainServer.api = { RunConnect, MatterCloud, WhatsOnChain }
