...
class LocalCache {
  ...
  async set (key, value) {
    ...
    if (had && !deepEqual(value, previous)) {
  ...
  static _estimateSize (value) {
    ...
    switch (typeof value) {
      ...
      case 'number':
        ...
        throw new Error(`Cannot cache number: ${_display(value)}`)
      ...
      case 'symbol': throw new Error(`Cannot cache symbol: ${_display(value)}`)
    }
  }
}
...
function parseMaxSizeMB (maxSizeMB) {
  ...
  throw new Error(`Invalid maxSizeMB: ${_display(maxSizeMB)}`)
}
