...
class LocalCache {
  ...
  get maxSizeMB () { return this._maxSizeMB / 1000 / 1000 }
  set maxSizeMB (value) { this._maxSizeBytes = parseMaxSizeMB(value) * 1000 * 1000 }
  ...
  static _estimateSize (value) {
    ...
    switch (typeof value) {
      ...
      case 'function': throw new Error(`Cannot cache function: ${_display(value)}`)
      default: return 5
    }
  }
}
...
function parseMaxSizeMB (maxSizeMB) {
  ...
  if (typeof maxSizeMB === 'number' && Number.isFinite(maxSizeMB) && maxSizeMB >= 0) return maxSizeMB
  throw new Error(`Invaid maxSizeMB: ${_display(maxSizeMB)}`)
}
