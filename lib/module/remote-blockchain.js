...
const { UnimplementedError, RequestError } = require('../util/errors')
...
class RemoteBlockchain {
  ...
  async _postTransaction (rawtx) { throw new UnimplementedError() }
  async _getTransactionData (txid) { throw new UnimplementedError() }
  async _getUtxos (scripthash, script) { throw new UnimplementedError() }
  ...
  async time (txid) {
    if (Log._infoOn) Log._info(TAG, 'Time', txid)
...
class WhatsOnChain extends RemoteBlockchain {
  ...
  async _postTransaction (rawtx) {
    ...
    await REST._post(url, { txhex: rawtx }, this.timeout, this._headers)
  }
  ...
  async _getTransactionData (txid) {
    ...
    try {
      const [jsonResult, hexResult] = await Promise.all([
        REST._get(jsonUrl, this.timeout, this._headers),
        REST._get(hexUrl, this.timeout, this._headers)
      ])
  ...
  async _getUtxos (scripthash, script) {
    ...
    const data = await REST._get(url, this.timeout, this._headers)
