...
class LocalPurse {
  ...
  async pay (rawtx, utxos) {
    ...
    let purseUtxos = await this.blockchain.utxos(this.script)
    ...
    for (const utxo of purseUtxos) {
      ...
      const numOutputsToAdd = this.splits - purseUtxos.length + numUtxosSpent - numOutputsToCreate
