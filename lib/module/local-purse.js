...
class LocalPurse {
  ...
  async pay (txhex, remaining) {
    const tx = new Transaction(txhex)

    if (typeof remaining === 'undefined') remaining = 1000 * tx.inputs.length
    ...
    const baseSatoshisRequired = Math.max(500, tx._estimateFee() + tx._getOutputAmount()) + remaining
    ...
    if (satoshisSpentBefore - satoshisReceivedBefore >= (tx.toBuffer().length + remaining) * _feeFactor) {
      Log._debug(TAG, 'Transaction already paid for. Skipping.')

      // Collect change if leftover after fees is bigger than the tx fee + 40 bytes (P2PKH avg)
      if (satoshisSpentBefore - satoshisReceivedBefore > 40 + tx.toBuffer().length * _feeFactor) {
      ...
      for (const utxo of utxos) {
        ...
        numUtxosSpent += 1
        // 108 bytes per P2PKH input seems to be a maximum
        satoshisRequired += Math.ceil(108 * _feeFactor)

        const numOutputsToAdd = this.splits - utxos.length + numUtxosSpent - numOutputsToCreate
        for (let i = 0; i < numOutputsToAdd; i++) {
          satoshisRequired += Math.ceil(40 * _feeFactor) // 40 bytes per P2PKH output seems to be average
          satoshisRequired += 546 // We also have to add the dust amounts
          numOutputsToCreate += 1
        }

        if (satoshisSpentTotal > satoshisRequired + remaining) break
      }

      // Make sure we have enough utxos
      if (satoshisSpentTotal <= satoshisRequired + remaining) {
        const info = `Required ${satoshisRequired + remaining} satoshis, ${satoshisSpentTotal} available in ${this.address}`
        throw new Error(`Not enough funds\n\n${info}`)
      }
      ...
      const satoshisPerOutput = Math.ceil((satoshisAddedInUtxos - satoshisRequiredForFees) / numOutputsToCreate)
      for (let i = 0; i < numOutputsToCreate; i++) {
        if (i === numOutputsToCreate - 1) {
          let txSize = tx.toBuffer().length // start with current size
          txSize += 40 // add the change P2PKH output
          txSize += numUtxosSpent * 108 // add each P2PKH input we'll sign
          txSize += remaining // add other signatures we haven't seen yet
          tx._fee = Math.ceil(txSize * _feeFactor) // calculate a fee
          tx.change(this.bsvAddress)
        } else {
    ...
    Log._debug(TAG, 'Paid about', paid, 'satoshis,', tx.getFee(), 'in fees')

    // Return the paid tx in hex
