...
class LocalPurse {
  ...
  async pay (txhex, remaining) {
    ...
    if (satoshisSpentBefore - satoshisReceivedBefore >= (tx.toBuffer().length + remaining) * _feeFactor) {
      ...
      // Collect change if leftover after fees is bigger than the tx fee + 37 bytes (P2PKH avg)
      if (satoshisSpentBefore - satoshisReceivedBefore > (37 + tx.toBuffer().length) * _feeFactor) {
      ...
      for (const utxo of utxos) {
        ...
        for (let i = 0; i < numOutputsToAdd; i++) {
          satoshisRequired += Math.ceil(37 * _feeFactor) // 37 bytes per P2PKH output seems right
      ...
      for (let i = 0; i < numOutputsToCreate; i++) {
        if (i === numOutputsToCreate - 1) {
          ...
          txSize += 37 // add the change P2PKH output
