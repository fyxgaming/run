...
class LocalPurse {
  ...
  async pay (txhex) {
    ...
    if (satoshisSpentBefore - satoshisReceivedBefore >= tx.toBuffer().length * _feeFactor) {
      ...
      for (const utxo of utxos) {
        ...
        if (satoshisSpentTotal > satoshisRequired) break
      }

      // Make sure we have enough utxos
      if (satoshisSpentTotal <= satoshisRequired) {
        const info = `Required ${satoshisRequired} satoshis, ${satoshisSpentTotal} available in ${this.address}`
        throw new Error(`Not enough funds\n\n${info}`)
      }
