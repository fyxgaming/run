...
// A default signature size when we don't know. Allows up to 3-3 multisig.
const DEFAULT_SIGNATURE_SIZE = 500
...
class LocalPurse {
  ...
  async pay (txhex, inputs) {
    ...
    for (let i = 0; i < inputs.length; i++) {
      const prevtx = new Transaction(inputs[i])
      tx.inputs[i].output = prevtx.outputs[tx.inputs[i].outputIndex]
      if (!tx.inputs[i].script.toBuffer().length) {
        const Buffer = bsv.deps.Buffer
        const script = new Script(Buffer.alloc(DEFAULT_SIGNATURE_SIZE))
        tx.inputs[i].setScript(Buffer.alloc(DEFAULT_SIGNATURE_SIZE))
      }
    }
