...
class LocalPurse {
  ...
  async pay (rawtx, parents) {
    ...
    for (const utxo of utxos) {
      ...
      if (!isJig(prevTx, utxo.vout) !== 'other') continue
    ...
    for (let i = numInputsBefore; i < tx.inputs.length; i++) {
      ...
      const sig = _signature(tx, i, prevout.script, prevout.satoshis, this.bsvPrivateKey)
  ...
  async utxos () {
    ...
    return utxos.filter((o, i) => isJig(txns[i], o.vout) === 'other')
  }
}
