 ...
 * proxy2.js
 *
 * A better proxy that supports Set, Map, and Uint8Array intrinsics.
 *
 ...
 *      _nativeOut (value)      // get(), forEach()
 *      _nativeIn (value)       // add(), set()
 *      _nativeRead ()          // has(), includes()
 *      _nativeUpdate ()        // clear(), delete(), etc.
 */
...
const RETURN_THIS_METHODS = ['add', 'copyWithin', 'fill', 'reverse', 'set']
const RETURN_ITERATOR_METHODS = ['entries', 'keys', 'values', Symbol.iterator]
const RETURN_VALUE_METHODS = ['find', 'get']
...
class Proxy2 {
  ...
  get (target, prop, receiver) {
    ...
    if (isIntrinsicMethod) {
      ...
      const methodProxy = new Proxy(value, new IntrinsicMethodHandler(prop))
...
class IntrinsicMethodHandler {
  constructor (prop) {
    ...
    this._returnsThis = RETURN_THIS_METHODS.includes(prop)
    this._returnsValue = RETURN_VALUE_METHODS.includes(prop)
    this._returnsIterator = RETURN_ITERATOR_METHODS.includes(prop)
  }
  ...
  apply (target, thisArg, args) {
    ...
    // TODO: Call nativeSet on args if necessary
    ...
    if (this._returnsValue) return handler && handler._nativeOut ? handler._nativeOut(ret) : ret

    // Iterator need to be specially handled
    if (this._returnsIterator) {
      // TODO
      return ret
    }
...
// Iterator needs to be converted manually
