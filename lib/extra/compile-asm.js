...
function compileAsm (asm) {
  ...
  for (const next of tokens) {
    ...
    if (typeof compileAsm.OP_CODES[next] !== 'undefined') {
      out.push(compileAsm.OP_CODES[next])
      continue
    }
    ...
    if (bytes.length < 256) {
      out = out.concat(compileAsm.OP_CODES.OP_PUSHDATA1).concat([bytes.length]).concat(bytes)
      continue
    }
    ...
    if (bytes.length < 256 * 256) {
      ...
      out = out.concat(compileAsm.OP_CODES.OP_PUSHDATA2).concat(len).concat(bytes)
      continue
    }
    ...
    if (bytes.length < 256 * 256 * 256 * 256) {
      ...
      out = out.concat(compileAsm.OP_CODES.OP_PUSHDATA4).concat(len).concat(bytes)
      continue
    }
...
compileAsm.OP_CODES = OP_CODES

compileAsm.deps = { parseHex }
