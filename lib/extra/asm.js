...
function asm (s) {
  ...
  for (const part of parts) {
    ...
    if (bytes.length < 256 * 256) {
      const len = [Math.floor(bytes.length / 256), bytes.length % 256]
    ...
    const len = [
      Math.floor(bytes.length / 256 / 256 / 256),
      Math.floor(bytes.length / 256 / 256) % 256,
      Math.floor(bytes.length / 256) % 256,
      bytes.length % 256
    ]
...
module.exports = Code._preinstall(asm)
