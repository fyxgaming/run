...
owner: ${spentJigs[i].owner}`)
   ...
   * @param {Set<txid: string>} fetchedTxids Transaction IDs that were force-refreshed already
   * @param {Map<origin: string, latestState: Jig>} synced jigs already updated
   */
  async fastForward (jig, fetchedTxids = new Set(), synced = new Map()) {
    ...
    let { txid, vout } = Location.parse(jig.location)
    let tx = await this.blockchain.fetch(txid, !fetchedTxids.has(txid))
    ...
    while (true) {
      ...
      if (typeof output.spentTxId === 'undefined') {
        ...
        const reason = 'The current blockchain API does not support the spentTxId field.'
      ...
      tx = await this.blockchain.fetch(output.spentTxId, !fetchedTxids.has(txid))
      fetchedTxids.add(output.spentTxId)
      const input = tx.inputs[output.spentIndex]
      if (input.prevTxId.toString('hex') !== txid || input.outputIndex !== vout) {
      ...
      // Import it and find our updated jig
      ...
      // Set the next txid and vout to do another update
      const parsed = Location.parse(jig.location)
      txid = parsed.txid
      vout = parsed.vout
    }
    ...
    this.fastForwardInnerTokens(jig, fetchedTxids, synced)
  }

  async fastForwardInnerTokens (x, fetchedTxids, synced) {
    ...
    for (const token of xray.tokens) {
      if (token !== x && token instanceof Jig) {
        await this.fastForward(token, fetchedTxids, synced)
      }
    }
  }
}
