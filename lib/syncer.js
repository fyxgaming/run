...
owner: ${spentJigs[i].owner}`)
  ...
  async fastForward (jig, dontRefresh = new Set(), seen = new Map()) {
    ...
    // How do we find all inner jigs, including ones that are inner of this one?
    // deeplyScanJig, or we have some custom code. Depends on what else needs it.
    ...
    for (const token of xray.tokens) {
      if (token !== jig && token instanceof Jig) {
        await this.fastForward(innerJig, dontRefresh, seen)
      }
    }
    */
