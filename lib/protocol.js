...
class Protocol {
  ...
  static async pluckBerry (location, blockchain, code) {
    ...
    async function pluckWithProtocol (protocol) {
      try {
        // TODO: Allow undeployed, with bad locations
        console.log('---')
        const sandboxedProtocol = code.install(protocol)
        console.log('---2')
        ...
        // TODO: Berry should do this
        const loc = Location.parse(berry.location)
        if (!loc.innerLocation) throw new Error(`Protocol must be set on Berry locations: ${berry.location}`)
    ...
    try {
      const locationObj = Location.parse(location)
      if (locationObj.innerLocation) {
        const protocol = Protocol.berryProtocols.find(x => x.location === locationObj.location)
        if (!protocol) throw new Error(`Protocol not installed: ${locationObj.location}`)
        return pluckWithProtocol(protocol)
      }
    } catch (e) { }
    ...
    for (const protocol of Protocol.berryProtocols) {
      try {
        const berry = await pluckWithProtocol(protocol)
        if (berry) return
      } catch (e) {
