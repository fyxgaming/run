...
class Protocol {
  constructor () {
    this.berryProtocols = new Set()
  }

  installBerryProtocol (protocol) {
    if (typeof protocol !== 'function' && typeof protocol.pluck !== 'function') {
      throw new Error(`Cannot install protocol: ${protocol}`)
    }
    this.berryProtocols.add(protocol)
  }

  async pluckBerry (location, blockchain, code) {
    ...
    if (locationObj && locationObj.innerLocation) {
      const protocol = Array.from(this.berryProtocols).find(x => x.location === locationObj.location)
    ...
    const errors = []
    for (const protocol of this.berryProtocols) {
      try {
