...
describe('Viewer', () => {
  describe('constructor', () => {
    it('should support address owners', () => {
      ...
      expect(viewer.locks.length).to.equal(1)
      expect(viewer.locks[0] instanceof StandardLock).to.equal(true)
      expect(viewer.locks[0].address).to.equal(address)
    })
    ...
    it('should support lock object owners', () => {
      ...
      expect(viewer.locks).to.deep.equal([lock])
    })
  ...
  describe('locks', () => {
    it('should always return the lock', () => {
      ...
      const lock = new CustomLock()
      const viewer = new Viewer(lock)
      expect(viewer.locks[0]).to.equal(lock)
      expect(viewer.locks[0]).to.equal(lock)
    })
  })
