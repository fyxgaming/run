...
it.only('RELAY', async () => {
  const owner = 'L3BQB9pYxXhrsHgejfi64EmNi4MDhRyeGwcYmDE6N7Xoai9CpiG8'
  const purse = 'L5K1nrJe65uHPYwbSSAjX78gDquKXsECVeMpDDme3eWtEna8bC3C'
  const run = new Run({ network: 'main', owner, purse })
  run.timeout = 100000
...
export const worker = new Run({
  app: 'relayx.io',
  trust: '*'
});
*/
  const ContractLocation = '656563e66b2d22e71817fb42e0f324589a6dc8581315f33b81173a03bbc9d229_o1'
  // USDR => d1fccff5841bdd61b1f2c03655ac9862118ed641f89803779d05f24f0b0a2499_o1
  ...
  async function loadContract (location) {
    location = location || ContractLocation
  ...
  async function getContract () {
    ...
    contract = await run.load(ContractLocation)
    await contract.sync()
    // try {
    // } catch (e) {
    //   this.contract = null;
  ...
  function getTokenValue (token) {
    ...
    for (let i = 0; i < token.constructor.decimals; i++) amount /= 10
    return amount
  }

  async function getTokens () {
    if (!syncing) {
      syncing = updateTokens()
    }
  ...
  async function getBalance () {
    ...
    console.log(tokens)
    const balance = tokens.reduce((total, token) => {
  ...
  async function updateJigs () {
    try {
      // run.inventory = new Run.Inventory()
      await run.inventory.sync()
      console.log(run.inventory.jigs)
    } catch (e) {
      console.log('update jigs error: ', e.message)
      await updateJigs()
    }
  }
  ...
  async function updateTokens () {
    ...
    console.log('LENGTH', jigs.length)
    const tokens = jigs.filter((jig) => {
      return jig instanceof contract
    })
    syncing = null
    return tokens || []
  }

  async function send (to, amount, contract, tokens) {
    ...
    let sender = null
    let sent = null
    console.log(combines.length)
    if (combines.length >= 2) {
      const C = contract
      sender = new C(...combines)
      // sender = contract.combine(...combines);
    } else {
      sender = combines[0]
    }
    await sender.sync()
    ...
    if (sender) {
      ...
      sent = sender.send(to, amount * (10 ** contract.decimals))
    }
    // run.transaction.end();
    await sender.sync()
    await sent.sync()
    console.log(sender, sender.sender, sender.owner)
  ...
  await send(run.owner.address, 1)
  console.log(await getBalance())
  await send(run.owner.address, 20)
  console.log(await getBalance())
  // await send(run.owner.address, 100)
  // console.log(await getBalance())
  console.log(send)
  ...
  async deploy(contract: any) {
    const run = this.run;
    const location = await run.deploy(contract);
    await run.sync();
    return location;
  }
...
describe('LocalPurse', () => {
  ...
  describe('pay', () => {
    ...
    it('no change if not enough to receive dust', async () => {
      ...
      const tx2 = await payFor(new Transaction().from(utxo).to(run.owner.address, 546), run)
