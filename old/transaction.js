...
class Transaction {
  ...
  // Internal getters to get the actual or playback state
  get _activeRecord () {
    return this._recordOverride || this._record
  }

  _nextResourceOwner () {
    if (this._resourceOwnerOverride) return this._resourceOwnerOverride
    const owner = this._kernel._owner.owner()
    return Array.isArray(owner) ? owner[0] : owner
  }

  // Methods to override the current record and owner during playback
  _override (record, resourceOwner) {
    this._recordOverride = record
    this._resourceOwnerOverride = resourceOwner
  }
  ...
  begin () { this._activeRecord._begin(); return _activeRun() }
  end () { this._activeRecord._end(); return _activeRun() }
  ...
  export () {
    ...
    if (this._record._beginCount === 0) {
      const suggestion = 'Hint: A transaction must first be created using begin() or loaded using import().'
      throw new Error(`No transaction in progress\n\n${suggestion}`)
    }

    return this._record._buildBsvTransaction(this._kernel).tx
  }

  import (tx) {
    Log._info(TAG, 'Importing', tx.hash)
    return this._record._import(tx, this._kernel, null, false)
  }

  rollback () {
    Log._info(TAG, 'Rollback')
    this._record._rollback(this._kernel._publisher._lastPosted, this._kernel, 'intentional rollback', false)
  }

  async sign () { await this._record._sign(this._kernel) }

  async pay () { await this._record._pay(this._kernel) }
  ...
  get actions () {
    return this._record._actions.map(action => {
      return {
        target: this._record._proxies.get(action.target),
   ...
   * from the cache. load() will trigger additional loads recursively.
  ...
  async _loadResource (location, options = {}) {
    ...
    const cachedState = await this._kernel._cache.get('jig://' + location)
    if (cachedState) {
      // Make sure the cached state is valid
      if (typeof cachedState.type !== 'string' || typeof cachedState.state !== 'object') {
        const hint = 'Hint: Could the cache be corrupted?'
